...
class CursorParkManager {
    ...
    async parkCursor(x, y, context) {
        ...
        const pageId = context?.pageId ||
            window.stateManagerInstance?.getState('currentUrlData')?.pageId ||
            'unknown';
        const parkedCursor = {
...
if (typeof window !== 'undefined') {
    window.cursorParkManager = {
        parkCursor: (x, y, context) => cursorParkManagerInstance.parkCursor(x, y, context),
        unparkCursor: (cursorId) => cursorParkManagerInstance.unparkCursor(cursorId),
        toggleParkCursor: (x, y, context) => cursorParkManagerInstance.toggleParkCursor(x, y, context),
        isParked: () => cursorParkManagerInstance.isParked()
    };
    // Export convenience functions
    window.parkCursor = (x, y, context) => cursorParkManagerInstance.parkCursor(x, y, context);
    window.unparkCursor = (cursorId) => cursorParkManagerInstance.unparkCursor(cursorId);
    window.toggleParkCursor = (x, y, context) => cursorParkManagerInstance.toggleParkCursor(x, y, context);
    window.isCursorParked = () => cursorParkManagerInstance.isParked();
...
//# sourceMappingURL=CursorParkManager.js.map