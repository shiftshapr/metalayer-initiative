...
import { normalizeVisibilityUsers, filterCurrentUser } from '../utils/visibilityHelpers.js';
import { AVATAR_FALLBACK_COLOR } from '../../../core/ConfigModule.js';
import { formatUserDisplayName, formatUserHandle, getAuraColorValue, getUserIdentity } from '../../../utils/Fallbacks.js';
/**
 * Refactored VisibilityManager
 * Business logic only - no UI concerns
 */
export class VisibilityManager {
    constructor(realtime, logger, state) {
        this.currentUserId = null; // ROOT CAUSE FIX: Use UUID, not email
        this.currentUserEmail = null; // Keep for filtering compatibility
     ...
     */
    async initialize(currentUserId) {
        ...
        try {
            ...
            this.realtime.on('presence', (eventType, newRecord, oldRecord) => {
    ...
    async refreshVisibilityAvatars(pageId) {
        try {
            ...
            if (!this.isActive) {
                ...
                if (!userId && typeof window !== 'undefined') {
                    ...
                    if (!userId && win.stateManagerInstance) {
                        try {
                            const currentUser = await win.stateManagerInstance.getState('currentUser');
            ...
            if (users && users.length > 0) {
                ...
                const filteredUsers = filterCurrentUser(usersWithAvatars, null, // Email parameter is deprecated/ignored - UUID only
