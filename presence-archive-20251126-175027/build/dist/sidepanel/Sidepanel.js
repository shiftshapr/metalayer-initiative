...
const getVisibilityRefresherFactory = () => {
    // createVisibilityRefresher is imported at top level, return it directly
    return createVisibilityRefresher;
};
...
import { createVisibilityRefresher } from '../features/visibility/index.js';
import { handleError } from '../utils/ErrorHandler.js';
import { Logger } from '../utils/Logger.js';
// Import diagnostic framework to make it available in console
...
if (legacyWindow.__CANOPI_SIDEPANEL_READY__) {
    Logger.warn('Canopi Sidepanel already initialized, skipping duplicate bootstrap.', 'general');
}
else {
    ...
    (async () => {
        try {
            ...
            // Use createVisibilityRefresher if available, otherwise fallback to no-op
            const refreshVisibility = visibilityFactory
                ? visibilityFactory(graph)
                : createVisibilityRefresher(graph);
            ...
            // BEST PRACTICE: Initialize in proper order using await, not setTimeout delays
            // BootController must complete before TabController (communities must be ready)
            await bootController.initialize();
            // TabController will wait for TabManager via event, and wait for communities via ensureCommunitiesReady()
