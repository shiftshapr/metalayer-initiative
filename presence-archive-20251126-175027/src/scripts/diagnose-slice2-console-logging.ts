...
interface ConsoleUsage {
  file: string;
  line: number;
  type: 'log' | 'warn' | 'error' | 'info' | 'debug' | 'other';
  statement: string;
}
...
function analyzeFile(filePath: string): void {
  try {
    ...
    lines.forEach((line, index) => {
      ...
      if (consoleMatch) {
        const type = consoleMatch[1] as ConsoleUsage['type'];
        results.push({
          file: relativePath,
          line: index + 1,
 ...
 */
function generateReport(): DiagnosticResult {
  const byType: Record<string, number> = {};
  const byFile: Record<string, FileReport> = {};
