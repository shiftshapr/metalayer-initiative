...
import type { SupabaseClient, SupabaseAppUserRow, SupabasePresenceRecord, User } from '../types/index.js';
...
// Declare global supabase for fallback (loaded as script tag)
declare const supabase: {
  createClient?: (url: string, key: string) => SupabaseClient;
} | undefined;

type SupabasePolicy = Omit<ModuleErrorPolicy, 'component'>;

const handleSupabaseError = <T = never>(
  error: unknown,
...
async function loadSupabaseClient(): Promise<(url: string, key: string) => SupabaseClient> {
  if (createClient) {
    return createClient;
  }
  ...
  try {
    ...
    const supabaseModule = await (import(supabasePath) as Promise<{
      createClient?: (url: string, key: string) => SupabaseClient;
      default?: {
        createClient?: (url: string, key: string) => SupabaseClient;
      };
    ...
    if (supabaseModule && typeof supabaseModule === 'function') {
      const clientFn = supabaseModule as (url: string, key: string) => SupabaseClient;
      createClient = clientFn;
  ...
  if (typeof window !== 'undefined') {
    const win = window as Window & { supabase?: { createClient: (url: string, key: string) => SupabaseClient } };
  ...
  if (typeof supabase !== 'undefined' && supabase) {
    const supabaseGlobal = supabase as { createClient?: (url: string, key: string) => SupabaseClient };
    ...
    if (typeof supabase === 'object') {
      Logger.debug('🔍 SUPABASE: Global supabase keys:', Object.keys(supabase as object).slice(0, 10), 'supabase');
    }
  }
...
class SupabaseService {
  ...
  private isTriggeringAuth: boolean = false; // Guard to prevent multiple simultaneous auth triggers
  ...
  async initialize(): Promise<SupabaseClient> {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        (window as Window & { supabase?: SupabaseClient }).supabase = this.client;
      }
        ...
        this.client.auth.onAuthStateChange((event: string, session: unknown) => {
          ...
          if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
            if (session && typeof session === 'object' && 'user' in session) {
              const sessionUser = (session as { user?: { email?: string } }).user;
   ...
   */
  private triggerChromeIdentityAuth(): void {
    ...
    setTimeout(async () => {
      try {
        const realGoogleAuth = (window as Window & { realGoogleAuth?: { getCurrentUser: () => Promise<User | null>; initialize: () => Promise<boolean>; initialized?: boolean } }).realGoogleAuth;
   ...
   */
  async getPageUsers(pageId: string): Promise<Array<Partial<User> & { pageId?: string; lastSeen?: string; isActive?: boolean }>> {
    if (!this.client) {
    ...
    try {
      ...
      const { data: presenceData, error: presenceError } = await this.client
        .from<SupabasePresenceRecord>('user_presence')
      ...
      const { data: appUserData, error: appUserError } = await this.client
        .from<SupabaseAppUserRow>('AppUser')
      ...
      const appUserMap = new Map<string, SupabaseAppUserRow>();
      ...
      return presenceData.map((record: SupabasePresenceRecord) => {
   ...
   */
  private async getPageUsersFallback(pageId: string): Promise<Array<Partial<User> & { pageId?: string; lastSeen?: string; isActive?: boolean }>> {
    if (!this.client) {
      return [];
    }
    ...
    try {
      ...
      if (baseDomainMatch) {
        ...
        if (similarData && similarData.length > 0) {
          ...
          return similarData.map((record: SupabasePresenceRecord) => ({
            id: record.user_id || '',
            userId: record.user_id || '',
  ...
  on(_event: string, handler: (eventType: string, newRecord: unknown, oldRecord: unknown) => void): void {
    ...
    channel.on('postgres_changes', 
      ...
      (payload?: { eventType?: string; new?: unknown; old?: unknown; key?: string; newPresences?: unknown[]; leftPresences?: unknown[] }) => {
        // Extract event type from payload
        const eventType = payload?.eventType || (payload as { event?: string })?.event || 'UNKNOWN';
