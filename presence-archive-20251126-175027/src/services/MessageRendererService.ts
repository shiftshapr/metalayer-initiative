...
export class MessageRendererService {
  private unifiedMessageRenderer: typeof UnifiedMessageRenderer;
  private unifiedMessageDisplay: UnifiedMessageDisplay;
  private communitiesModule?: CommunitiesModule;
  private getMessageActionsMenu?: (message: Message, canEdit: boolean, canDelete: boolean) => Promise<string> | string;
  private getCurrentUser?: () => User | null;
  private getCurrentChatData?: () => Message[];
  private formatMessageTime?: (timestamp: string | Date | null) => string;
   ...
   */
  async renderMessage(message: Message, options: RenderMessageOptions = {}): Promise<HTMLElement> {
    const {
      isReply = false,
      isFocusMode = false,
      canEdit = null,
    ...
    if (this.getMessageActionsMenu && typeof window !== 'undefined') {
      const win = window as Window & { getMessageActionsMenu?: (message: Message, canEdit: boolean, canDelete: boolean) => Promise<string> | string };
    ...
    const messageElement = await this.unifiedMessageRenderer.renderMessage(message, {
      ...
      communityName,
      formattedTime,
      reactionCount: reactionCount || (Array.isArray((message as unknown as { reactions?: unknown[] }).reactions) ? ((message as unknown as { reactions: unknown[] }).reactions?.length || 0) : 0),
      replyCount: calculatedReplyCount,
      bookmarkCount: bookmarkCount || (typeof (message as { bookmarkCount?: number }).bookmarkCount === 'number' ? (message as { bookmarkCount: number }).bookmarkCount : 0),
      isBookmarked: isBookmarked || (typeof (message as { isBookmarked?: boolean }).isBookmarked === 'boolean' ? (message as { isBookmarked: boolean }).isBookmarked : false),
      hasUserReplied,
      hasUserReposted,
      hasUserShared,
    ...
    const contentWrapper = messageElement.querySelector('.message-content-wrapper') as HTMLElement | null;
    if (contentWrapper) {
   ...
   */
  async renderMessages(messages: Message[], container: HTMLElement, options: DisplayOptions = {}): Promise<void> {
   ...
   */
  async updateMessage(messageId: string, updates: MessageUpdates): Promise<void> {
    ...
    if (updates.isBookmarked !== undefined) {
      const bookmarkButton = messageElement.querySelector('.bookmark-btn') as HTMLElement | null;
      if (bookmarkButton) {
...
let messageRendererServiceInstance: MessageRendererService | null = null;

/**
 * Initialize the service with dependencies
 */
export function initializeMessageRendererService(dependencies: MessageRendererDependencies): MessageRendererService {
 ...
 */
export function getMessageRendererService(): MessageRendererService {
