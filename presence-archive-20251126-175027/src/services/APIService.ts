...
class MetaLayerAPI {
  ...
  constructor(baseURL: string = API_CONFIG.baseUrl, fallbackURL: string = API_CONFIG.fallbackUrl) {
    this.baseURL = baseURL;
    this.fallbackURL = fallbackURL;
  }

  private _buildUrl(endpoint: string, useFallback = false): string {
  ...
  private _classifyNetworkError(error: unknown): { timeout: boolean; connectionRefused: boolean; error: Error } {
  ...
  async request<T = unknown>(endpoint: string, options: APIRequestOptions = {}): Promise<APIResponse<T>> {
    ...
    let user: { id?: string; email?: string; name?: string; avatarUrl?: string; auraColor?: string } | null = null;
    try {
      ...
      const currentUserRaw = stateManagerInstance?.getState?.('currentUser') as { id?: string; email?: string; name?: string; avatarUrl?: string; auraColor?: string | Promise<string> } | null;
      ...
      if (currentUserRaw && currentUserRaw.id) {
        ...
        Logger.debug('🔍 USER_IDENTITY: ✅ Using currentUser for authentication', { userId: user.id }, 'api');
        // CRITICAL FIX: Handle case where auraColor is a Promise (from reactive systems)
        if (currentUserRaw.auraColor && typeof currentUserRaw.auraColor === 'object' && typeof (currentUserRaw.auraColor as Promise<string>).then === 'function') {
          Logger.debug('🔍 USER_IDENTITY: ⚠️ auraColor is a Promise, resolving...', 'api');
          try {
            const resolvedAuraColor = await (currentUserRaw.auraColor as Promise<string>);
        ...
        if (user && !user.auraColor && user.id) {
          ...
          try {
            ...
            if (response.ok) {
              const userData = await response.json() as { auraColor?: string; avatarUrl?: string; data?: { auraColor?: string; avatarUrl?: string } };
        ...
        if (authManager && typeof authManager.getCurrentUser === 'function') {
          ...
          Logger.debug('🔍 USER_IDENTITY: ✅ Using authManager for authentication', { userId: user?.id }, 'api');
        }
      }
    ...
    const currentUserForHeaders = stateManagerInstance?.getState?.('currentUser') as { id?: string; email?: string; name?: string; avatarUrl?: string } | null;
...
if (typeof window !== 'undefined') {
  const win = window as Window & { api?: unknown };
  if (!win.api) {
    (win as { api?: MetaLayerAPI }).api = apiServiceInstance;
  }
  // Store in stateManager for legacy access
  if (stateManagerInstance?.setState) {
    stateManagerInstance.setState('api', apiServiceInstance as unknown);
  }
}
