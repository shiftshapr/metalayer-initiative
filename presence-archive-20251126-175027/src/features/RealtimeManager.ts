...
import type { Message, PresenceData, AurasIntegration, SupabaseRealtimeChannel, SupabaseClient } from '../types/index.js';
...
type WindowWithRealtimeClient = Window & {
  ...
  SupabaseRealtimeClient?: new () => SupabaseRealtimeClientBridge;
};
...
class RealtimeManager {
  ...
  private userPresenceChannel: SupabaseRealtimeChannel | null = null;
  private availabilityChannel: SupabaseRealtimeChannel | null = null;
  ...
  private initializePresence(pageId: string): void {
    ...
    // Initialize presence tracking using COMP method (TypeScript migration: use service)
    let supabase: SupabaseClient | null = null;
    try {
      supabase = supabaseServiceInstance.getClient();
    } catch (error: unknown) {
      this.log('WARN', '⚠️ PRESENCE: SupabaseService not initialized, attempting initialization...');
      // Service might not be initialized yet, but we'll handle gracefully
    }
    if (supabase) {
  ...
  private trackUserPresence(pageId: string): void {
    ...
    try {
      ...
      this.log('WARN', '⚠️ PRESENCE: SupabaseService not available for presence tracking');
    }
  ...
  private initializeUserPresenceSubscription(): void {
    ...
    try {
      ...
      this.log('WARN', '⚠️ PRESENCE: SupabaseService not available for user_presence subscription');
      return;
    }
    if (!supabase) {
      this.log('WARN', '⚠️ PRESENCE: Supabase client not available');
      return;
    }
  ...
  private initializeAvailabilitySubscription(): void {
    ...
    try {
      ...
      this.log('WARN', '⚠️ STATUS: SupabaseService not available for availability subscription');
      return;
    }
    if (!supabase) {
      this.log('WARN', '⚠️ STATUS: Supabase client not available');
      return;
    }
    ...
    try {
      ...
      availabilityChannel.subscribe((status: string) => {
          if (status === 'SUBSCRIBED') {
...
async function initializeSupabaseRealtimeClient(): Promise<void> {
  try {
    ...
    const winRealtime = window as WindowWithRealtimeClient;
    ...
    const waitForSupabase = async (): Promise<SupabaseClient | null> => {
      let attempts = 0;
    ...
    if (supabase && supabase.realtime) {
      ...
      type SupabaseRealtimeClientConstructor = new () => SupabaseRealtimeClientBridge;
      const winWithClient = window as WindowWithRealtimeClient;
      const SupabaseRealtimeClient = winWithClient.SupabaseRealtimeClient as SupabaseRealtimeClientConstructor | undefined;
      if (typeof SupabaseRealtimeClient !== 'undefined') {
        ...
        Object.assign(winWithClient, { supabaseRealtimeClient: supabase as unknown as SupabaseRealtimeClientBridge });
      }
...
async function sendSupabaseMessage(message: SupabaseMessage): Promise<boolean> {
  ...
  try {
    ...
    const supabaseRealtimeClient = (window as WindowWithRealtimeClient).supabaseRealtimeClient;
...
async function joinPageWithSupabase(pageId: string, pageUrl: string): Promise<void> {
  ...
  const client = (window as WindowWithRealtimeClient).supabaseRealtimeClient;
...
async function sendPresenceEventToAPI(kind: PresenceEventKind, availability: AvailabilityStatus | null = null, customLabel: string | null = null): Promise<PresenceEventResponse> {
  ...
  try {
    ...
    const apiBaseUrl = (window as Window & { API_BASE_URL?: string }).API_BASE_URL || API_CONFIG.baseUrl;
    ...
    void (getCurrentUserEmail ? (await getCurrentUserEmail()) || (window.currentUser as { email?: string })?.email || null : null);
