  ...
  try {
    ...
    if (communities.length > 0) {
      ...
      if (typeof window !== 'undefined') {
        (window as Window & { currentUrlData?: typeof initialUrlData }).currentUrlData = initialUrlData;
      }
...
export async function loadCombinedAvatars(communityIds: string[]): Promise<void> {
  ...
  try {
    ...
    const currentUrlData = stateManagerInstance.getState('currentUrlData') as { pageId?: string; rawUrl?: string; normalizedUrl?: string } | null;
