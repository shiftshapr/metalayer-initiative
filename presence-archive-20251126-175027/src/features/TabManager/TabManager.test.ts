...
import { TabManager, getTabManager } from './TabManager.js';
...
import { DEFAULT_TABS, DEFAULT_STATE } from './types.js';
...
export class TabManagerTestSuite {
  ...
  constructor(_logger: typeof Logger = Logger) {
    // Logger parameter kept for API compatibility but not used internally
  }
  ...
  async runAll(): Promise<void> {
    Logger.debug('🧪 Starting Tab Manager Test Suite...\n', null, 'general');
   ...
   */
  async testTabConfiguration(): Promise<void> {
    Logger.debug('📋 Testing TabConfiguration...', null, 'general');
    ...
    try {
      ...
      Logger.debug('✅ TabConfiguration tests passed\n', null, 'general');
    } catch (error) {
   ...
   */
  async testTabDisplay(): Promise<void> {
    Logger.debug('📺 Testing TabDisplay...', null, 'general');
    ...
    try {
      ...
      const manageButton: HTMLElement | null = container.querySelector('.manage-button');
      this.assert(manageButton !== null, 'Manage button should exist');

      // Test tab click
      const firstTab: HTMLElement | null = renderedTabs[0] as HTMLElement | null;
      if (!firstTab) {
        throw new Error('First tab element not found');
      }
      firstTab.click();
      ...
      Logger.debug('✅ TabDisplay tests passed\n', null, 'general');
    } catch (error) {
   ...
   */
  async testTabManagerModal(): Promise<void> {
    Logger.debug('🪟 Testing TabManagerModal...', null, 'general');
    ...
    try {
      ...
      // Test modal creation
      const modalElement: HTMLElement | null = document.getElementById('tab-manager-modal');
      this.assert(modalElement !== null, 'Modal should be created');
      if (!modalElement) {
      ...
      const tabList: HTMLElement | null = modalElement.querySelector('#tab-list');
      if (!tabList) {
        throw new Error('Tab list element not found');
      }
      const tabItems: NodeListOf<Element> = tabList.querySelectorAll('.tab-list-item');
      this.assert(tabItems.length === 3, 'Should render 3 tab items');

      modal.close();
      
      Logger.debug('✅ TabManagerModal tests passed\n', null, 'general');
    } catch (error) {
   ...
   */
  async testTabManager(): Promise<void> {
    Logger.debug('🎯 Testing TabManager...', null, 'general');
    ...
    try {
      ...
      // Test tab switching
      const config: TabConfiguration = manager.getConfiguration();
      await config.switchToTab('settings-tab');
      ...
      Logger.debug('✅ TabManager tests passed\n', null, 'general');
    } catch (error) {
   ...
   */
  async testCurrentPreviousTabTracking(): Promise<void> {
    Logger.debug('🔄 Testing currentTab/previousTab tracking...', null, 'general');
    ...
    try {
      ...
      Logger.debug('✅ currentTab/previousTab tracking tests passed\n', null, 'general');
    } catch (error) {
   ...
   * Record test result
   */
  private recordFailure(test: string, error: unknown): void {
    const errorMessage = error instanceof Error ? error.message : String(error);
    this.results.push({ test, passed: false, error: errorMessage });
    Logger.error(`❌ ${test} failed:`, errorMessage, 'general');
  }
  ...
  private printResults(): void {
    ...
    if (failed === 0) {
      Logger.debug(`✅ All tests passed! (${passed} tests)`, null, 'general');
    } else {
      Logger.debug(`❌ ${failed} test(s) failed, ${passed} passed`, null, 'general');
      this.results.filter(r => !r.passed).forEach(r => {
        Logger.debug(`  - ${r.test}: ${r.error}`, null, 'general');
      });
    }
  }
}
...
if (typeof window !== 'undefined') {
  ...
  (window as Window & { TabManagerTestSuite?: typeof TabManagerTestSuite }).TabManagerTestSuite = TabManagerTestSuite;
  
  // Auto-run tests if in test mode
  if (window.location.search.includes('test=true')) {
    const suite = new TabManagerTestSuite();
    suite.runAll().catch((error: unknown) => Logger.error('TabManager test suite error:', error, 'test'));
  }
}
