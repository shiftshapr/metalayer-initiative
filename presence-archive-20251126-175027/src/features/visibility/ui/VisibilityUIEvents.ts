...
export class VisibilityUIEvents {
  ...
  private visibilityTab: VisibilityTab | null = null;
  private modal: VisibilityModal | null = null;
  private isInitialized: boolean = false;
  private tabActivationObserver: MutationObserver | null = null;
   ...
   */
  private setupVisibilityTabClick(): void {
    const checkForTab = () => {
      const btn = document.querySelector('[data-tab="visibility-tab"]') as HTMLElement;
      if (!btn) {
   ...
   */
  private setupTabActivationWatcher(): void {
   ...
   */
  private async refreshVisibilityOnTabOpen(): Promise<void> {
    ...
    const win = typeof window !== 'undefined' ? window as Window & {
      __CANOPI_MODULE_GRAPH__?: {
        visibilityManager?: {
          refreshVisibilityAvatars: (pageId: string) => Promise<unknown[]>;
        };
      };
    } : null;
   ...
   */
  private navigateToVisibilityTab(): void {
    const win = typeof window !== 'undefined' ? window as Window & {
