...
export class VisibilityTab {
  ...
  private goInvisibleBtn: HTMLButtonElement | null = null;
   ...
   * Render users list
   */
  async render(users: VisibilityUser[]): Promise<void> {
    if (!this.container) {
    ...
    try {
      ...
      const currentUser = (typeof window !== 'undefined' && 
        (window as Window & { currentUser?: User }).currentUser) 
        ? (window as Window & { currentUser?: User }).currentUser 
        : null;
      ...
      const filteredUsers = users.filter(user => {
        if (currentUserId && user.id && String(user.id) === String(currentUserId)) return false;
        return true;
      });
   ...
   * Create header with count, search, and Go Invisible button
   */
  private createHeader(userCount: number): HTMLElement {
   ...
   * Create user list
   */
  private async createUserList(users: VisibilityUser[], currentPageId: string | null): Promise<HTMLElement> {
   ...
   */
  private async createUserItem(user: VisibilityUser, currentPageId: string | null): Promise<HTMLElement> {
    ...
    if (createUnifiedAvatar && typeof createUnifiedAvatar === 'function') {
      try {
        const avatarHTML = await createUnifiedAvatar(user as User, 'visibility', {
          size: 32,
          showAura: true,
          showStatus: true
        });
   ...
   */
  private createFallbackAvatar(container: HTMLElement, user: VisibilityUser): void {
   ...
   * Set up Go Invisible button
   */
  private setupGoInvisibleButton(): void {
    if (!this.goInvisibleBtn) return;
    ...
    this.goInvisibleBtn.addEventListener('click', async () => {
      ...
      const uiManager = (win as Window & { uiManager?: { switchTab?: (tabId: string) => void } }).uiManager;
