...
const getApi = (): MetaLayerAPI | null => {
...
const ensureApi = (context: string): MetaLayerAPI | null => {
  const api = getApi();
  if (!api) {
...
const getUserPreferencesManager = (): { isInitialized?: boolean; getPreference?: (key: string) => Promise<string | number | boolean | null>; savePreference?: (key: string, value: unknown, options?: { batch?: boolean }) => Promise<unknown> } | null => {
  const win = window as Window & { userPreferencesManager?: { isInitialized?: boolean; getPreference?: (key: string) => Promise<string | number | boolean | null>; savePreference?: (key: string, value: unknown, options?: { batch?: boolean }) => Promise<unknown> } };
...
class ProfileManager {
    ...
    if (visibilitySettingsBtn) {
      ...
      this.visibilitySettingsHandler = (e: Event) => {
        ...
        setTimeout(async () => {
          ...
          try {
            const module = await import('../../extension/features/VisibilitySettingsManager.js') as { visibilitySettingsManagerInstance?: { ensureEventListeners?: () => Promise<void> } };
  ...
  async handleUserUpdate(user: User) {
    ...
    if (win.syncStorageWithDatabase && typeof win.syncStorageWithDatabase === 'function') {
      win.syncStorageWithDatabase().catch((err: unknown) => {
              ...
              if (apiAuraColor !== '#ffffff' && apiAuraColor !== 'white' && apiAuraColor !== '#fff' && this.profileData) {
                ...
                if (currentUser) {
                  ...
                  if (typeof window !== 'undefined' && (window as { currentUser?: User }).currentUser) {
                    (window as { currentUser: User }).currentUser.auraColor = apiAuraColor;
                  }
                }
...
function getCurrentUserAvatarBgColor(): string {
  ...
  if (window.currentUser && window.currentUser.auraColor && window.currentUser.auraColor !== AVATAR_FALLBACK_COLOR) {
    Logger.debug(`✅ AURA_FIX: Found database aura color in currentUser: ${window.currentUser.auraColor}`, null, 'profile');
  ...
  if (visibilityDataFiltered2?.active && currentUser?.id) {
    const userData = visibilityDataFiltered2.active.find((u: User) => u.id === currentUser.id);
    if (userData && userData.auraColor && userData.auraColor !== AVATAR_FALLBACK_COLOR) {
...
async function getCurrentUserAvatarColor(): Promise<string> {
  if (window.currentUser && typeof window.currentUser.auraColor === 'string' && window.currentUser.auraColor.length) {
    return window.currentUser.auraColor;
  }
...
function setCustomAvatarColor(color: string) {
  if (window.currentUser) {
    ...
    if (window.ProfileManager && typeof window.ProfileManager === 'object' && 'updateProfile' in window.ProfileManager && typeof (window.ProfileManager as { updateProfile: unknown }).updateProfile === 'function') {
      (window.ProfileManager as { updateProfile: (user: User) => void }).updateProfile(window.currentUser);
    }
  }
}
...
function resetCustomAvatarColor() {
  if (window.currentUser) {
    window.currentUser.auraColor = AVATAR_FALLBACK_COLOR; // Default white
    // Update profile if ProfileManager instance exists
    if (window.ProfileManager && typeof window.ProfileManager === 'object' && 'updateProfile' in window.ProfileManager && typeof (window.ProfileManager as { updateProfile: unknown }).updateProfile === 'function') {
      (window.ProfileManager as { updateProfile: (user: User) => void }).updateProfile(window.currentUser);
    }
  ...
  if (auraBtn) {
    auraBtn.addEventListener('click', (e) => {
      ...
      if (typeof window.showColorPickerModal === 'function') {
        ...
        Logger.debug('❌ showColorPickerModal not available', null, 'profile');
      }
    });
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', (e) => {
      ...
      if (typeof window.logout === 'function') {
        ...
        Logger.debug('❌ logout function not available, creating COMP method logout', 'profile');
        // COMP METHOD: Create logout function if not available
        window.performLogout = async function() {
          Logger.debug('🔧 LOGOUT: Performing logout', null, 'profile');
          try {
            // Clear user data
            window.currentUser = undefined;
            window.supabaseUser = undefined;
      ...
      window.location.reload();
            ...
            Logger.debug('✅ LOGOUT: Logout completed', null, 'profile');
          } catch (error) {
            Logger.error('❌ LOGOUT: Error during logout', error, 'profile');
          }
        };
...
function addAllProfileMenuHandlers() {
  ...
  const profileManager = window.profileManager || (window.ProfileManager && typeof window.ProfileManager === 'object' && 'instance' in window.ProfileManager ? (window.ProfileManager as { instance?: ProfileManager }).instance : undefined);
  if (profileManager) {
    ...
    Logger.warn('⚠️ PROFILE_MENU: ProfileManager instance not found', null, 'profile');
  }
...
async function updateAvailabilityEverywhere(availability: string) {
  ...
  try {
    ...
    const profileManagerInstance = (window.profileManager || (window.ProfileManager && (window.ProfileManager as { instance?: ProfileManager }).instance)) as ProfileManager | undefined;
...
async function updateThemeEverywhere(theme: string) {
  ...
  try {
    ...
    if (window.currentUser) {
      ...
      Logger.debug('✅ THEME_UPDATE: Updated window.currentUser', null, 'profile');
    }
    ...
    if (themeToggle) {
      ...
      if (window.visibilitySettingsManager && typeof window.visibilitySettingsManager === 'object' && 'updateThemeStatus' in window.visibilitySettingsManager && typeof (window.visibilitySettingsManager as { updateThemeStatus: unknown }).updateThemeStatus === 'function') {
        (window.visibilitySettingsManager as { updateThemeStatus: () => void }).updateThemeStatus();
      }
