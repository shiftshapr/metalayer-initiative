...
export class AnchorHighlighter {
  ...
  removeHighlight(element: HTMLElement): void {
    try {
      ...
      if (state) {
        ...
        if (state.animationId && typeof state.animationId === 'number') {
          cancelAnimationFrame(state.animationId);
        }

        // Remove classes
        const highlightClass = this.getHighlightClass(state.style as HighlightStyle);
  ...
  private getHighlightClass(style: HighlightStyle): string {
    const styleStr = typeof style === 'string' ? style : 'pulse';
  ...
  updateDuration(element: HTMLElement, newDuration: number): void {
    ...
    if (state) {
      ...
      if (newDuration > 0) {
        state.timeoutId = window.setTimeout(() => {
          this.removeHighlight(element);
        }, newDuration) as unknown as ReturnType<typeof setTimeout>;
      }
    }
  }
