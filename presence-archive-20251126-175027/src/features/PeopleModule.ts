...
let peopleTabInitPromise: Promise<void> | null = null;

function normalizePeopleUser(row: SupabasePeopleListRow): PeopleDisplayUser {
...
async function fetchPeopleFromTables(client: SupabaseClient): Promise<{
  ...
  for (const tableName of TABLE_NAME_VARIATIONS) {
    ...
    try {
      const { data, error } = await client
        .from<SupabasePeopleListRow>(tableName)
      ...
      const formattedError: SupabaseQueryError = {
...
async function fetchActivePresenceIds(
  client: SupabaseClient,
  userIds: string[],
  sinceIso: string,
  _displayUsers: PeopleDisplayUser[], // Unused but kept for API compatibility
): Promise<Set<string>> {
  ...
  try {
    ...
    data?.forEach((record: { user_id?: string; is_active?: boolean; last_seen?: string }) => {
  ...
  } catch (presenceQueryError: unknown) {
...
async function renderPeopleTab(): Promise<boolean> {
  ...
  try {
    ...
    const currentUser = stateManagerInstance.getState('currentUser') as { id?: string; email?: string } | null;
...
async function ensurePeopleTabInitialized(): Promise<void> {
 ...
 */
async function initializePeopleTab(): Promise<void> {
...
function initializePeopleModule(): void {
