...
import { initializeMessageSystemIntegration, type MessageSystemIntegrationType } from './MessageSystemIntegration.js';
...
const getWindowFunction = (name: string): unknown => {
    if (typeof window === 'undefined')
        return undefined;
    return (window as Window & Record<string, unknown>)[name];
};
...
type MessagesModulePolicy = Omit<ModuleErrorPolicy, 'component'>;

const handleMessagesModuleError = <T = never>(
    error: unknown,
    policy: MessagesModulePolicy
...
const setCurrentChatData = (messages: Message[]) => {
...
async function initializeNewMessageSystem() {
    try {
        ...
        messageSystemIntegration = await initializeMessageSystemIntegration({
            ...
            onMessageUpdate: (messages) => {
                ...
                if (newMessages.length > 0) {
                    ...
                    newMessages.forEach(msg => {
                        const addMessageFn = getWindowFunction('addMessageToChat') as ((msg: Message | Partial<Message>) => void) | undefined;
            ...
            showNotification: (getWindowFunction('showNotification') as ((msg: string) => void) | undefined) || ((msg: string) => Logger.debug('📢', { msg }, 'messages'))
        });
...
const getMessageTimestampValue = (value: Date | string | number | unknown): number => {
...
const resolveAuthorFromPayload = (rawMessage: RawMessagePayload, authorId: string | undefined): User => {
    const authorSource = (rawMessage.author ??
        rawMessage.AppUser ??
        rawMessage.user ??
        getCurrentUser()) as User | Record<string, unknown> | null | undefined;
    const authorObj = authorSource && typeof authorSource === 'object' ? authorSource as User : null;
    return {
        id: (authorObj?.id as string | undefined) || authorId || 'unknown-user',
        name: formatAuthorName(authorObj),
        handle: formatUserHandle(authorObj),
        email: authorObj?.email as string | undefined,
        avatarUrl: authorObj?.avatarUrl as string | undefined,
        auraColor: (authorObj?.auraColor as string | undefined) || AVATAR_FALLBACK_COLOR
    };
};
const normalizeMessagePayload = (rawMessage: RawMessagePayload): Message => {
    ...
    const getStringOrDate = (value: unknown): Date | string => {
        if (value instanceof Date)
            return value;
        if (typeof value === 'string')
            return value;
    ...
    try {
        ...
        if (UnifiedMessageRenderer && typeof UnifiedMessageRenderer.generateMessageHTML === 'function') {
            ...
            if (message.communityId) {
                ...
                const communitiesModule = getWindowFunction('CommunitiesModule') as Window['CommunitiesModule'] | undefined;
        ...
        return messageDiv;
            ...
            const createUnifiedMessageElementFn = getWindowFunction('createUnifiedMessageElement') as ((message: Message) => HTMLElement | Promise<HTMLElement>) | undefined;
 ...
 */
function updateMessageInChat(updatedMessage: Message | Partial<Message>): void {
    ...
    if (!messageElement) {
        ...
        const addMessageToChatFn = getWindowFunction('addMessageToChat') as ((message: Message | Partial<Message>) => void | Promise<void>) | undefined;
 ...
 */
function removeMessageFromChat(deletedMessage: Message | { id: string }): void {
...
async function addMessageToChat(rawMessage: RawMessagePayload | Message): Promise<void> {
    ...
    try {
        ...
        if (messageElement) {
            const addMessageActionListenersFn = getWindowFunction('addMessageActionListeners') as ((messageDiv: HTMLElement, message: Message) => void) | undefined;
        ...
        const loadMessageReactionsFn = getWindowFunction('loadMessageReactions') as ((messageId: string) => Promise<void>) | undefined;
...
function getSenderName(userId: string | undefined): string {
    ...
    if (visibilityData && Array.isArray(visibilityData)) {
        const user = visibilityData.find((u: User | Record<string, unknown>) => (u as User).id === userId);
        if (user) {
 ...
 * COMP METHOD: Matches original implementation
 */
function formatMessageTime(createdAt: Date | string | null | undefined): string {
    if (!createdAt)
        return 'Unknown';
...
async function createUnifiedMessageElement(message: Message): Promise<HTMLElement> {
    try {
        ...
        if (UnifiedMessageRenderer && typeof UnifiedMessageRenderer.renderMessage === 'function') {
            ...
            let communityName = '';
            if (message.communityId) {
                const communitiesModule = getWindowFunction('CommunitiesModule') as { getCommunityName?: (id: string) => string | Promise<string> } | undefined;
                if (communitiesModule && typeof communitiesModule.getCommunityName === 'function') {
                    const name = communitiesModule.getCommunityName(message.communityId);
                    communityName = typeof name === 'string' ? name : (name instanceof Promise ? await name : '');
                }
 ...
 */
function updateReactionDisplay(messageId: string, reactions: Array<Record<string, unknown>> | unknown): void {
...
async function loadMessageReactions(messageId: string, reactionBtn: HTMLElement | null): Promise<void> {
    ...
    try {
        ...
        const apiWithReactions = api as typeof api & { getReactions?: (messageId: string) => Promise<unknown> };
        if (apiWithReactions && typeof apiWithReactions.getReactions === 'function') {
            ...
            const { data: supabaseReactions, error } = await windowSupabase
                .from<SupabaseReactionRow>('reactions')
                .select('*')
 ...
 */
async function loadChatHistory(pageIdOrRawUrl?: string | null, activeCommunities?: string[]): Promise<void> {
    ...
    try {
        ...
        const existingStateMessages = stateManagerInstance ? (stateManagerInstance.getState('chat.data') as Message[] | null) : null;
        ...
        Logger.debug(`📜 loadChatHistory: loadDefaultView returned ${messages.length} messages, sorted to ${sortedMessages.length}`, { pageId, communityId, messageIds: sortedMessages.map((m: Message) => m.id) }, 'messages');
        ...
        const newMessages = messages.filter((msg: Message) => !stateMessageIds.has(msg.id) && !domMessageIds.has(msg.id));
...
type WindowWithLoadHistory = Window & {
    loadChatHistory?: typeof loadChatHistory;
};

let loadHistoryBindingApplied = false;

const ensureLoadChatHistoryBinding = (): void => {
    if (typeof window === 'undefined') {
        return;
    }
    const win = window as WindowWithLoadHistory;
...
async function handleDeleteMessage(message: Message): Promise<void> {
    ...
    try {
        ...
        const robustIntegration = getWindowFunction('robustIntegration') as { isInitialized?: boolean; deleteMessage?: (messageId: string) => Promise<void> } | undefined;
...
type NormalizedSendMessagePayload = {
    content: string;
    parentId: string | null;
    conversationId: string | null;
};

function normalizeSendMessageInput(input: SendMessageViaSupabaseInput, fallbackParentId?: string | null, fallbackConversationId?: string | null): NormalizedSendMessagePayload | null {
 ...
 * COMP METHOD: Matches original implementation
 */
async function sendMessageViaSupabase(input: SendMessageViaSupabaseInput, parentId: string | null = null, conversationId: string | null = null): Promise<Message | null> {
 ...
 * COMP METHOD: Matches original implementation
 */
async function getSenderAvatar(author: User | Record<string, unknown> | null | undefined): Promise<string> {
    try {
 ...
 * COMP METHOD: Matches original implementation
 */
function parseMessageUrl(url: string): { pageId?: string; messageId?: string; parentId?: string; isValid: boolean } | null {
...
async function handleReaction(message: Message): Promise<void> {
    try {
        ...
        try {
            ...
            const responseData = response.data as { reactions?: Array<Record<string, unknown>> } | null | undefined;
    ...
    const attachClickHandler = async () => {
        ...
        if (!openModal) {
            ...
            try {
                ...
                await waitForCondition(
                    () => {
                        const modal = getWindowFunction('openMessageModal') as ((options: { mode: string; pageId: string; communityId?: string }) => Promise<void>) | null;
                        const unifiedModal = getWindowFunction('unifiedMessageModal') as { open?: (options: { mode: string; pageId: string; communityId?: string }) => Promise<void> } | null;
        ...
        chatTextarea.addEventListener('click', async (e) => {
            ...
            try {
                ...
                const currentUrlData = stateManager && typeof stateManager.getState === 'function' ? stateManager.getState('currentUrlData') as { pageId?: string } | null : null;
                const activeCommunities = stateManager && typeof stateManager.getState === 'function' ? stateManager.getState('ui.activeCommunities') as string[] | null : null;
...
async function sendChatMessage(): Promise<void> {
    ...
    try {
        if (editingMessageId) {
            ...
            const sendMessageViaSupabaseFn = getWindowFunction('sendMessageViaSupabase') as ((input: SendMessageViaSupabaseInput, parentId?: string | null, conversationId?: string | null) => Promise<Message | null>) | null;
...
export function initializeMessagesModule(): void {
