...
async function authenticateWithSupabase(user: User): Promise<void> {
  try {
    ...
    // UUID ONLY - check session by UUID, not email
    // Now properly typed - no need for as unknown
    const sessionResult = await supabase.auth.getSession();
    const session = sessionResult?.data?.session;
    // UUID ONLY - compare by user.id (UUID), not email
    if (session && session.user && user.id && session.user.id === user.id) {
    ...
    // UUID-ONLY: Use user.id (UUID) instead of email for authentication
    const authenticateUserForRealtime = (window as Window & { authenticateUserForRealtime?: (userId: string, userName?: string) => Promise<boolean> }).authenticateUserForRealtime;
    if (!authenticateUserForRealtime) {
      Logger.error('❌ SUPABASE AUTH: authenticateUserForRealtime not available', null, 'auth');
      return;
    }
    
    // UUID-ONLY: Pass UUID instead of email
    if (!user.id) {
      Logger.error('❌ SUPABASE AUTH: User ID (UUID) is required for authentication', null, 'auth');
      return;
    }
    const authSuccess = await authenticateUserForRealtime(user.id, user.name || user.email?.split('@')[0] || 'user');
    ...
    // UUID-ONLY: Use UUID as primaryIdentifier (matching RealtimeManager pattern)
    const supabaseRealtimeClient = (window as Window & { supabaseRealtimeClient?: SupabaseRealtimeClientBridge }).supabaseRealtimeClient;
    if (supabaseRealtimeClient && supabaseRealtimeClient.setCurrentUser) {
      await supabaseRealtimeClient.setCurrentUser(user.id, null, 'abe5ec85-4ba6-456f-adaf-03d7d51cecf4');
    ...
    if (currentUser && user.email) {
      try {
        // Now properly typed - window.api is in global.d.ts
        ...
        // Extract AppUser from response - handle both { data: AppUser } and direct AppUser
        const appUserData = (appUserResponse && typeof appUserResponse === 'object' && 'data' in appUserResponse 
          ? (appUserResponse as { data?: unknown }).data 
          : appUserResponse) as AppUser | null | undefined;
        
        if (!appUserData || typeof appUserData !== 'object') {
          Logger.warn('⚠️ AUTH: Invalid AppUser response', null, 'auth');
          return;
        }
        
        const appUser = appUserData as AppUser;
...
async function getCurrentUserId(): Promise<string | null> {
  try {
    // ROOT CAUSE FIX: Use stateManager (TypeScript migration - no window.currentUser)
    // UUID ONLY - no email fallback
    const user = stateManagerInstance.getState('currentUser') as UserType | null;
    if (user && user.id) {
      // Validate UUID format
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (uuidRegex.test(user.id)) {
        return user.id; // Valid UUID
      } else {
        Logger.error('❌ AUTH: user.id is not a valid UUID', { userId: user.id }, 'auth');
        return null; // Invalid ID - don't return email
      }
    }
    return null; // No user or no ID - don't return email
  } catch (error: unknown) {
    ...
    return null; // UUID ONLY - no email fallback
  }
}
