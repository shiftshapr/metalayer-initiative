...
import { Web3AuthAdapter, type Web3AuthConfig } from './adapters/Web3AuthAdapter.js';
...
export class AuthProvider {
   ...
   */
  async initialize(configs: AuthAdapterConfig[]): Promise<void> {
    if (this.initialized) {
        ...
        switch (config.provider) {
          case 'web3auth': {
            if (this.isWeb3AuthAvailable()) {
              adapter = new Web3AuthAdapter(config.config as unknown as Web3AuthConfig);
   ...
   */
  async signIn(options?: Record<string, unknown>): Promise<AuthResult> {
    if (!this.currentAdapter) {
      return {
        success: false,
   ...
   */
  private isWeb3AuthAvailable(): boolean {
    return (
