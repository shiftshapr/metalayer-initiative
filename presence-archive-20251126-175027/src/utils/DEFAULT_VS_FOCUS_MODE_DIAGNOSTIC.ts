...
import { normalizeChatData } from '../types/window-utils.js';
...
type DiagnosticChatData = Array<Message | ChatMessage> | Record<string, Message[]>;

type WindowWithDiagnostics = Window & {
  currentChatData?: DiagnosticChatData;
  ReplyLoader?: {
    loadAllReplies: () => void;
  };
}
...
function captureDefaultModeState(): DefaultModeState {
  ...
  if (win.currentChatData) {
    const normalizedData = normalizeChatData<Message | ChatMessage>(win.currentChatData);
    state.currentChatData = normalizedData.map((m: Message | ChatMessage) => ({
      id: m.id,
...
function captureFocusModeState(): FocusModeState {
  ...
  // Check currentChatData
  if (win.currentChatData) {
    const focusChatData = normalizeChatData<Message | ChatMessage>(win.currentChatData);

    state.currentChatData = focusChatData.map((m: Message | ChatMessage) => {
...
function compareStates(defaultState: DefaultModeState, focusState: FocusModeState): Comparison[] {
  ...
  if (defaultState.replyCounts && Object.keys(defaultState.replyCounts).length > 0) {
    ...
    defaultReplyMessages.forEach(msgId => {
      ...
      if (!defaultInfo) {
        return; // Skip if defaultInfo is undefined
      }
