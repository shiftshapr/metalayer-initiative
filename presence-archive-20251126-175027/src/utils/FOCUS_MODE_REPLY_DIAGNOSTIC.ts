...
export async function diagnoseFocusModeReplies(messageId: string): Promise<DiagnosticResults> {
  ...
  try {
    ...
    const supabaseQuery = (window.supabase as unknown) as { from: (table: string) => { select: (columns: string) => { eq: (col: string, val: unknown) => { single: () => Promise<{ data: Record<string, unknown> | null; error: { message: string } | null }> } } } };
    const queryResult = await supabaseQuery.from('messages').select('*').eq('id', messageId).single();
    const { data: dbMessage, error: dbError } = queryResult;
    ...
    const msg = Array.isArray(msgRaw) ? (msgRaw[0] ?? null) : (typeof msgRaw === 'object' && msgRaw !== null ? msgRaw : null);
    const messageRaw = messageInChat || (dbMessage as MessageData) || {};
    const message = Array.isArray(messageRaw) ? (messageRaw[0] ?? null) : (typeof messageRaw === 'object' && messageRaw !== null ? messageRaw : null);
    
    resolutionSteps.step1MsgCommunityId = (msg ? ((msg as Message & MessageData).communityId || (msg as Record<string, unknown>).community_id as string) : null) || null;
    resolutionSteps.step2MessageCommunityId = (message ? ((message as Message & MessageData).communityId || (message as Record<string, unknown>).community_id as string) : null) || null;
    resolutionSteps.step3ActiveCommunities = (window.activeCommunities && window.activeCommunities[0]) || null;
    
    resolutionSteps.final = (msg ? (msg.communityId || (msg as Record<string, unknown>).community_id as string) : null) ||
                            (message ? (message.communityId || (message as Record<string, unknown>).community_id as string) : null) ||
    ...
    if (dbMessage) {
      ...
      const supabaseQuery = (window.supabase as unknown) as { from: (table: string) => { select: (columns: string) => { eq: (col: string, val: unknown) => { eq: (col: string, val: unknown) => { eq: (col: string, val: unknown) => { is: (col: string, val: unknown) => { order: (col: string, opts: { ascending: boolean }) => Promise<{ data: unknown[] | null; error: { message: string } | null }> } } } } } } };
      const queryResult = await supabaseQuery.from('messages').select('*').eq('parent_id', messageId).eq('page_id', dbMsg.page_id || dbMsg.pageId).eq('community_id', dbMsg.community_id || dbMsg.communityId).is('deleted_at', null).order('created_at', { ascending: true });
      const { data: replies, error: repliesError } = queryResult;
    ...
    const focusModeState: FocusModeState = {
      ...
      currentPageId: (typeof window.currentUrlData === 'object' && window.currentUrlData !== null && 'pageId' in window.currentUrlData ? (window.currentUrlData as { pageId?: string }).pageId : undefined) || (typeof (window as Window & { currentPage?: { pageId?: string } }).currentPage === 'object' && (window as Window & { currentPage?: { pageId?: string } }).currentPage ? (window as Window & { currentPage?: { pageId?: string } }).currentPage?.pageId : undefined)
    };
