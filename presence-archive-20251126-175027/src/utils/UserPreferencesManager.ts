...
declare const window: Window & {
  ...
  api?: {
    request: <T = unknown>(url: string, options: { method: string; body?: string }) => Promise<{ data?: T; error?: string; status?: number }>;
  };
  eventBus?: {
  ...
  getPreference?: (key: PreferenceKey) => Promise<PreferenceValue | null>;
...
export class UserPreferencesManager {
  ...
  constructor() {
    ...
    const schema: PreferenceSchema = {
      theme: {
        ...
        dbColumn: 'theme',
        defaultValue: 'light',
        validator: (value: unknown): value is Theme => typeof value === 'string' && ['light', 'dark', 'auto'].includes(value),
      ...
      auraColor: {
        ...
        dbColumn: 'aura_color',
        defaultValue: '#98d416',
        validator: (value: unknown): value is string => typeof value === 'string' && /^#[0-9A-Fa-f]{6}$/i.test(value),
      ...
      auraIntensity: {
        ...
        defaultValue: 0.5,
        validator: (value: unknown): value is number => typeof value === 'number' && value >= 0 && value <= 1,
      ...
      globalAvailability: {
        ...
        validator: (value: unknown): value is Availability => typeof value === 'string' && ['AVAILABLE', 'BUSY', 'AWAY', 'OFFLINE'].includes(value),
      ...
      headline: {
        ...
        dbColumn: 'headline',
        defaultValue: '',
        validator: (value: unknown): value is string => value === '' || (typeof value === 'string' && value.length >= 20 && value.length <= 1000),
      ...
      displayName: {
        ...
        defaultValue: '',
        validator: (value: unknown): value is string => value === '' || (typeof value === 'string' && value.length >= 4 && value.length <= 16),
  ...
  async initialize(userId: string) {
    ...
    if (!userId) {
      ...
      const currentUserFromState = stateManagerInstance.getState('currentUser') as { id?: string } | null;
   ...
   */
  async savePreference<K extends PreferenceKey>(key: K, value: Preferences[K], options: SaveOptions = {}): Promise<boolean> {
    if (!this.schema[key]) {
   ...
   */
  async savePreferences(preferences: Partial<Preferences>, options: SaveOptions = {}): Promise<Partial<Record<PreferenceKey, boolean>>> {
  ...
  async saveBatchToDatabase(preferences: Partial<Preferences>): Promise<void> {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(preferences)) {
        this.queueForRetry(key as PreferenceKey, value);
        this.emitPreferenceSaveFailed(key as PreferenceKey, value, error instanceof Error ? error : new Error(String(error)));
      }
   ...
   */
  updateCurrentUser() {
    // TypeScript migration: Update stateManager first
    ...
    if (typeof window !== 'undefined' && (window as { currentUser?: unknown }).currentUser) {
      const win = window as { currentUser: typeof currentUser };
      if (win.currentUser) {
        win.currentUser.theme = this.preferences.theme;
        win.currentUser.auraColor = this.preferences.auraColor;
        win.currentUser.auraIntensity = this.preferences.auraIntensity;
        win.currentUser.isVisible = this.preferences.isVisible;
        win.currentUser.visibilityEnabled = this.preferences.isVisible;
        win.currentUser.globalAvailability = this.preferences.globalAvailability;
        win.currentUser.availability = this.preferences.globalAvailability;
        win.currentUser.headline = this.preferences.headline;
        win.currentUser.displayName = this.preferences.displayName;
      }
    }
  }
  ...
  applyPreferencesToUI(skipTheme: boolean = false) {
    ...
    if (this.preferences.isVisible !== undefined) {
      const visibilityToggle = document.getElementById('visibility-toggle') as HTMLInputElement | null;
      if (visibilityToggle) {
    ...
    if (this.preferences.headline !== undefined) {
      const headlineInput = document.getElementById('settings-headline-input') as HTMLInputElement | null;
      if (headlineInput && headlineInput.value !== this.preferences.headline) {
        headlineInput.value = this.preferences.headline;
        const headlineManager = window.settingsHeadlineManager as { updateCharCount?: () => void } | undefined;
        headlineManager?.updateCharCount?.();
      }
    }
    ...
    if (this.preferences.displayName !== undefined) {
      ...
      if (displayNameInput && displayNameInput.value !== this.preferences.displayName) {
        ...
        const displayManager = window.displayNameManager as { updateCharCount?: () => void } | undefined;
        displayManager?.updateCharCount?.();
      }
    }
   ...
   */
  emitPreferenceChanged(key: PreferenceKey, value: PreferenceValue, oldValue: PreferenceValue | undefined, source: string = 'user') {
    const eventData = {
      key,
      value,
      oldValue,
...
if (typeof window !== 'undefined') {
  // Assign to window using proper typing (no window type assertion needed - red-line compliance)
  ...
  // Type assertion needed due to intersection type requirement from other type definitions
  (window as unknown as { getPreference: (key: PreferenceKey) => Promise<PreferenceValue | null> }).getPreference = (key: PreferenceKey) => userPreferencesManager.getPreference(key);
  (window as unknown as { savePreference: (key: PreferenceKey, value: PreferenceValue, options?: SaveOptions) => Promise<boolean> }).savePreference = (key: PreferenceKey, value: PreferenceValue, options?: SaveOptions) => userPreferencesManager.savePreference(key, value, options);
}
