...
class ProvenanceLinkInjector {
  ...
  private injectedMessages: Set<string> = new Set();
  private baseUrl: string = 'https://app.canopi.live';

  /**
   * Initialize the link injector
   */
  async initialize(): Promise<void> {
    const service = (window as Window & { provenanceService?: { getProvenanceUrl?: (messageId: string, baseUrl: string) => string } }).provenanceService;
    if (!service) {
   ...
   */
  private injectLinksForExistingMessages(): void {
    const messages = document.querySelectorAll('[data-message-id], .message, .chat-message');
    
    messages.forEach(messageElement => {
      const messageId = this.getMessageId(messageElement as HTMLElement);
      if (messageId && !this.injectedMessages.has(messageId)) {
        this.injectLink(messageElement as HTMLElement, messageId);
      }
    });
  }
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const messageId = this.getMessageId(node as HTMLElement);
            if (messageId && !this.injectedMessages.has(messageId)) {
              this.injectLink(node as HTMLElement, messageId);
            }

            // Also check children (in case message container is added)
            const messageElements = (node as HTMLElement).querySelectorAll?.('[data-message-id], .message, .chat-message');
            if (messageElements) {
              messageElements.forEach((msgEl) => {
                const id = this.getMessageId(msgEl as HTMLElement);
                if (id && !this.injectedMessages.has(id)) {
                  this.injectLink(msgEl as HTMLElement, id);
                }
              });
            }
          }
   ...
   */
  private getMessageId(element: HTMLElement): string | null {
    ...
    const idElement = element.querySelector?.('[data-message-id]') as HTMLElement;
    if (idElement?.dataset?.messageId) {
      return idElement.dataset.messageId;
    }

    // Try to extract from message object if stored
    const elementWithMessage = element as HTMLElement & { message?: { id?: string } };
   ...
   */
  private injectLink(messageElement: HTMLElement, messageId: string): void {
   ...
   */
  injectForMessage(messageId: string, messageElement: HTMLElement | null = null): void {
    if (!messageElement) {
      // Find message element by ID
      messageElement = document.querySelector(`[data-message-id="${messageId}"]`) as HTMLElement;
      if (!messageElement) {
