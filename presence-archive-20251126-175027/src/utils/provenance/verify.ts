...
class ProvenanceVerifier {
  ...
  async verifyArtifact(
    ...
    try {
      ...
      // Determine algorithm
      const actorAlgorithm = (artifact.actor && typeof artifact.actor === 'object' && 'algorithm' in artifact.actor ? (artifact.actor as { algorithm?: string }).algorithm : undefined);
      const publicKeyAlgorithm = (publicKey && typeof publicKey === 'object' && 'algorithm' in publicKey && publicKey.algorithm && typeof publicKey.algorithm === 'object' && 'name' in publicKey.algorithm ? (publicKey.algorithm as { name?: string }).name : undefined);
  ...
  ): Promise<VerificationResult & { keyId?: string }> {
   ...
   */
  async verifyArtifactChain(
    artifacts: ProvenanceArtifact[]
  ): Promise<Array<VerificationResult & { artifactId: string; timestamp: string; action?: string }>> {
    const results: Array<VerificationResult & { artifactId: string; timestamp: string; action?: string }> = [];
    ...
    for (const artifact of artifacts) {
      ...
      results.push({
        ...
        action: (artifact.claim && typeof artifact.claim === 'object' && 'action' in artifact.claim ? (artifact.claim as { action?: string }).action : undefined),
        ...verification
      });
    }
