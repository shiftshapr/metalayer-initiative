...
class ProvenanceDiagnostic {
  ...
  async initialize(): Promise<void> {
    const windowWithService = (window as unknown) as Window & { provenanceService?: import('./ProvenanceService').default };
   ...
   */
  async displayMessageProvenance(messageId: string): Promise<void> {
    const windowWithService = (window as unknown) as Window & { provenanceService?: import('./ProvenanceService').default };
    ...
    try {
      const artifacts: ProvenanceArtifact[] = await service.getMessageProvenance(messageId);
      ...
      // Verify artifacts
      let verificationResults: Array<VerificationResult & { artifactId: string; timestamp: string; action?: string }> = [];
      const windowWithVerifier = (window as unknown) as Window & { provenanceVerifier?: { verifyArtifactChain: (artifacts: ProvenanceArtifact[]) => Promise<Array<VerificationResult & { artifactId: string; timestamp: string; action?: string }>> } };
          ...
          ">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
              <div style="font-weight: bold; color: #fff;">
                ${(artifact.claim && typeof artifact.claim === 'object' && 'action' in artifact.claim ? (artifact.claim as { action?: string }).action : undefined) || 'unknown'} #${index + 1}
              </div>
   ...
   */
  private exportArtifacts(artifacts: ProvenanceArtifact[], messageId: string): void {
    const data = {
