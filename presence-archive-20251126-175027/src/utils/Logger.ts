...
class Logger {
  ...
  private static readonly isProduction: boolean = 
    ...
    typeof window !== 'undefined' && (window as { __PRODUCTION__?: boolean }).__PRODUCTION__ === true;
  ...
  static log(level: string, message: string, data: unknown = null, context = 'general'): void {
  ...
  log(level: string, message: string, data: unknown = null, context = 'general'): void {
    Logger.log(level, message, data, context);
  }

  // Static convenience methods
  static error(message: string, data: unknown = null, context = 'general'): void {
    this.log('error', message, data, context);
  }

  static warn(message: string, data: unknown = null, context = 'general'): void {
    this.log('warn', message, data, context);
  }

  static info(message: string, data: unknown = null, context = 'general'): void {
    this.log('info', message, data, context);
  }

  static debug(message: string, data: unknown = null, context = 'general'): void {
    this.log('debug', message, data, context);
  }

  static success(message: string, data: unknown = null, context = 'general'): void {
    this.log('success', message, data, context);
  }

  static getHistory(level: string | null = null, context: string | null = null, limit = 100): LogEntry[] {
  ...
  static configureDebugContexts(contexts: string[] | null | undefined): void {
    ...
    this.debugContextAllowlist = new Set(
      contexts
  ...
  private static loadDebugContextAllowlistFromWindow(): void {
    ...
    const win = window as { __DEBUG_CONTEXTS__?: string[] };
    if (Array.isArray(win.__DEBUG_CONTEXTS__)) {
      this.configureDebugContexts(win.__DEBUG_CONTEXTS__);
    }
  }

  static isDebugEnabled(context = 'general'): boolean {
