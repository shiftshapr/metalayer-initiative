...
class AvatarUtils {
   ...
   */
  static async getAvatarUrl(user: User, context = 'visibility'): Promise<AvatarData> {
    ...
    avatarUrl = user.avatarUrl ?? (user as { picture?: string }).picture ?? null;
    if (avatarUrl) {
      avatarSource = 'user-object';
    } else {
      // Try to get from window.currentUser if available
      const win = window as Window & { currentUser?: User };
      if (typeof window !== 'undefined' && win.currentUser) {
        const currentUser = win.currentUser;
        if (currentUser && currentUser.id === user.id) {
          avatarUrl = currentUser.avatarUrl ?? (currentUser as { picture?: string }).picture ?? null;
          if (avatarUrl) {
   ...
   */
  private static async getAuraIntensity(user: User): Promise<number> {
    // Try to get from user object
    let intensity = (user as { auraIntensity?: number }).auraIntensity;
    ...
    if (intensity === undefined && typeof window !== 'undefined') {
      ...
      if (win.currentUser && win.currentUser.id === user.id) {
        intensity = (win.currentUser as { auraIntensity?: number }).auraIntensity;
      }
    }
    ...
    if (intensity === undefined && typeof window !== 'undefined') {
      const win = window as Window & { 
        userPreferencesManager?: { 
          ...
          getPreference: (key: string) => Promise<string | number | null>;
        } 
      };
   ...
   */
  static async createUnifiedAvatar(user: User, context = 'visibility', options: AvatarOptions = {}): Promise<string> {
