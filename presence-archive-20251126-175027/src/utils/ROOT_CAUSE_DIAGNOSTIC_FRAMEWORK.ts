...
import type {
  ...
  SidepanelSettingsInjectionResult,
  ProfileLoggingFirehoseDiagnosticResult,
  UserHoverGlobalDiagnosticResult,
  RealtimePresenceTypingResult,
  ActiveCommunityStateDiagnosticResult,
  LoadChatHistoryConnectivityResult,
  LoadHistoryRegistrationResult,
  BackendConnectivityDiagnosticResult
} from './diagnostic-result-types';
import { getErrorBoundaryRecords, type ErrorBoundaryResult } from './ErrorHandlingPolicy.js';
...
type ErrorBoundaryRecordLite = ErrorBoundaryResult<unknown>;
type ErrorBoundaryRegistryLite = {
  getRecords?: () => ErrorBoundaryRecordLite[];
  records?: ErrorBoundaryRecordLite[];
};
...
type WindowWithDiagnostics = Window & {
  ...
  __canopiErrorBoundaries?: ErrorBoundaryRegistryLite;
}
...
diagnosticFramework.register('headline-displayname-persistence', async (): Promise<HeadlineDisplayNameDiagnosticResult> => {
  ...
  if (results.checks) {
    const currentUser = window.currentUser as { headline?: string; displayName?: string } | undefined;
  ...
  const headlineSources = results.checks ? [
    ...
    (results.checks.currentUser as { headline?: string | null } | undefined)?.headline,
  ...
  const displayNameSources = results.checks ? [
    ...
    (results.checks.currentUser as { displayName?: string | null } | undefined)?.displayName,
...
diagnosticFramework.register('css-computed-values', async (): Promise<CssComputedValuesDiagnosticResult> => {
  const results: CssComputedValuesDiagnosticResult & { elements: Array<{ selector: string; exists: boolean; computed?: Record<string, string>; rect?: { x: number; y: number; width: number; height: number }; [key: string]: unknown }> } = {
    timestamp: new Date().toISOString(),
    elements: []
  };
...
// ============================================================================
diagnosticFramework.register('active-community-state-alignment', async (): Promise<ActiveCommunityStateDiagnosticResult> => {
  ...
  const readStateArray = async (path: string): Promise<string[]> => {
    try {
  ...
  const windowGetStateCommunities = toStringArray(
    typeof window.getState === 'function' ? window.getState('ui.activeCommunities') : undefined
  );
...
diagnosticFramework.register('load-history-registration', async (): Promise<LoadHistoryRegistrationResult> => {
  const timestamp = new Date().toISOString();
  const issues: string[] = [];
  const recommendations: string[] = [];
...
diagnosticFramework.register('theme-application', async (): Promise<ThemeDiagnosticResult> => {
  ...
  if (results.theme) {
    const currentUser = window.currentUser as { theme?: string; [key: string]: unknown } | undefined;
...
diagnosticFramework.register('user-hover-current-user', async (): Promise<UserHoverGlobalDiagnosticResult> => {
  const timestamp = new Date().toISOString();
  const stateManagerUser = typeof window.stateManagerInstance?.getState === 'function'
    ? (window.stateManagerInstance.getState('currentUser') as { id?: string } | null)
    : null;
  const modalInstance = window.userHoverModal as { getViewerUserId?: () => string | null; initialize?: () => Promise<void> } | undefined;
  const viewerUserId = typeof modalInstance?.getViewerUserId === 'function'
    ? modalInstance.getViewerUserId()
    : null;

  const result: UserHoverGlobalDiagnosticResult = {
    timestamp,
...
diagnosticFramework.register('backend-connectivity', async (): Promise<BackendConnectivityDiagnosticResult> => {
  const timestamp = new Date().toISOString();
  const issues: string[] = [];
  const checks: BackendConnectivityDiagnosticResult['checks'] = [];

  const resolvedBase = (typeof window !== 'undefined' && (window as Window & { API_BASE_URL?: string }).API_BASE_URL)
    || API_CONFIG.baseUrl;
  ...
  const runCheck = async (url: string): Promise<{ status: 'success' | 'failed'; latencyMs?: number; error?: string; note?: string }> => {
...
diagnosticFramework.register('load-chat-history-connectivity', async (): Promise<LoadChatHistoryConnectivityResult> => {
  const timestamp = new Date().toISOString();
  const issues: string[] = [];
  
  // Get API base URL
  const apiBaseURL = typeof window !== 'undefined' && (window as Window & { API_BASE_URL?: string }).API_BASE_URL
    ? (window as Window & { API_BASE_URL: string }).API_BASE_URL
    : (typeof window !== 'undefined' && window.stateManagerInstance?.getState
      ? (window.stateManagerInstance.getState('api') as { baseURL?: string } | null)?.baseURL
      : null) || API_CONFIG.baseUrl;

  // Get backend health service state
  const backendHealth = typeof window !== 'undefined' && (window as Window & { backendHealthService?: { getState: () => { status: string; lastChecked: number | null; lastError: string | null; retryDelayMs: number; consecutiveFailures: number } } }).backendHealthService
    ? (window as Window & { backendHealthService: { getState: () => { status: string; lastChecked: number | null; lastError: string | null; retryDelayMs: number; consecutiveFailures: number } } }).backendHealthService.getState()
    : (typeof window !== 'undefined' && window.stateManagerInstance?.getState
      ? (window.stateManagerInstance.getState('system.backendHealth') as { status: string; lastChecked: number | null; lastError: string | null; retryDelayMs: number; consecutiveFailures: number } | null)
      : null);

  const result: LoadChatHistoryConnectivityResult = {
    timestamp,
    apiBaseURL,
  ...
  const currentUrlData = typeof window !== 'undefined' && window.stateManagerInstance?.getState
    ? (window.stateManagerInstance.getState('currentUrlData') as { pageId?: string } | null)
    : null;
...
diagnosticFramework.register('api-request-response', async (): Promise<ApiRequestResponseResult> => {
    const results: ApiRequestResponseResult & { recentRequests: Array<{ url?: string; method?: string; body?: unknown; duration?: number; success?: boolean; response?: string; [key: string]: unknown }>; errors: Array<{ type?: string; error?: string; stack?: string; [key: string]: unknown }> } = {
      timestamp: new Date().toISOString(),
      recentRequests: [] as Array<{ url?: string; method?: string; body?: unknown; duration?: number; success?: boolean; response?: string; [key: string]: unknown }>,
      errors: [] as Array<{ type?: string; error?: string; stack?: string; [key: string]: unknown }>
    };
...
diagnosticFramework.register('sidepanel-settings-injections', async (): Promise<SidepanelSettingsInjectionResult> => {
  const moduleGraph = window.__CANOPI_MODULE_GRAPH__;
  const notes: string[] = [];

  const result: SidepanelSettingsInjectionResult = {
    timestamp: new Date().toISOString(),
    windowFlags: {
      hasModuleGraph: Boolean(moduleGraph),
      moduleGraphKeys: moduleGraph ? Object.keys(moduleGraph as Record<string, unknown>) : [],
...
diagnosticFramework.register('realtime-presence-typing', async (): Promise<RealtimePresenceTypingResult> => {
  const result: RealtimePresenceTypingResult = {
    timestamp: new Date().toISOString(),
  ...
  try {
    ...
    result.samples = users.slice(0, 3).map((user) => ({
      ...
      auraColor: (user as { auraColor?: string }).auraColor ?? '#ffffff',
...
diagnosticFramework.register('profile-logging-firehose', async (): Promise<ProfileLoggingFirehoseDiagnosticResult> => {
  const timestamp = new Date().toISOString();
  const environment: 'production' | 'development' | 'unknown' = Logger.isProductionMode()
    ? 'production'
...
// ============================================================================
diagnosticFramework.register('slice8-error-handling-consistency', async (): Promise<DiagnosticResult> => {
  const timestamp = new Date().toISOString();
  ...
  const latestByKey = new Map<string, ErrorBoundaryRecordLite>();
  ...
  const missingBoundaries = requiredBoundaries
    ...
    .map(req => ({ ...req, status: 'missing' as const }));

  const severityMismatches: Array<Record<string, string | undefined>> = [];
  const fatalWithoutRethrow: Array<Record<string, string | undefined>> = [];
  const recoverableRethrow: Array<Record<string, string | undefined>> = [];
...
if (typeof window !== 'undefined') {
  ...
  (window as unknown as { diagnosticFramework: RootCauseDiagnosticFramework }).diagnosticFramework = diagnosticFramework;
