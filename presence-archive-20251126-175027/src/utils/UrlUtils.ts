...
import { normalizeUrl, type NormalizationResult } from './UrlNormalization.js';
 ...
 */
export async function getCurrentPageUri(): Promise<string | null> {
  try {
 ...
 */
export async function normalizeCurrentUrl(): Promise<NormalizationResult & { rawUrl: string }> {
  try {
