...
export class UnifiedMessageRenderer {
   ...
   */
  static async generateMessageHTML(message: Message, options: MessageOptions = {}): Promise<string> {
    const {
      isReply = false,
      isFocusMode = false,
      author = null,
    ...
    if (avatarUser) {
      try {
        ...
        handleError(error, {
            ...
            context: {
                operation: 'catch',
            component: 'UnifiedMessageRenderer'
            }
        });;
        ...
        const nameSource = avatarUser.name ?? avatarUser.displayName ?? '?';
    ...
    if (!XIcons || typeof XIcons.reply !== 'function') {
      Logger.error('❌ UnifiedMessageRenderer: XIcons not available!', { 
        XIcons: !!XIcons,
    ...
    Logger.debug('🔍 UnifiedMessageRenderer: Generating icons...', {
      hasXIcons: !!XIcons,
    ...
    Logger.debug('🔍 UnifiedMessageRenderer: Icons generated', {
    ...
    if (!replyIcon || !repostIcon || !likeIcon || !shareIcon || !bookmarkIcon) {
      Logger.error('❌ UnifiedMessageRenderer: Icon generation failed!', {
        replyIcon: !!replyIcon,
    ...
    Logger.debug('🔍 UnifiedMessageRenderer: Buttons generated', {
    ...
    if (!replyButton || !reactionButton || !bookmarkButton || !shareButton) {
      Logger.error('❌ UnifiedMessageRenderer: Button generation failed!', {
   ...
   */
  static async renderMessage(message: Message, options: MessageOptions = {}): Promise<HTMLElement> {
    const {
      isReply = false,
      isFocusMode = false,
      author = null,
   ...
   * Format message time
   */
  static formatMessageTime(timestamp: string | Date | undefined, _isFocusMode: boolean = false): string {
    if (!timestamp) return '';
...
const boundGenerateMessageHTML = UnifiedMessageRenderer.generateMessageHTML.bind(UnifiedMessageRenderer);
const boundRenderMessage = UnifiedMessageRenderer.renderMessage.bind(UnifiedMessageRenderer);
const boundConvertUrlsToLinks = UnifiedMessageRenderer.convertUrlsToLinks.bind(UnifiedMessageRenderer);
const boundFormatMessageTime = UnifiedMessageRenderer.formatMessageTime.bind(UnifiedMessageRenderer);

let hasInitializedRenderer = false;

export const unifiedMessageRendererApi: UnifiedMessageRendererApi = {
  renderer: UnifiedMessageRenderer,
  renderMessage: boundRenderMessage,
  generateMessageHTML: boundGenerateMessageHTML,
  convertUrlsToLinks: boundConvertUrlsToLinks,
  formatMessageTime: boundFormatMessageTime,
  initialize: initializeUnifiedMessageRenderer
};

export function initializeUnifiedMessageRenderer(): UnifiedMessageRendererApi | null {
  if (hasInitializedRenderer) {
    return unifiedMessageRendererApi;
  }
  if (typeof window === 'undefined') {
    Logger.warn('⚠️ UnifiedMessageRenderer: Skipping DOM bootstrap (no window).', null, 'messages');
    return null;
  }
  (window as Window & { UnifiedMessageRenderer?: typeof UnifiedMessageRenderer }).UnifiedMessageRenderer = UnifiedMessageRenderer;
  hasInitializedRenderer = true;
  Logger.debug('✅ UnifiedMessageRenderer: API initialized', null, 'messages');
  return unifiedMessageRendererApi;
}

if (typeof window !== 'undefined') {
  initializeUnifiedMessageRenderer();
}
