...
class ProfileManager {
        ...
        if (!userAvatar) {
            ...
            if (legacyContext.AvatarUtils && legacyContext.currentUser) {
                try {
                    ...
                    const sourceUser = (this.profileData || legacyContext.currentUser || {
                    ...
                    if (!userDataForAvatar.availability) {
                        ...
                        if (legacyContext.currentUser && (legacyContext.currentUser.availability || legacyContext.currentUser.globalAvailability)) {
                            ...
                            const currentUserInVisibility = legacyContext.currentVisibilityDataUnfiltered.active.find(u => String(u.id || u.userId || u.user_id) === String(legacyContext.currentUser?.id));
    ...
    `;
        ...
        if (legacyContext.AvatarUtils && currentUser) {
            try {
                smallAvatarHTML = await legacyContext.AvatarUtils.createUnifiedAvatar(currentUser, 'menu', {
                    showAura: true,
                    size: 24
                });
            }
...
async function updateAuraColorEverywhere(color) {
    ...
    try {
        ...
        if (legacyContext.currentVisibilityData?.active) {
            const currentUserInVisibility = legacyContext.currentVisibilityData.active.find(u => String(u.id || u.userId || u.user_id) === String(legacyContext.currentUser?.id));
            if (currentUserInVisibility) {
                currentUserInVisibility.aura_color = color;
                currentUserInVisibility.auraColor = color;
            }
