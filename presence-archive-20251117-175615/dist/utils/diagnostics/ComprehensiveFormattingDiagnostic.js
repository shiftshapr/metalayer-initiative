...
const getMessages = () => Array.from(document.querySelectorAll('.message[data-message-id]'));
const addFinding = (collection, finding) => {
    if (finding) {
        collection.push(finding);
    }
};
const diagnoseMessageIcons = (messages) => {
...
const diagnoseAvatarAuras = (messages) => {
    ...
    if (messagesWithAuras.length < messages.length) {
        const auraColors = Array.from(new Set(messages
...
const diagnoseFocusModeLink = () => {
    const bodies = Array.from(document.querySelectorAll('.message-content, .message-body'));
    const bodiesWithLink = bodies.filter(body => {
        const hasClick = !!body.getAttribute('onclick') || typeof body.onclick === 'function' || !!body.getAttribute('data-focus-mode');
        const parent = body.closest('.message');
        const parentHasClick = !!parent &&
...
const diagnoseRendererAvailability = () => {
    const diagWindow = window;
    const rendererAvailable = typeof diagWindow.UnifiedMessageRenderer !== 'undefined';
    const generateHTMLAvailable = rendererAvailable && typeof diagWindow.UnifiedMessageRenderer?.generateMessageHTML === 'function';
    const avatarUtilsAvailable = typeof diagWindow.AvatarUtils !== 'undefined';
    const createAvatarAvailable = !!diagWindow.AvatarUtils && typeof diagWindow.AvatarUtils.createUnifiedAvatar === 'function';
...
const diagnoseMessageStructure = (messages) => {
...
export const runComprehensiveFormattingDiagnostic = async () => {
    ...
    window.formattingDiagnosticResults = result;
    const issues = [
...
//# sourceMappingURL=ComprehensiveFormattingDiagnostic.js.map