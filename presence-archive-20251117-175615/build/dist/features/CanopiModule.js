...
const resolveAuthorFromPayload = (rawMessage, authorId) => {
    ...
    return {
        ...
        avatarUrl: authorSource?.avatarUrl || null,
...
const renderMessageElement = async (message, chatContainer = getChatMessagesContainer()) => {
    ...
    if (legacyContext.UnifiedMessageRenderer &&
        typeof legacyContext.UnifiedMessageRenderer.generateMessageHTML === 'function') {
                        ...
                        if (messages && messages.length > 0) {
                            ...
                            const convertedMessages = messages.map((msg) => ({
                                ...
                                author: {
                                    ...
                                    name: formatAuthorName(msg.AppUser),
                                    handle: formatUserHandle(msg.AppUser),
                                    avatarUrl: msg.AppUser?.avatarUrl || null,
                                    auraColor: msg.AppUser?.auraColor || window.AVATAR_FALLBACK_COLOR || '#ffffff'
                                }
                            }));
        ...
        if (normalizedMessages.length > 0) {
            ...
            normalizedMessages.forEach(msg => {
                if (!msg.parentId) {
                    ...
                    const replies = normalizedMessages.filter(m => m.parentId === msg.id &&
                        !m.deletedAt &&
                        m.content &&
    ...
    try {
        if (editingMessageId) {
            ...
            if (typeof window !== 'undefined' && window.handleEditMessage) {
                ...
                if (messageToEdit) {
                    await window.handleEditMessage({ ...messageToEdit, content: content });
                }
                else {
...
//# sourceMappingURL=CanopiModule.js.map