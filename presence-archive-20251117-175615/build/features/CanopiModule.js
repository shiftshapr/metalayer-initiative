...
function updateMessageInChat(updatedMessage) {
    ...
    if (contentElement) {
        const contentWithLinks = convertUrlsToLinks(updatedMessage.content || '');
...
function getSenderName(userId) {
    ...
    if (typeof window !== 'undefined' && window.currentChatData) {
        const message = window.currentChatData.find((m) => m.authorId === userId || m.author?.id === userId);
                ...
                for (const message of allMessages) {
                    ...
                    try {
                        ...
                        const html = await UnifiedMessageRenderer.generateMessageHTML(message, {
                            ...
                            formattedTime: message.createdAt ? UnifiedMessageRenderer.formatMessageTime(message.createdAt) : '',
...
async function handleEditMessage(message) {
    ...
    chatTextarea.value = message.content || '';
...
async function handleShareMessage(message, shareType = 'link') {
    ...
    try {
        if (shareType === 'link') {
            ...
            const tweetText = `Check out this message from @${message.author?.handle || message.author?.name || 'Metalayer'}: "${message.content}" ${messageUrl}`;
