...
export class AnchorNavigator {
     ...
     */
    async waitForElement(selector, timeout = this.maxWaitTime) {
        return new Promise((resolve) => {
            const startTime = Date.now();
            const checkElement = () => {
     ...
     */
    async scrollToElement(element, behavior = 'smooth', offsetTop = 100) {
        try {
