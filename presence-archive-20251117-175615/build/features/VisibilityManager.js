            ...
            const usersWithAvatars = await Promise.all(users.map(async (user) => {
                try {
                    ...
                    return {
                        ...
                        handle: userProfile?.handle || user.email?.split('@')[0] || 'unknown',
                        avatarUrl: userProfile?.avatarUrl || null,
                        auraColor: user.auraColor || AVATAR_FALLBACK_COLOR,
    ...
    if (usersWithAvatars.length > 0) {
        for (const user of usersWithAvatars) {
            try {
                ...
                const avatarUrl = user.avatarUrl || user.picture || '/icons/default-user.svg';
                const auraColor = user.auraColor || '#aaaaaa';
                // Create list item
