...
class DisplayNameManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this.charMax.textContent = this.maxLength; // Set max chars in UI
  ...
  setupEventListeners() {
    ...
    this.displayNameInput.addEventListener('focus', () => {
      const hasContent = this.displayNameInput.value.trim().length > 0;
  ...
  async saveDisplayName() {
    try {
      ...
      // ROOT CAUSE FIX: Min 4, max 20 chars, null is ok
      ...
      if (window.userPreferencesManager && window.userPreferencesManager.isInitialized) {
        ...
        await window.userPreferencesManager.savePreference('displayName', newDisplayName, { batch: false });
