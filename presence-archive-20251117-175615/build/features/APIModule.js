...
class MetaLayerAPI {
    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            const isConnectionError = error.message && (error.message.includes('Failed to fetch') ||
            ...
            if (isConnectionError) {
                ...
                if (!window._apiConnectionErrors || window._apiConnectionErrors < 1) {
                    window._apiConnectionErrors = (window._apiConnectionErrors || 0) + 1;
    ...
    async getPresenceByUrl(url, communityIds = null) {
        ...
        try {
            ...
            if (window.currentUser && window.currentUser.id) {
                ...
                console.log('🔍 API: Using window.currentUser for authentication:', user.id);
            }
            else if (window.authManager && typeof window.authManager.getCurrentUser === 'function') {
                user = await window.authManager.getCurrentUser();
                ...
                console.log('🔍 API: Using realGoogleAuth for authentication:', user?.id);
            }
            else {
    ...
    async getChatHistory(communityId, threadId = null, uri = null) {
        ...
        try {
            ...
            if (uri) {
                ...
                const normalizedUrl = window.normalizeUrl ? window.normalizeUrl(uri) : uri;
            ...
            let query = window.supabase.from('messages').select('*, AppUser:user_id(*)');
            ...
            const msgs = messages?.map(msg => ({
                ...
                author: {
                    ...
                    // FIX: Get aura color from AppUser table, not window.currentUser
                    auraColor: msg.AppUser?.auraColor || window.AVATAR_FALLBACK_COLOR
                },
                conversation: {
