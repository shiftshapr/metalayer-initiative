...
(function() {
  ...
  if (window.ReplyLoader && window.ReplyLoader.loadAllReplies) {
    ...
    window.ReplyLoader.loadAllReplies = async function(messageId, pageId, communityId) {
      ...
      try {
        ...
        trace('REPLY_LOADER_RESULT', { 
          messageId, 
          replyCount: result?.length || 0, 
          replies: result 
        });
        return result;
      } catch (err) {
        trace('REPLY_LOADER_ERROR', { messageId, error: err.message, stack: err.stack });
        throw err;
      }
    };
  }
  ...
  if (window.addMessageToFocus) {
    ...
    window.addMessageToFocus = async function(message, isReply = false) {
      ...
      try {
        ...
        setTimeout(() => {
          const domElement = document.querySelector(`[data-message-id="${message?.id}"]`);
          if (domElement) {
  ...
  // Export trace data
  window.getFocusModeReplyTrace = function() {
    return {
      traces,
      summary: {
