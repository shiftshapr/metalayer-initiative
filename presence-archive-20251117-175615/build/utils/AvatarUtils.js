...
class AvatarUtils {
    ...
    static async getAvatarUrl(user, context = 'visibility') {
        ...
        avatarUrl = user.avatarUrl || user.avatar_url || user.picture || null;
        if (avatarUrl) {
            ...
            if (typeof window !== 'undefined' && window.currentUser) {
                ...
                if ((currentUser.id === user.id || currentUser.user_id === user.user_id) && currentUser.avatarUrl) {
    ...
    static async createUnifiedAvatar(user, context = 'visibility', options = {}) {
        ...
        <div class="avatar-container" style="width: ${size}px; height: ${size}px; position: relative; overflow: visible;">
          ${showAura ? `
            ...
            <!-- Removed avatar-aura-ring - aura-background now provides the border/glow effect -->
          ` : ''}
          <img 
            src="${avatarUrl}" 
            alt="${userName}" 
            class="avatar-img" 
            style="width: ${size}px; height: ${size}px; border-radius: 50%; position: relative; z-index: 1; object-fit: cover; border: none; background-color: transparent; box-shadow: ${showAura ? `0 0 ${glowSize}px ${auraColor}40` : 'none'}; cursor: pointer;" 
            referrerpolicy="no-referrer"
            title="${userName}${user.email ? ` (${user.email})` : ''}"
