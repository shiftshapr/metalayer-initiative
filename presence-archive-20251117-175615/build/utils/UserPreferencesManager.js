...
class UserPreferencesManager {
  constructor() {
    ...
    // Batching for multiple preference changes
    this.batchQueue = [];
    this.batchTimeout = null;
    this.batchDelay = 500; // 500ms batching window
  ...
  async loadAllPreferences() {
    ...
    try {
      ...
      this.metrics.errors.push({ type: 'load', error: error.message, timestamp: Date.now() });
      this.log('load', 'failure', duration, error);
      throw error;
    } finally {
      this.isLoading = false;
    }
  ...
  async savePreference(key, value, options = {}) {
    ...
    try {
      ...
      this.metrics.errors.push({ type: 'save', key, error: error.message, timestamp: Date.now() });
      this.emitPreferenceSaveFailed(key, value, error);
      return false;
    }
  }
  ...
  async saveBatchToDatabase(preferences) {
    ...
    try {
      ...
      this.metrics.errors.push({ type: 'save', error: error.message, timestamp: Date.now() });
      this.log('save', 'failure', duration, error);
  ...
  updateCurrentUser() {
    ...
    window.currentUser.aura_color = this.preferences.auraColor;
