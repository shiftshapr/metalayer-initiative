...
class UnifiedSettingsStorage {
   ...
   * @param {string} key - Setting key (e.g., 'theme', 'auraColor', 'status', 'visibilityEnabled')
   * @param {any} value - Setting value
   * @param {object} options - Optional: { skipApi: boolean, apiKey: string }
   */
    ...
    try {
      ...
      if (window.currentUser) {
        if (key === 'theme' || key === 'auraColor' || key === 'status' || key === 'visibilityEnabled') {
          if (!window.currentUser.preferences) {
            window.currentUser.preferences = {};
          }
          window.currentUser.preferences[key] = value;
        }
        
        // Also update direct properties for backward compatibility
        if (key === 'auraColor') {
          window.currentUser.auraColor = value;
          window.currentUser.aura_color = value;
        }
        if (key === 'status' || key === 'availability') {
          window.currentUser.availability = value;
          window.currentUser.globalAvailability = value;
        }
   ...
   * @param {object} options - Optional: { skipApi: boolean, apiKey: string, forceDatabase: boolean }
   */
  async getSetting(key, defaultValue = null, options = {}) {
    try {
      ...
      if (window.currentUser) {
        if (window.currentUser.preferences && window.currentUser.preferences[key] !== undefined) {
          const value = window.currentUser.preferences[key];
