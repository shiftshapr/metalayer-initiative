    ...
    try {
      ...
      if (messageContent) {
        ...
        if (textarea) {
          ...
          textarea.focus();
          (window as any)?.autoResize?.(textarea);
        }
      ...
      if (visibilityContent) {
        (window as any)?.showNotification?.('Visibility anchoring feature coming soon!');
  ...
  private async migrateFromChromeStorage(): Promise<void> {
    try {
      ...
      for (const key of keys) {
        ...
        if (value !== undefined) {
          await this.graph.stateManager.setState(key, value as unknown);
        }
      }
    } catch (error) {
