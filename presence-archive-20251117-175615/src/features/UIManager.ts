...
interface ApiClient {
  request: (url: string, options?: RequestInit & { headers?: Record<string, string> }) => Promise<any>;
}
...
const createFallbackAvatar = (user?: User): string => {
...
export class UIManager {
  ...
  private readonly window?: UIManagerWindow;
  ...
  constructor(private readonly deps: UIManagerDependencies) {
  ...
  private setupEventListeners(): void {
    ...
    this.document.addEventListener('errorOccurred', (event: Event) => {
      const detail = (event as CustomEvent<{ message?: string; details?: any }>).detail;
  ...
  private handleError(errorData: { message?: string; details?: any }): void {
    this.logger.error?.('Handling error', errorData);
    this.showError(errorData.message || 'An error occurred', errorData.details);
  }

  private showError(message: string, details: any = null): void {
    if (!this.document) return;
...
function createDefaultDependencies(): UIManagerDependencies {
  const envWindow = (typeof window !== 'undefined' ? window : undefined) as UIManagerWindow | undefined;
...
export const updateVisualHierarchy = (): void => diagnosticsController.updateVisualHierarchy();
export const debugHierarchy = (): void => diagnosticsController.debugHierarchy();
export const forceRefreshCSS = (): void => diagnosticsController.forceRefreshCSS();
export const setupTabNavigation = (): void => uiManagerInstance.setupTabNavigation();
export const setupMessageInputEventListeners = (): void => uiManagerInstance.setupMessageInputEventListeners();
export const initializeTheme = (): Promise<void> => uiManagerInstance.initializeTheme();
export const setTheme = (theme: string): Promise<void> => uiManagerInstance.setTheme(theme);
export const toggleTheme = (): Promise<void> => uiManagerInstance.toggleTheme();

export default uiManagerInstance;

if (typeof window !== 'undefined') {
  const win = window as UIManagerWindow;
  win.UIManager = UIManager as unknown as typeof UIManager;
  ...
  win.autoResize = (textarea: HTMLTextAreaElement | null) => autoResize(textarea);
