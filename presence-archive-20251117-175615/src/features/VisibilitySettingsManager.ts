...
interface UserPreferencesManager {
  ...
  savePreference: (key: string, value: any, options?: { batch?: boolean }) => Promise<boolean>;
}

interface UnifiedSettingsStorage {
  getSetting: (key: string, defaultValue: any, options?: any) => Promise<any>;
}
...
let saveSetting: ((key: string, value: any, options?: any) => Promise<void>) | undefined;
let saveSettings: ((settings: Record<string, any>) => Promise<void>) | undefined;
let getSetting: ((key: string, defaultValue: any, options?: any) => Promise<any>) | undefined;
...
class VisibilitySettingsManager {
  ...
  setupEventListeners(): void {
    ...
    if (this.themeToggle) {
      ...
      this.themeToggle.addEventListener('change', async (e: Event) => {
        e.stopPropagation();
  ...
  async loadSettings(): Promise<void> {
    try {
      ...
      if (userPreferencesManager && userPreferencesManager.isInitialized) {
        ...
        isVisible = await userPreferencesManager.getPreference('isVisible');
        status = await userPreferencesManager.getPreference('globalAvailability');
        ...
        // Load status
        status = await (getSetting || this.getSettingFallback)('status', 'AVAILABLE', { apiKey: 'status' });
      }
   ...
   */
  async getSettingFallback(key: string, defaultValue: any, options: any = {}): Promise<any> {
    try {
      const storageData = await chrome.storage.local.get([key]) as Record<string, any>;
