...
declare const chrome: {
  storage?: {
    local?: {
      get(keys: string[] | Record<string, any>, callback: (result: Record<string, any>) => void): void;
...
const getChromeStorage = <T extends Record<string, any>>(keys: string[]): Promise<T> => {
...
const setChromeStorage = (items: Record<string, any>): Promise<void> => {
...
const ensureLegacyUser = (user?: LegacyUser | null): LegacyUser => {
  ...
  if (!user.id) {
    user.id = user.user_id || `user-${Date.now()}`;
  }
  if (!user.name) {
...
class ProfileManager {
  ...
  private avatarCache: Map<string, any>;
    ...
    try {
      ...
      if (legacyContext.preRenderInitializer?.getPreRenderData) {
        ...
        if (preRenderData.auraColor && legacyContext.currentUser) {
          ...
          legacyContext.currentUser.aura_color = preRenderData.auraColor;
      ...
      if (typeof legacyContext.getState === 'function') {
        ...
        if (storedColor && storedColor !== legacyContext.AVATAR_FALLBACK_COLOR && legacyContext.currentUser) {
          ...
          legacyContext.currentUser.aura_color = storedColor;
    ...
    if (!userAvatar) {
      ...
      if (legacyContext.AvatarUtils && legacyContext.currentUser) {
        try {
          ...
          if (!auraColorValue) {
            auraColorValue = this.profileData?.auraColor || this.profileData?.aura_color || legacyContext.currentUser?.auraColor || legacyContext.currentUser?.aura_color;
          }

          // CRITICAL FIX: Use profileData if available (has aura color), otherwise fall back to currentUser
          const sourceUser: LegacyUser = (this.profileData || legacyContext.currentUser || {
          ...
          const userDataForAvatar: LegacyUser = {
            ...
            aura_color: auraColorValue || this.profileData?.aura_color || legacyContext.currentUser?.aura_color
          };
          ...
          if (!userDataForAvatar.availability) {
            ...
            if (legacyContext.currentUser && (legacyContext.currentUser.availability || legacyContext.currentUser.globalAvailability)) {
              ...
              const currentUserInVisibility = legacyContext.currentVisibilityDataUnfiltered.active.find(u => 
                String(u.id || u.userId || u.user_id) === String(legacyContext.currentUser?.id)
              );
  ...
  performLogout() {
    ...
    try {
      // Clear user data
      legacyContext.currentUser = null;
      legacyContext.supabaseUser = null;
  ...
  async handleUserUpdate(user) {
    ...
    if (legacyContext.syncStorageWithDatabase && typeof legacyContext.syncStorageWithDatabase === 'function') {
      legacyContext.syncStorageWithDatabase().catch(err => {
    ...
    else if (this.profileData.id && legacyContext.api) {
      ...
      try {
        ...
        if (userResponse && (userResponse.auraColor || userResponse.aura_color)) {
          ...
          if (legacyContext.currentUser) {
            ...
            legacyContext.currentUser.aura_color = auraColor;
          }
  ...
  async updateUserAvatar() {
    ...
    if (!auraColorValue && this.profileData.id && legacyContext.api) {
    ...
    try {
      ...
      if (legacyContext.AvatarUtils) {
        ...
        if (legacyContext.currentUser && String(legacyContext.currentUser.id || legacyContext.currentUser.user_id) === String(userForAvatar.id || userForAvatar.userId)) {
  ...
  refreshProfileAvatar() {
    ...
    if (legacyContext.currentVisibilityDataUnfiltered && legacyContext.currentVisibilityDataUnfiltered.active) {
      const userInVisibility = legacyContext.currentVisibilityDataUnfiltered.active.find(
...
async function updateAuraColorEverywhere(color) {
  ...
  try {
    ...
    if (legacyContext.currentUser) {
      ...
      legacyContext.currentUser.aura_color = color;
    ...
    if (legacyContext.currentVisibilityData?.active) {
      const currentUserInVisibility = legacyContext.currentVisibilityData.active.find(u => 
        String(u.id || u.userId || u.user_id) === String(legacyContext.currentUser?.id)
      );
      if (currentUserInVisibility) {
        currentUserInVisibility.aura_color = color;
...
async function getCurrentUserAuraColor() {
  ...
  if (legacyContext.currentUser && legacyContext.currentUser.auraColor && legacyContext.currentUser.auraColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
    console.log(`✅ AURA_MODAL: Found database aura color in currentUser: ${legacyContext.currentUser.auraColor}`);
    return legacyContext.currentUser.auraColor;
  }
  
  if (legacyContext.currentUser && legacyContext.currentUser.aura_color && legacyContext.currentUser.aura_color !== legacyContext.AVATAR_FALLBACK_COLOR) {
    console.log(`✅ AURA_MODAL: Found database aura_color in currentUser: ${legacyContext.currentUser.aura_color}`);
    return legacyContext.currentUser.aura_color;
  }
  ...
  if (legacyContext.currentVisibilityData && legacyContext.currentVisibilityData.active) {
    ...
    if (currentUserEmail) {
      ...
      if (userData && userData.aura_color && userData.aura_color !== legacyContext.AVATAR_FALLBACK_COLOR) {
  ...
  if (profileManager) {
    ...
    if (visibilityBtn) {
      ...
      if (!visibilityBtn.dataset.handlerAttached) {
        ...
        if (!profileManager.visibilitySettingsHandler) {
          profileManager.visibilitySettingsHandler = (e: Event) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🔧 PROFILE MANAGER: COMP METHOD - Visibility settings button clicked');
...
async function updateAvailabilityEverywhere(availability) {
  ...
  try {
    ...
    const profileManagerInstance = legacyContext.profileManager || (legacyContext.ProfileManager && legacyContext.ProfileManager.instance) || this;
