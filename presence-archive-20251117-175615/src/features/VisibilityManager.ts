...
import { User, VisibilityData } from '../types/index.js';
...
declare const window: Window & {
  ...
  saveSetting?: (key: string, value: any, options?: any) => Promise<void>;
...
class VisibilityManager {
  ...
  async initialize(currentUserEmail: string): Promise<void> {
    try {
      ...
      this.supabase.on('presence', (eventType: string, newRecord: any, oldRecord: any) => {
   ...
   * Fetch avatar URLs for users
   */
  private async fetchUserAvatars(users: any[]): Promise<User[]> {
    try {
  ...
  private async updateVisibilityUI(users: User[]): Promise<void> {
    try {
      ...
      if (typeof window !== 'undefined') {
        (window as any).currentVisibilityDataUnfiltered = { active: normalizedUsers };
        (window as any).currentVisibilityData = { active: normalizedUsers };
      }
...
async function updateVisibleTab(avatars: User[]): Promise<void> {
  ...
  if (typeof window !== 'undefined') {
    (window as any).currentVisibilityData = { active: avatars };
    (window as any).currentVisibilityDataUnfiltered = { active: avatars };
  }
  ...
  if (usersWithAvatars.length > 0) {
    for (const user of usersWithAvatars) {
      try {
        ...
        if (typeof (window as any).AvatarUtils !== 'undefined' && 
            typeof (window as any).AvatarUtils.createUnifiedAvatar === 'function') {
          try {
            const avatarHTML = await (window as any).AvatarUtils.createUnifiedAvatar(user, 'visibility', {
              size: 32,
              showAura: true,
              showStatus: true
            });
  ...
  if (goInvisibleBtn) {
    goInvisibleBtn.addEventListener('click', async () => {
      ...
      if (typeof (window as any).setVisibilityStatus === 'function') {
        await (window as any).setVisibilityStatus(false);
      }
    });
  }
...
if (typeof window !== 'undefined') {
  (window as any).updateVisibleTab = updateVisibleTab;
