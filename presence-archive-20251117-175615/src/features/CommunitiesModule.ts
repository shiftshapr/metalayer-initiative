...
import { stateManagerInstance, getState, setState } from '../core/StateManager.js';
import { User } from '../types/index.js';
import { updateCommunityDropdown, updatePlaceholderText, getPrimaryCommunityName } from './CommunityHelpers.js';
import { loadCommunities, loadCombinedAvatars } from './CommunityLoaders.js';
import { AvatarUtils } from '../utils/AvatarUtils.js';
import { formatUserDisplayName, formatUserHandle, getUserIdentity } from '../utils/Fallbacks.js';
...
type LegacyIntegrationContext = typeof globalThis & {
  ...
  currentVisibilityDataUnfiltered?: { active?: any[] };
  currentVisibilityData?: { active?: any[] };
...
const getCurrentUser = (): User | undefined =>
  stateManagerInstance.getState('currentUser') as User | undefined;

const getCurrentUrlData = (): { pageId?: string; rawUrl?: string; normalizedUrl?: string } | undefined =>
  stateManagerInstance.getState('currentUrlData');

const setCurrentUrlData = (urlData: { pageId?: string; rawUrl?: string; normalizedUrl?: string } | undefined): void => {
...
const getActiveCommunitiesState = (): string[] =>
  stateManagerInstance.getState('ui.activeCommunities') || [];

const setActiveCommunitiesState = (communities: string[]): void => {
...
const getCurrentVisibilityDataUnfiltered = () =>
  stateManagerInstance.getState('currentVisibilityDataUnfiltered') as { active?: any[] } | undefined;
...
class CommunitiesModule {
  ...
  initializeCommunityDropdown() {
    ...
    const activateDropdown = () => {
      ...
      newTrigger.addEventListener('click', (e: Event) => {
      ...
      document.addEventListener('click', (e: Event) => {
        ...
        if (panelStyle.display === 'block' || 
          ...
          if (!panelEl.contains(e.target as Node) && !newTrigger.contains(e.target as Node)) {
