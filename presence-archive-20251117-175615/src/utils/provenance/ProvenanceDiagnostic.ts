...
class ProvenanceDiagnostic {
  ...
  async initialize(): Promise<void> {
    if (!(window as any).provenanceService) {
    ...
    if (typeof window !== 'undefined') {
      (window as any).provenanceDiagnostic = this;
  ...
  async displayMessageProvenance(messageId: string): Promise<void> {
    ...
    try {
      ...
      const verifier = (window as any).provenanceVerifier;
      if (verifier) {
          ...
          ">
            ...
            ${('error' in verification && verification.error) ? `
              <div style="margin-top: 8px; padding: 8px; background: #3a1a1a; border-left: 3px solid #f44336; border-radius: 4px; font-size: 11px; color: #ff6b6b;">
                Error: ${(verification as { error?: string }).error}
              </div>
            ` : ''}
          </div>
        `;
...
if (typeof window !== 'undefined') {
  (window as any).provenanceDiagnostic = new ProvenanceDiagnostic();
  console.log('[ProvenanceDiagnostic] Available at window.provenanceDiagnostic');
}
