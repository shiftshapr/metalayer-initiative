-- Migration: Add recursive reply chain scoring function
-- This function calculates top-scoring reply chains for message threads

-- ============================================================================
-- RECURSIVE REPLY CHAIN SCORING FUNCTION
-- ============================================================================
...
BEGIN
  ...
  WITH RECURSIVE reply_tree AS (
    ...
    WHERE m.parent_id::UUID = p_parent_id
    ...
    FROM messages m
    JOIN "AppUser" u ON m.user_id = u.id
    JOIN reply_tree rt ON m.parent_id::UUID = rt.reply_id
  ...
  reply_scores AS (
    ...
    LEFT JOIN LATERAL (
      SELECT COUNT(*)::INTEGER AS count
      FROM messages m
      WHERE m.parent_id::UUID = rt.reply_id
        AND NOT EXISTS (
          SELECT 1 FROM message_deletions md 
          WHERE md.message_id::UUID = m.id
        )
  ...
  direct_children AS (
    ...
    FROM reply_scores
    WHERE parent_id::UUID = p_parent_id
  ),
  ...
  chain_scores AS (
    SELECT 
      ...
      (
        SELECT SUM(reply_score)::INTEGER
