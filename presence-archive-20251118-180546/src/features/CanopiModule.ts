...
import { User, Message, Reaction, SupabaseClient } from '../types/index.js';
...
import { 
  ...
  type MessageSystemIntegration as MessageSystemIntegrationType
} from './MessageSystemIntegration.js';
import { UnifiedMessageDisplay } from '../components/UnifiedMessageDisplay.js';
import type { Message as MessageStoreMessage } from '../services/MessageStore.js';
...
type LegacyIntegrationContext = typeof globalThis & {
  ...
  normalizeUrl?: (url: string) => Promise<NormalizedUrlData> | NormalizedUrlData;
  robustIntegration?: {
  ...
  addMessageToChat?: (message: Partial<Message> & Record<string, unknown>) => Promise<void> | void;
  ...
  UnifiedMessageRenderer?: {
    renderMessage: (message: Message, options: Record<string, unknown>) => Promise<HTMLElement>;
    generateMessageHTML?: (message: Message, options: Record<string, unknown>) => Promise<string>;
...
let messageSystemIntegration: MessageSystemIntegrationType | null = null;
let unifiedMessageDisplay: UnifiedMessageDisplay | null = null;

/**
 * Initialize the new message system
 */
async function initializeNewMessageSystem(): Promise<void> {
  try {
    // Get Supabase client
    const supabase = typeof window !== 'undefined' 
      ? (window as Window & { supabase?: SupabaseClient }).supabase 
      : null;
    ...
    messageSystemIntegration = await initializeMessageSystemIntegration({
      ...
      onMessageUpdate: (messages: MessageStoreMessage[]) => {
...
type MessagePayload = Partial<Message> & Record<string, unknown>;
...
const normalizeMessagePayload = (rawMessage: MessagePayload): Message => {
  ...
  const appUser = rawMessage.AppUser as { id?: string } | undefined;
  ...
  const getStringOrDate = (value: unknown): string | Date => {
  ...
  const normalized: Message = {
    ...
    content: ensureMessageContent({
      content: (rawMessage.content ?? rawMessage.body ?? getStringValue(rawMessage.message) ?? '') as string
    }),
    ...
    createdAt: getStringOrDate(
      rawMessage.createdAt ?? 
      rawMessage.created_at ?? 
      rawMessage.timestamp
    ),
    updatedAt: getStringOrDate(
...
const renderMessageElement = async (
  ...
  if (renderer && typeof renderer.generateMessageHTML === 'function') {
    ...
    if (isReply) {
      ...
      } else if (typeof window !== 'undefined' && (window as Window & { focusedMessage?: Message | string | null }).focusedMessage) {
        messageDiv.classList.add('visible');
      }
    }
...
function updateMessageInChat(updatedMessage: Message): void {
  ...
  if (!messageElement) {
    ...
    if (legacyContext.addMessageToChat) {
      legacyContext.addMessageToChat(updatedMessage as Partial<Message> & Record<string, unknown>);
    }
    return;
  }
...
function getSenderName(userId: string): string {
  ...
  const activeUsers = Array.isArray(visibilityData) 
    ? visibilityData 
    : (visibilityData && typeof visibilityData === 'object' && 'active' in visibilityData && Array.isArray((visibilityData as { active?: unknown }).active))
      ? ((visibilityData as { active: User[] }).active)
      : [];
...
async function loadMessageReactions(messageId: string, reactionBtn?: HTMLElement | null): Promise<void> {
  ...
  try {
    ...
    const windowApi = typeof window !== 'undefined' ? (window as Window & { api?: { getReactions?: (messageId: string) => Promise<any> } }).api : undefined;
    if (windowApi && typeof windowApi.getReactions === 'function') {
      ...
      const windowSupabase = (window as Window & { supabase?: SupabaseClient }).supabase;
      if (!windowSupabase) return;
...
function addMessageActionListeners(messageDiv: HTMLElement, message: Message): void {
  ...
  if (replyButton) {
    replyButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleReplyClick = (window as Window & { handleReplyClick?: (messageId: string, message: Message) => void }).handleReplyClick;
      if (typeof handleReplyClick === 'function') {
        ...
        if (chatTextarea) {
          ...
          const setReplyContext = (window as Window & { setReplyContext?: (messageId: string, message: Message) => void }).setReplyContext;
  ...
  if (reactionButton) {
    reactionButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleReactionClick = (window as Window & { handleReactionClick?: (messageId: string, message: Message) => void }).handleReactionClick;
  ...
  if (bookmarkButton) {
    bookmarkButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleBookmarkClick = (window as Window & { handleBookmarkClick?: (messageId: string, message: Message) => void }).handleBookmarkClick;
  ...
  if (shareButton) {
    shareButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleShareClick = (window as Window & { handleShareClick?: (messageId: string, message: Message) => void }).handleShareClick;
  ...
  if (editButton) {
    editButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleEditClick = (window as Window & { handleEditClick?: (messageId: string, message: Message) => void }).handleEditClick;
  ...
  if (deleteButton) {
    deleteButton.addEventListener('click', (e) => {
      e.stopPropagation();
      const handleDeleteClick = (window as Window & { handleDeleteClick?: (messageId: string, message: Message) => void }).handleDeleteClick;
...
async function handleMessageFocus(messageOrId: Message | string): Promise<void> {
  try {
    ...
    const result = await messageSystemIntegration.loadFocusMode(
      pageId,
      message.parentId,
      { communityId }
    );
    ...
    await unifiedMessageDisplay.render(result.replies, container, {
      ...
      onMessageClick: (msg: MessageStoreMessage) => {
      ...
      onReplyClick: (msg: MessageStoreMessage) => {
        if (!msg.author || !legacyContext.setReplyContext) return;
        const legacyMessage: Message = {
          id: msg.id,
          content: msg.content,
          authorId: msg.author.id,
      ...
      onFocusClick: (msg: MessageStoreMessage) => {
        if (!msg.author) return;
        const legacyMessage: Message = {
          id: msg.id,
          content: msg.content,
...
async function loadChatHistory(communityIdOrRawUrl?: string | null, activeCommunitiesOrUndefined?: string[]): Promise<void> {
  ...
  try {
    ...
    // Normalize URL
    let pageId: string;
    let normalizedUrlData: NormalizedUrlData | undefined = getCurrentUrlData();
    ...
    await unifiedMessageDisplay.render(messages, chatMessages, {
      focusContext: 'default',
      onMessageClick: (message: MessageStoreMessage) => {
      ...
      onReplyClick: (message: MessageStoreMessage) => {
        if (!message.author || !legacyContext.setReplyContext) return;
        const legacyMessage: Message = {
          id: message.id,
          content: message.content,
          authorId: message.author.id,
      ...
      onFocusClick: (message: MessageStoreMessage) => {
        if (!message.author) return;
        const legacyMessage: Message = {
          id: message.id,
          content: message.content,
    ...
    const legacyMessages: Message[] = messages.filter((msg: MessageStoreMessage) => msg.author).map((msg: MessageStoreMessage) => ({
      id: msg.id,
      content: msg.content,
      authorId: msg.author!.id,
      communityId: communityId,
      ...
      if (footer) {
        ...
        if (toggleBtn) {
          toggleBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const toggleThreadReplies = (window as Window & { toggleThreadReplies?: (conversationId: string, messageElement: HTMLElement) => void }).toggleThreadReplies;
...
async function handleEditMessage(message: Message): Promise<void> {
  ...
  const saveEdit = async () => {
    ...
    if (newContent && newContent !== message.content) {
      try {
        // Use robust integration if available, fallback to legacy
        const robustIntegration = (window as Window & { robustIntegration?: { isInitialized: boolean; editMessage?: (id: string, content: string) => Promise<void> } }).robustIntegration;
        if (typeof window !== 'undefined' && robustIntegration && robustIntegration.isInitialized) {
          ...
          const client = typeof window !== 'undefined' ? ((window as Window & { supabaseRealtimeClient?: { editMessage?: (id: string, content: string) => Promise<void>; [key: string]: unknown } }).supabaseRealtimeClient) : null;
...
async function sendMessageViaSupabase(content: string, parentId: string | null = null, conversationId: string | null = null): Promise<Message | null> {
  ...
  const robustIntegration = ((window as unknown) as Window & { robustIntegration?: { isInitialized: boolean; sendMessage?: (content: string, parentId?: string | null, conversationId?: string | null) => Promise<Message> } }).robustIntegration;
  if (typeof window !== 'undefined' && robustIntegration && robustIntegration.isInitialized) {
  ...
  const client = typeof window !== 'undefined' ? ((window as Window & { supabaseRealtimeClient?: { sendMessage?: (content: string, parentId?: string | null, conversationId?: string | null) => Promise<Message>; [key: string]: unknown } }).supabaseRealtimeClient) : null;
...
async function getSenderAvatar(author: User | null | undefined): Promise<string> {
  ...
  const AvatarUtils = ((window as unknown) as Window & { AvatarUtils?: { createUnifiedAvatar?: (author: User, type: string, options?: Record<string, unknown>) => Promise<string> } }).AvatarUtils;
...
async function focusOnMessage(message: Message): Promise<void> {
  ...
  const handleMessageFocus = (window as Window & { handleMessageFocus?: (messageOrId: Message | string) => Promise<void> }).handleMessageFocus;
...
async function handleBackNavigation(): Promise<void> {
  ...
  const winWithFocus = window as Window & { focusedMessage?: Message | string | null; previousView?: string };
...
async function handleReaction(message: Message): Promise<void> {
  ...
  const reactionsIntegration = (window as Window & { reactionsIntegration?: { reactionsManager?: { addReaction: (messageId: string, emoji: string) => Promise<void> } } }).reactionsIntegration;
...
function setupMessageInputEventListeners(): void {
  ...
  if (cancelContextButton) {
    cancelContextButton.addEventListener('click', () => {
      const clearContext = (window as Window & { clearContext?: () => void }).clearContext;
...
async function sendChatMessage(): Promise<void> {
  ...
  try {
    if (editingMessageId) {
      // Handle edit
      const handleEditMessage = (window as Window & { handleEditMessage?: (message: Message) => Promise<void> }).handleEditMessage;
      ...
      const sendMessageViaSupabase = (window as Window & { sendMessageViaSupabase?: (content: string, parentId?: string | null, conversationId?: string | null) => Promise<Message | null> }).sendMessageViaSupabase;
      if (typeof window !== 'undefined' && sendMessageViaSupabase) {
        ...
        if (messageData) {
          ...
          const addMessageToChat = (window as Window & { addMessageToChat?: (message: Partial<Message> & Record<string, unknown>) => Promise<void> }).addMessageToChat;
    ...
    chatTextarea.value = '';
    const autoResize = (window as Window & { autoResize?: (textarea: HTMLTextAreaElement | null) => void }).autoResize;
...
const attachLegacyIntegrations = (): void => {
  ...
  const win = (window as unknown) as LegacyIntegrationContext;
