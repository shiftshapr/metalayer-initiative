...
import '../types/global.js'; // Ensure global types are loaded
import { SupabaseClient, User as UserType, APIResponse } from '../types/index.js';
...
interface ApiClient {
  request: <T = unknown>(url: string, options?: { method?: string; body?: string; headers?: Record<string, string> }) => Promise<APIResponse<T>>;
}
...
interface CustomAvatarColorResult {
...
async function authenticateWithSupabase(user: User): Promise<void> {
  try {
    ...
    const { data: { session }, error: sessionError } = await ((supabase.auth as unknown) as { getSession: () => Promise<{ data: { session: { user: { email?: string }; expires_at: number } | null }; error: unknown }> }).getSession();
    ...
    if (window.currentUser && user.email) {
      try {
        ...
        const appUser = await api.request(`/v1/users/${encodeURIComponent(user.email)}`, {
          method: 'POST',
          body: JSON.stringify({
            email: user.email,
        ...
        if (appUser && appUser.id) {
          ...
          if (window.currentUser) {
            ...
            // Set userId (camelCase only - RED-LINE compliance)
            ...
            try {
              ...
              if (completeUserData) {
                ...
                if (dbAvatarUrl && window.currentUser && dbAvatarUrl !== window.currentUser.avatarUrl) {
                ...
                if (authManager && typeof authManager.updateUserProfile === 'function') {
                  authManager.updateUserProfile({
                    auraColor: window.currentUser?.auraColor ?? null,
...
async function testRealtimeAfterAuth(pageId: string): Promise<boolean> {
  ...
  try {
    const testRealtimeWithAuth = (window as Window & { testRealtimeWithAuth?: (supabase?: SupabaseClient, pageId?: string) => Promise<{ success: boolean }> }).testRealtimeWithAuth;
  ...
  try {
    ...
    if (realGoogleAuth && realGoogleAuth.signInWithGoogle) {
      ...
      if (result && result.user) {
        ...
        console.log('🔍 REAL_GOOGLE_AUTH: Real avatar URL:', (result.user as UserType & { user_metadata?: { avatar_url?: string } }).user_metadata?.avatar_url);
      }
...
async function completeOTPForRealtime(otpCode: string): Promise<boolean> {
  ...
  try {
    ...
    if (result.success && result.user) {
      ...
      if (result.session && typeof result.session === 'object' && 'expires_at' in result.session && typeof (result.session as { expires_at: unknown }).expires_at === 'number') {
        console.log('✅ OTP VERIFICATION: Session expires at:', new Date((result.session as { expires_at: number }).expires_at * 1000));
      }
...
async function getCurrentUserEmail(): Promise<string | null> {
  try {
    ...
    const realGoogleAuth = (window as Window & { realGoogleAuth?: { getCurrentUser: () => Promise<User | null>; signInWithGoogle?: () => Promise<any> } }).realGoogleAuth;
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error('[AUTH] Error getting current user:', errorMessage);
    throw new Error('User not authenticated');
  }
}

// Get the avatar color for the current user (custom or default)
function getCurrentUserAvatarColor(): Promise<string> {
  return new Promise((resolve) => {
    // Modernized: Use StateManager instead of Chrome Storage
    const getState = window.getState;
    if (getState) {
      getState('customAvatarColor').then((result: unknown) => {
        const colorResult = result as CustomAvatarColorResult;
        if (colorResult && colorResult.customAvatarColor) {
          ...
          getCurrentUserEmail().then(email => {
            if (email) {
              ...
              resolve((window as Window & { AVATAR_FALLBACK_COLOR?: string }).AVATAR_FALLBACK_COLOR || '#ffffff');
            }
          }).catch(() => {
            resolve((window as Window & { AVATAR_FALLBACK_COLOR?: string }).AVATAR_FALLBACK_COLOR || '#ffffff'); // Default white
          });
        }
      });
    } else {
...
function getUserAvatarBgColor(): string {
