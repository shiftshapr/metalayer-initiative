import { User, TabData, Preferences, Message } from '../types/index.js';
...
function createDefaultDependencies(): UIManagerDependencies {
  ...
  const messaging: MessagingService = {
    sendMessage: async (content: string) => {
      try {
        ...
        Logger.error('SEND_CHAT_MESSAGE: sendLegacyMessage failed', error as unknown as LogData);
  ...
  const visibility: VisibilityService = {
    updateVisibleTab: async (users: VisibleUser[] | User[]) => {
      // Type cast to match the expected signature - updateVisibleTab expects User[]
      return await updateVisibleTabService(users as User[]);
    }
  };
