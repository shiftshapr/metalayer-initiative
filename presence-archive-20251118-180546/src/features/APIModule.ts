...
class MetaLayerAPI {
  ...
  async request<T = unknown>(endpoint: string, options: ExtendedAPIRequestOptions = {}): Promise<APIResponseOrNull<T>> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const error = new Error(errorDetails) as any;
  ...
  async getChatHistory(communityId: string, threadId: string | null = null, uri: string | null = null): Promise<{ conversations: Array<{ id: string; [key: string]: unknown }>; messages: Message[] }> {
    ...
    const supabase = stateManagerInstance.getState('supabase') as any;
    ...
    try {
      ...
      if (messages && messages.length > 0) {
        ...
        console.log(`🔍 CHAT_API: Sample message pageIds:`, messages.slice(0, 3).map((m: { page_id?: string }) => m.page_id));
      } else {
        ...
        console.log(`🔍 CHAT_API: Sample messages in DB (first 10):`, allMessages?.map((m: { page_id?: string; community_id?: string }) => ({ page_id: m.page_id, community_id: m.community_id })));
      }
