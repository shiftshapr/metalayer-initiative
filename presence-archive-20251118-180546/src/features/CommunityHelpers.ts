...
export async function updateCommunityDropdown(communities: Community[]): Promise<void> {
  ...
  const activeCommunities: string[] = Array.isArray(activeCommunitiesRaw) 
    ? activeCommunitiesRaw.filter((item): item is string => typeof item === 'string')
    : [];
  ...
  communities.forEach((community, index) => {
    ...
    if (checkbox) {
      checkbox.addEventListener('change', async (e: Event) => {
        ...
        const activeCommunitiesRaw = await getState('activeCommunities');
        let activeCommunities: string[] = Array.isArray(activeCommunitiesRaw) 
          ? activeCommunitiesRaw.filter((item): item is string => typeof item === 'string')
          : [];
