...
import { AnchorNavigator, anchorNavigator } from './AnchorNavigator';
import { AnchorHighlighter, anchorHighlighter } from './AnchorHighlighter';
...
export class NotificationAnchorManager {
  ...
  private navigator: AnchorNavigator;
  private highlighter: AnchorHighlighter;
  ...
  async navigateToAnchor(anchor: NotificationAnchor, url?: string): Promise<boolean> {
    try {
      this.logger.info('Starting anchor navigation:', anchor as unknown as LogData);
  ...
  async highlightElement(
    ...
    try {
      ...
      if (!element) {
        this.logger.warn('Element not found:', selector as unknown as LogData);
        return false;
      }

      await this.highlighter.applyHighlight(
        element,
        style as any,
        duration
      );
      ...
      this.logger.error('Error highlighting element:', error as unknown as LogData);
      return false;
    }
  }
  ...
  async scrollToElement(
    ...
    try {
      ...
      const offset = this.navigator.calculateScrollOffset();
      await this.navigator.scrollToElement(element, behavior as any, offset);

      return true;
    } catch (error) {
      this.logger.error('Error scrolling to element:', error as unknown as LogData);
      return false;
    }
  }
  ...
  private focusElement(element: HTMLElement): void {
    try {
      ...
      this.logger.debug('Could not focus element:', error as unknown as LogData);
    }
  }
  ...
  createAnchorFromSelection(url: string): NotificationAnchor | null {
    try {
      ...
      this.logger.error('Error creating anchor from selection:', error as unknown as LogData);
      return null;
    }
  }
  ...
  async testAnchor(anchor: NotificationAnchor): Promise<boolean> {
    try {
      ...
      this.logger.error('Error testing anchor:', error as unknown as LogData);
      return false;
    }
  }
