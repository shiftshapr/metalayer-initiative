...
type LegacyContext = typeof globalThis & {
  [key: string]: unknown;
  currentUser?: LegacyUser;
  preRenderInitializer?: PreRenderInitializer;
...
const legacyContext = globalThis as LegacyContext;

// Helper functions for type-safe access to legacyContext properties
const getApi = (): { request: (url: string, options?: { method?: string; body?: string }) => Promise<Record<string, unknown>> } | null => {
  const api = legacyContext.api as { request?: unknown } | undefined;
  if (api && typeof api.request === 'function') {
    return api as { request: (url: string, options?: { method?: string; body?: string }) => Promise<Record<string, unknown>> };
  }
  return null;
};

const ensureApi = (context: string): { request: (url: string, options?: { method?: string; body?: string }) => Promise<Record<string, unknown>> } | null => {
  const api = getApi();
  if (!api) {
...
const getVisibilityDataUnfiltered = (): { active: User[] } | null => {
  const candidate = legacyContext.currentVisibilityDataUnfiltered as { active?: unknown } | undefined;
...
const getVisibilityDataFiltered = (): { active: User[] } | null => {
  const candidate = legacyContext.currentVisibilityData as { active?: unknown } | undefined;
  if (candidate && Array.isArray(candidate.active)) {
    return { active: candidate.active as User[] };
  }
  return null;
...
const getUserPreferencesManager = (): { isInitialized?: boolean; savePreference?: (key: string, value: unknown, options?: { batch?: boolean }) => Promise<unknown> } | null => {
  const manager = legacyContext.userPreferencesManager as { isInitialized?: boolean; savePreference?: (key: string, value: unknown, options?: { batch?: boolean }) => Promise<unknown> } | undefined;
  return manager ?? null;
};
...
const ensureLegacyUser = (user?: LegacyUser | null): LegacyUser => {
  if (!user) {
    return {
      id: `user-${Date.now()}`,
      name: 'User',
...
class ProfileManager {
  ...
  async waitForAuthentication() {
    ...
    const waitForAuth = new Promise<void>((resolve) => {
      ...
      document.addEventListener(
        'authUIUpdate',
        (event: Event) => {
          const detail = (event as CustomEvent<{ isAuthenticated?: boolean; user?: LegacyUser }>).detail;
      ...
      document.addEventListener(
        'userUpdated',
        (event: Event) => {
          const detail = (event as CustomEvent<LegacyUser>).detail;
          if (detail && detail.id) {
            checkAuth();
          }
        },
    ...
    const waitForPreRender = new Promise<void>((resolve) => {
      if (legacyContext.preRenderInitializer?.isInitialized) {
    ...
    const hasUser = legacyContext.currentUser && legacyContext.currentUser.id;
    const preRenderReady = legacyContext.preRenderInitializer && legacyContext.preRenderInitializer.isInitialized;
  ...
  async initializeProfileAvatarImmediately() {
    ...
    const currentUser = legacyContext.currentUser;
    const preRenderData = legacyContext.preRenderInitializer?.getPreRenderData();
  ...
  async loadAuraColorFromStorage() {
    try {
      ...
      if (legacyContext.preRenderInitializer?.getPreRenderData) {
        const preRenderData = legacyContext.preRenderInitializer.getPreRenderData();
        console.log('🔧 PROFILE_MANAGER: Pre-render data available:', {
          isInitialized: preRenderData.isInitialized,
          auraColor: preRenderData.auraColor,
        ...
        if (preRenderData.auraColor && legacyContext.currentUser) {
          console.log('🎨 PROFILE_MANAGER: Using aura color from pre-render data (DATABASE):', preRenderData.auraColor);
          legacyContext.currentUser.auraColor = preRenderData.auraColor;
      ...
      if (typeof legacyContext.getState === 'function') {
        ...
        if (storedColor && storedColor !== legacyContext.AVATAR_FALLBACK_COLOR && legacyContext.currentUser) {
          console.log('🎨 PROFILE_MANAGER: Using aura color from storage (USER PREFERENCE):', storedColor);
          legacyContext.currentUser.auraColor = storedColor;
  ...
  async setupProfileMenuAndAuraModal() {
    ...
    if (!userAvatar) {
      ...
      if (legacyContext.AvatarUtils && legacyContext.currentUser) {
        try {
          ...
          if (!auraColorValue) {
            auraColorValue = this.profileData?.auraColor || legacyContext.currentUser?.auraColor;
          }

          // CRITICAL FIX: Use profileData if available (has aura color), otherwise fall back to currentUser
          const fallbackUser: LegacyUser = {
            id: legacyContext.currentUser?.id || 'anonymous-user',
            name: legacyContext.currentUser?.name || 'User',
            email: legacyContext.currentUser?.email || 'user@example.com'
          } as LegacyUser;
          const sourceUser: LegacyUser = (this.profileData || legacyContext.currentUser || fallbackUser) as LegacyUser;
          if (!sourceUser.id) {
            sourceUser.id = legacyContext.currentUser?.id || `user-${Date.now()}`;
          }
          const userDataForAvatar: LegacyUser = {
            ...sourceUser,
            auraColor: auraColorValue || this.profileData?.auraColor || legacyContext.currentUser?.auraColor
          };
            ...
            if (legacyContext.currentUser && (legacyContext.currentUser.availability || legacyContext.currentUser.globalAvailability)) {
              ...
              if (visibilityData) {
                const currentUserInVisibility = visibilityData.active.find((u: User) => 
                  String(u.id || (u as LegacyUser).userId) === String(legacyContext.currentUser?.id)
                );
          ...
          const avatarHTML = await legacyContext.AvatarUtils.createUnifiedAvatar(userDataForAvatar, 'profile', {
            showAura: true,
  ...
  async createUserMenu() {
    ...
    const currentUser = ensureLegacyUser(legacyContext.currentUser);

    // Create small avatar using AvatarUtils if available
    let smallAvatarHTML = '';
    if (legacyContext.AvatarUtils && currentUser) {
      try {
        smallAvatarHTML = await legacyContext.AvatarUtils.createUnifiedAvatar(
          currentUser as User,
          'menu',
          {
            showAura: true,
    ...
    if (userAvatarContainer) {
      ...
      if (legacyContext.getComputedStyle(userAvatarContainer).position === 'static') {
    ...
    if (legacyContext.EventBus && typeof legacyContext.EventBus === 'object') {
      const eventBus = legacyContext.EventBus as { on?: (event: string, handler: (data: { key?: string; value?: unknown }) => void) => void };
  ...
  addUserMenuEventListeners() {
    ...
    if (visibilitySettingsBtn) {
      ...
      this.visibilitySettingsHandler = (e: Event) => {
        ...
        setTimeout(async () => {
          const visibilitySettingsManager = legacyContext.visibilitySettingsManager as { ensureEventListeners?: () => Promise<void> } | undefined;
  ...
  async toggleTheme() {
    ...
    const currentThemeGetter = typeof legacyContext.getCurrentUserTheme === 'function'
      ? legacyContext.getCurrentUserTheme as () => Promise<'light' | 'dark' | 'auto'>
      : undefined;
    const currentTheme = document.body.getAttribute('data-theme') || 
                        document.documentElement.getAttribute('data-theme') || 
    ...
    if (userPreferencesManager && userPreferencesManager.isInitialized && typeof userPreferencesManager.savePreference === 'function') {
    ...
    } else if (typeof legacyContext.updateThemeEverywhere === 'function') {
      ...
      await legacyContext.updateThemeEverywhere(newTheme);
    } else if (typeof legacyContext.toggleTheme === 'function') {
      console.log('🔧 PROFILE MANAGER: Using legacyContext.toggleTheme');
      await legacyContext.toggleTheme();
      ...
      if (legacyContext.currentUser) {
        legacyContext.currentUser.theme = newTheme;
      }
   ...
   */
  async handleUserUpdate(user: LegacyUser) {
    ...
    if (legacyContext.syncStorageWithDatabase && typeof legacyContext.syncStorageWithDatabase === 'function') {
      legacyContext.syncStorageWithDatabase().catch((err: unknown) => {
    ...
    // Second priority: from legacyContext.currentUser (should have been set during auth)
    else if (legacyContext.currentUser?.auraColor) {
      if (this.profileData) {
        this.profileData.auraColor = legacyContext.currentUser.auraColor;
   ...
   */
  async handleAuraColorUpdate(auraData: { auraColor?: string; color?: string; userId?: string; user?: LegacyUser; [key: string]: unknown }) {
   ...
   */
  async handleAuthUIUpdate(authData: { isAuthenticated?: boolean; user?: LegacyUser; [key: string]: unknown }) {
    ...
    if (authData.isAuthenticated && authData.user) {
      ...
      // Second priority: from legacyContext.currentUser (set by AuthModule)
      else if (legacyContext.currentUser?.auraColor && this.profileData) {
        this.profileData.auraColor = legacyContext.currentUser.auraColor;
        console.log('✅ PROFILE_MANAGER: Using auraColor from legacyContext.currentUser:', legacyContext.currentUser.auraColor);
      }
      // Third priority: fetch from API if user ID is available
  ...
  async updateUserAvatar() {
    ...
    // Fallback to legacyContext.currentUser if pre-render data not available
    if (!auraColorValue && legacyContext.currentUser?.auraColor) {
      auraColorValue = legacyContext.currentUser.auraColor;
      console.log('✅ PROFILE_MANAGER: Using auraColor from legacyContext.currentUser:', auraColorValue);
    }
    ...
    try {
      ...
      if (legacyContext.AvatarUtils) {
        ...
        // ROOT CAUSE FIX: Ensure availability is set for status dots
        // CRITICAL: Prioritize legacyContext.currentUser.availability/globalAvailability (most up-to-date)
        if (legacyContext.currentUser && String(legacyContext.currentUser.id || (legacyContext.currentUser as LegacyUser).userId) === String(userForAvatar.id || (userForAvatar as LegacyUser).userId)) {
          // Current user: use legacyContext.currentUser.availability/globalAvailability (most up-to-date from database)
          userForAvatar.availability = legacyContext.currentUser.availability || legacyContext.currentUser.globalAvailability || userForAvatar.availability;
        ...
        if (!userForAvatar.availability) {
          ...
          // Default to AVAILABLE if active, OFFLINE if not
          if (!userForAvatar.availability) {
            userForAvatar.availability = (userForAvatar.isActive || (userForAvatar as LegacyUser).is_active) ? 'AVAILABLE' : 'OFFLINE';
          }
        }
        
        const avatarHTML = await legacyContext.AvatarUtils.createUnifiedAvatar(userForAvatar, 'profile', {
          showAura: true,
          showStatus: true,  // ROOT CAUSE FIX: Enable status dots on profile avatar
          size: 32
        });
  ...
  refreshProfileAvatar() {
    ...
    if (visibilityData?.active && this.profileData) {
      const userInVisibility = visibilityData.active.find(
        (u: User) => u.email === this.profileData!.email || (u as LegacyUser).userId === this.profileData!.email
      );
   ...
   */
  onProfileUpdate(callback: (user: LegacyUser | null) => void) {
...
async function getCurrentUserAuraColor() {
  ...
  if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
    try {
      ...
      const cachedAuraColor = storageResult.userAuraColor || storageResult.auraColor;
      if (cachedAuraColor && cachedAuraColor !== '#ffffff' && cachedAuraColor !== 'ffffff' && cachedAuraColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
  ...
  if (legacyContext.currentUser && legacyContext.currentUser.auraColor && legacyContext.currentUser.auraColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
    console.log(`✅ AURA_MODAL: Found database auraColor in currentUser: ${legacyContext.currentUser.auraColor}`);
  ...
  if (visibilityDataFiltered?.active) {
    ...
    if (currentUserEmail) {
      const userData = visibilityDataFiltered.active.find((u: User) => u.email === currentUserEmail);
  ...
  if (visibilityData?.active) {
    const currentUserEmail = legacyContext.currentUser?.email;
    if (currentUserEmail) {
      const userData = visibilityData.active.find((u: User) => u.email === currentUserEmail);
      if (userData && userData.auraColor && userData.auraColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
  ...
  return legacyContext.AVATAR_FALLBACK_COLOR;
}
...
function getCurrentUserAvatarBgColor() {
  ...
  if (visibilityDataFiltered2?.active) {
    const currentUserEmail = legacyContext.currentUser?.email;
    if (currentUserEmail) {
      const userData = visibilityDataFiltered2.active.find((u: User) => u.email === currentUserEmail);
      if (userData && userData.auraColor && userData.auraColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
  ...
  if (typeof legacyContext.getState === 'function') {
    try {
      const storedColor = legacyContext.getState('userAvatarBgColor');
      if (storedColor && storedColor !== legacyContext.AVATAR_FALLBACK_COLOR) {
...
function getCurrentUserAvatarColor() {
  ...
  return legacyContext.AVATAR_FALLBACK_COLOR; // Default white
}
...
function setCustomAvatarColor(color: string) {
  if (legacyContext.currentUser) {
    ...
    if (legacyContext.ProfileManager && typeof legacyContext.ProfileManager === 'object' && 'updateProfile' in legacyContext.ProfileManager && typeof (legacyContext.ProfileManager as { updateProfile: unknown }).updateProfile === 'function') {
      (legacyContext.ProfileManager as { updateProfile: (user: LegacyUser) => void }).updateProfile(legacyContext.currentUser);
    }
  }
}
...
function addAllProfileMenuHandlers() {
  ...
  const profileManager = legacyContext.profileManager || (legacyContext.ProfileManager && typeof legacyContext.ProfileManager === 'object' && 'instance' in legacyContext.ProfileManager ? (legacyContext.ProfileManager as { instance?: ProfileManager }).instance : undefined);
  if (profileManager) {
...
async function updateAvailabilityEverywhere(availability: string) {
  ...
  try {
    ...
    // Step 3: Update local user object immediately
    if (legacyContext.currentUser) {
      legacyContext.currentUser.availability = availability;
      legacyContext.currentUser.globalAvailability = availability;
    ...
    const profileManagerInstance = (legacyContext.profileManager || (legacyContext.ProfileManager && (legacyContext.ProfileManager as { instance?: ProfileManager }).instance)) as ProfileManager | undefined;
...
async function updateThemeEverywhere(theme: string) {
  ...
  try {
    ...
    if (themeToggle) {
      ...
      if (legacyContext.visibilitySettingsManager && typeof legacyContext.visibilitySettingsManager === 'object' && 'updateThemeStatus' in legacyContext.visibilitySettingsManager && typeof (legacyContext.visibilitySettingsManager as { updateThemeStatus: unknown }).updateThemeStatus === 'function') {
        (legacyContext.visibilitySettingsManager as { updateThemeStatus: () => void }).updateThemeStatus();
      }
...
async function getCurrentUserAvailability() {
  ...
  // Fallback 1: Try to get from current user object (from database)
  if (legacyContext.currentUser) {
    const userAvailability = legacyContext.currentUser.availability || legacyContext.currentUser.globalAvailability;
...
legacyContext.ProfileManager = ProfileManager;
legacyContext.profileManager = profileManagerInstance;
legacyContext.getCurrentUserAuraColor = getCurrentUserAuraColor;
legacyContext.updateAuraColorEverywhere = updateAuraColorEverywhere;
legacyContext.getCurrentUserTheme = getCurrentUserTheme;
legacyContext.updateThemeEverywhere = updateThemeEverywhere;
legacyContext.getCurrentUserAvailability = getCurrentUserAvailability;
legacyContext.updateAvailabilityEverywhere = updateAvailabilityEverywhere;
legacyContext.getCurrentUserAvatarBgColor = getCurrentUserAvatarBgColor;
legacyContext.getCurrentUserAvatarColor = getCurrentUserAvatarColor;
legacyContext.setCustomAvatarColor = setCustomAvatarColor;
legacyContext.resetCustomAvatarColor = resetCustomAvatarColor;
legacyContext.handleAvatarClick = handleAvatarClick;
legacyContext.handleClickOutside = handleClickOutside;
legacyContext.addProfileAvatarClickHandler = addProfileAvatarClickHandler;
legacyContext.addAuraButtonClickHandler = addAuraButtonClickHandler;
legacyContext.addLogoutButtonClickHandler = addLogoutButtonClickHandler;
legacyContext.addAllProfileMenuHandlers = addAllProfileMenuHandlers;
legacyContext.showColorPickerModal = showColorPickerModal;
legacyContext.closeColorPickerModal = closeColorPickerModal;
legacyContext.updateColorPreview = updateColorPreview;
legacyContext.isValidHex = isValidHex;
legacyContext.getAvatarColor = getAvatarColor;
