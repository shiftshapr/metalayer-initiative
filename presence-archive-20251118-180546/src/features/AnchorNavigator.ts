...
export class AnchorNavigator {
  ...
  async navigateToUrl(
    ...
    try {
      this.logger.info('Navigating to URL', url as unknown as LogData);
      ...
      this.logger.error('Error navigating to URL', error as unknown as LogData);
      return null;
    }
  }
    ...
    return new Promise((resolve) => {
      ...
      const checkStatus = async () => {
        try {
          ...
          this.logger.error('Error checking tab status', error as unknown as LogData);
          resolve(false);
        }
      };
  ...
  async waitForElement(
    ...
    return new Promise((resolve) => {
      ...
      const checkElement = () => {
        ...
        if (element) {
          this.logger.info('Element found', selector as unknown as LogData);
          resolve(element);
          return;
        }
        
        if (Date.now() - startTime > timeout) {
          this.logger.warn('Element not found (timeout)', selector as unknown as LogData);
          resolve(null);
          return;
        }
  ...
  async findElement(anchor: NotificationAnchor): Promise<HTMLElement | null> {
    try {
      ...
      this.logger.error('Error finding element', error as unknown as LogData);
      return null;
    }
  }
  ...
  calculateScrollOffset(): number {
    try {
      ...
      this.logger.debug('Error calculating scroll offset', error as unknown as LogData);
      return 100;
    }
  }
