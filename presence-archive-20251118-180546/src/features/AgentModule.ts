...
const getAgentApiUrl = (): string => {
  if (typeof window !== 'undefined') {
    const configManager = (window as Window & { configManager?: { get?: (key: string) => any } }).configManager;
    if (configManager && typeof configManager.get === 'function') {
      const apiUrl = configManager.get('apiUrl');
      if (apiUrl) return `${apiUrl}/api/agent`;
    }
  }
  if (typeof window !== 'undefined' && (window as Window & { METALAYER_API_URL?: string }).METALAYER_API_URL) {
    return `${(window as Window & { METALAYER_API_URL?: string }).METALAYER_API_URL}/api/agent`;
  }
...
async function handleYouTubeVideoRequest(message: string, videoData: VideoData): Promise<string> {
  try {
    ...
    const youtubeService = (window as Window & { youtubeService?: { processYouTubeVideo?: (videoData: any) => Promise<any> } }).youtubeService;
    const processedVideo = youtubeService ? await youtubeService.processYouTubeVideo(videoData) : null;
      ...
      setTimeout(() => {
        chrome.tabs.sendMessage(tab.id!, { action: 'extractPageContent' }, (response) => {
          ...
          if (response && (response as any).content) {
            console.log('Page content loaded successfully:', (response as any).content.title);
            // Check if content has changed (different hash)
            const newHash = (response as any).content.contentHash;
            if (contentHash !== newHash) {
              contentHash = newHash;
              pageContentCache = (response as any).content;
              cachedChunks = (response as any).content.content?.chunks || [];
              
              // Update agent welcome message with page info
              updateAgentWelcomeWithPageInfo((response as any).content);
            }
          } else {
...
function debugAgentTab(): void {
  ...
  console.log("YouTube service:", typeof (window as Window & { youtubeService?: any }).youtubeService, 'general');
...
(window as Window & { debugAgentTab?: () => void }).debugAgentTab = debugAgentTab;

// Alternative simple debug function
(window as Window & { debugAgent?: () => void }).debugAgent = function(): void {
...
(window as Window & { AgentModule?: any }).AgentModule = AgentModule;
(window as Window & { initializeAgentTab?: () => void }).initializeAgentTab = initializeAgentTab;
