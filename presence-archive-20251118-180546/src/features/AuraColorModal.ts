...
interface AurasIntegration {
  ...
  setAura(userId: string, color: string): void;
}

interface SupabaseRealtimeClient {
  channels?: Map<string, any>;
  broadcastAuraColorChange(color: string): Promise<void>;
}

// These will be injected or imported when available
let handleAuraChange: ((data: { userId: string; auraColor: string; source?: string }) => Promise<void>) | undefined;
let aurasIntegration: AurasIntegration | undefined;
let supabaseRealtimeClient: SupabaseRealtimeClient | undefined;
let refreshVisibilityAvatars: (() => Promise<void>) | undefined;
let refreshAllMessageAvatars: (() => Promise<void>) | undefined;
let normalizeCurrentUrl: (() => Promise<{ pageId?: string; normalizedUrl?: string; normalized?: string }>) | undefined;
let sendSupabaseMessage: ((data: any) => Promise<void>) | undefined;
...
export async function getCurrentUserAvatarBgColor(): Promise<string> {
  ...
  const api = stateManagerInstance.getState('api') as any | undefined;
  if (currentUser && api) {
  ...
  const currentVisibilityDataUnfiltered = stateManagerInstance.getState('currentVisibilityDataUnfiltered') as { active?: Array<{ id?: string; userId?: string; auraColor?: string }> } | undefined;
  if (currentVisibilityDataUnfiltered?.active) {
    const userId = currentUser?.id;
    if (userId) {
      const userInVisibility = currentVisibilityDataUnfiltered.active.find(u => 
...
export async function showColorPickerModal(): Promise<void> {
  ...
  setTimeout(async () => {
    ...
    resetBtn.addEventListener('click', () => {
      ...
      const user = stateManagerInstance.getState("currentUser") as User | undefined;
      if (user) {
        const name = (user.userMetadata as any)?.full_name || user.name || 'User';
        ...
        try {
          ...
          document.dispatchEvent(new CustomEvent('auraColorUpdated', {
            detail: {
              ...
              user: stateManagerInstance.getState("currentUser") as User | undefined,
              userId: userId
            }
          }));
...
export function closeColorPickerModal(): void {
...
export function updateColorPreview(hex: string): void {
...
export async function setCustomAvatarColor(color: string): Promise<void> {
...
export async function resetUserAvatarBgColor(): Promise<void> {
...
export async function broadcastAuraColorChange(color: string): Promise<void> {
  if (supabaseRealtimeClient) {
    await supabaseRealtimeClient.broadcastAuraColorChange(color);
...
export async function updateUserAuraInUI(userId: string, auraColor: string, skipVisibilityRefresh: boolean = false): Promise<void> {
  try {
    const currentUser = stateManagerInstance.getState('currentUser') as User | undefined;
    ...
    if (currentVisibilityDataUnfiltered?.active) {
      ...
      if (userInVisibility) {
        (userInVisibility as any).auraColor = auraColor;
        console.log('✅ AURA_UI_UPDATE: Updated aura color in visibility data');
      }
    }
    
    const currentVisibilityData = stateManagerInstance.getState('currentVisibilityData') as { active?: Array<{ id?: string; userId?: string; auraColor?: string }> } | undefined;
    if (currentVisibilityData?.active) {
      const userInVisibility = currentVisibilityData.active.find(u => 
        String(u.id || u.userId) === String(userId)
      );
      if (userInVisibility) {
        (userInVisibility as any).auraColor = auraColor;
    ...
    if (currentUser && String(currentUser.id) === String(userId)) {
      ...
      if (profileAvatarContainer) {
        ...
        let profileAvatarUpdateInProgress = stateManagerInstance.getState('profileAvatarUpdateInProgress') as boolean | undefined;
...
function getLatestAuraColorFromPresence(userId: string): string | null {
  try {
    // Check if we have presence data stored
    const currentPresenceData = stateManagerInstance.getState('currentPresenceData') as { active?: Array<{ id?: string; userId?: string; email?: string; auraColor?: string }> } | undefined;
    const presenceData = stateManagerInstance.getState('presenceData') as { active?: Array<{ id?: string; userId?: string; email?: string; auraColor?: string }> } | undefined;
    ...
    const visibilityData = stateManagerInstance.getState('currentVisibilityData') as { active?: Array<{ id?: string; userId?: string; auraColor?: string }> } | undefined;
...
function updateAllMessageAvatars(userId: string, auraColor: string): void {
  ...
  allMessages.forEach((message: Element) => {
    const messageEl = message as HTMLElement;
    const authorId = messageEl.dataset.authorId || ((message.querySelector('[data-user-id]') as HTMLElement)?.dataset.userId);
    if (authorId === userId) {
...
export function updateAvatarAura(avatar: Element, auraColor: string): void {
  ...
  const auraElement = avatar.querySelector('.aura-effect') as HTMLElement | null;
  if (auraElement) {
...
export function updateAllVisibilityAvatars(userId: string, auraColor: string): void {
  ...
  visibilityContainers.forEach((container: Element) => {
...
export async function broadcastAuraChange(auraColor: string): Promise<void> {
  try {
      ...
      try {
        ...
        if (supabase.realtime && supabase.realtime.channels) {
          ...
          channel = Array.isArray(channels) 
            ? channels.find((ch: any) => ch.topic === channelName)
            : Object.values(channels).find((ch: any) => {
