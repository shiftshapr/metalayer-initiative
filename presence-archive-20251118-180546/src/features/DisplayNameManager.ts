...
interface UserPreferencesManager {
  ...
  savePreference(key: string, value: string | null, options?: { batch?: boolean }): Promise<void>;
}
...
class DisplayNameManager {
  ...
  async readDisplayName(): Promise<void> {
    try {
      ...
      const userPreferencesManager = (window as Window & { userPreferencesManager?: UserPreferencesManager }).userPreferencesManager;
      ...
      if (storageData.displayName) {
        this.currentDisplayName = storageData.displayName as string;
        this.originalDisplayName = storageData.displayName as string;
        if (this.displayNameInput) {
          this.displayNameInput.value = this.currentDisplayName;
        }
        this.updateCharCount();
      ...
      if (window.currentUser && (window.currentUser as any).id) {
        try {
          ...
          const response = await fetch(`http://216.238.91.120:3002/v1/users/${(window.currentUser as any).id}`, {
            method: 'GET',
            headers: {
  ...
  async saveDisplayName(): Promise<void> {
    try {
      ...
      if (window.currentUser) {
        (window.currentUser as any).displayName = newDisplayName;
        (window.currentUser as any).name = newDisplayName || (window.currentUser as any).name;
      }
  ...
  async deleteDisplayName(): Promise<void> {
    try {
      ...
      // Update window.currentUser
      if (window.currentUser) {
        (window.currentUser as any).displayName = null;
      }
  ...
  private async getAuthToken(): Promise<string> {
    try {
      ...
      return (authData.authToken || authData.googleAccessToken || '') as string;
    } catch (error) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    (window as Window & { displayNameManager?: { initialize: () => Promise<void> } }).displayNameManager = new DisplayNameManager();
    (window as Window & { displayNameManager?: { initialize: () => Promise<void> } }).displayNameManager?.initialize();
  });
} else {
  (window as Window & { displayNameManager?: { initialize: () => Promise<void> } }).displayNameManager = new DisplayNameManager();
  (window as Window & { displayNameManager?: { initialize: () => Promise<void> } }).displayNameManager?.initialize();
...
(window as Window & { DisplayNameManager?: any }).DisplayNameManager = DisplayNameManager;
