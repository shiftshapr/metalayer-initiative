 ...
 */
export async function initializePeopleTab(): Promise<void> {
  ...
  try {
    ...
    const displayUsers = users?.filter((user: UserData) => {
        ...
        if (userEmails.length > 0) {
          ...
          try {
            ...
            if (presenceError) {
              ...
              presenceData.forEach((p: PresenceData) => {
                if (p.is_active) {
                  const user = displayUsers.find((u: UserData) => u.email === p.user_email);
                  if (user) {
    ...
    const usersHTML = displayUsers.map((user: UserData) => {
