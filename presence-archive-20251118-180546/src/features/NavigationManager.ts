...
// Chrome extension APIs are available globally in extension context
...
class NavigationManager {
  ...
  async navigateToUrl(url: string, target?: NavigationTarget | string): Promise<void> {
    try {
  ...
  async focusAndHighlight(target?: NavigationTarget | string): Promise<void> {
    try {
      if (!target) return;
...
if (typeof window !== 'undefined') {
  (window as typeof window & { navigationManager?: NavigationManager }).navigationManager = navigationManager;
}
