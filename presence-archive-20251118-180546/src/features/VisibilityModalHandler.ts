...
declare const window: Window & {
  ...
  userPreferencesManager?: {
    ...
    getPreference: (key: string) => Promise<boolean>;
    savePreference: (key: string, value: boolean) => Promise<void>;
  };
  unifiedSettingsStorage?: {
    getSetting: (key: string, defaultValue: boolean, options?: any) => Promise<boolean>;
  };
  visibilitySettingsManager?: {
    saveVisibility: () => Promise<void>;
  };
  saveSetting?: (key: string, value: any, options?: any) => Promise<void>;
  visibilityModalHandler?: VisibilityModalHandler;
  VisibilityModalHandler?: typeof VisibilityModalHandler;
};
...
class VisibilityModalHandler {
  ...
  interceptVisibilityTabClicks(): void {
    ...
    document.addEventListener('click', async (e: MouseEvent) => {
      const visibilityTabBtn = (e.target as HTMLElement).closest('[data-tab="visibility-tab"]') as HTMLElement | null;
      if (visibilityTabBtn) {
   ...
   */
  async handleGoVisible(): Promise<void> {
    try {
      ...
      const visibilityToggleEl = document.getElementById('visibility-toggle') as HTMLInputElement | null;
  ...
  async getAuthToken(): Promise<string> {
    try {
      const authData = await chrome.storage.local.get(['authToken', 'googleAccessToken']) as { authToken?: string; googleAccessToken?: string };
  ...
  switchToVisibilityTab(): void {
    ...
    allTabButtons.forEach((btn: Element) => {
