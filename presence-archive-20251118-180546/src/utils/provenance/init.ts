...
(async function initializeProvenance(): Promise<void> {
  'use strict';
  ...
  try {
    ...
    const chrome = (window as Window & { chrome?: { runtime?: { getURL: (path: string) => string } } }).chrome;
    ...
    // Initialize service (using proper typing - no (window as any) - red-line compliance)
    ...
    const service = ((window as unknown) as ProvenanceWindow).provenanceService;
    if (service) {
      await service.initialize();
      
      // Initialize diagnostic and link injector if service is enabled
      if (service.isEnabled) {
        const diagnostic = ((window as unknown) as ProvenanceWindow).provenanceDiagnostic;
        if (diagnostic) {
          await diagnostic.initialize();
        }
        
        const injector = ((window as unknown) as ProvenanceWindow).provenanceLinkInjector;
        if (injector) {
        ...
        document.addEventListener('keydown', (e) => {
          if (e.ctrlKey && e.shiftKey && e.key === 'P') {
            e.preventDefault();
            const diag = ((window as unknown) as ProvenanceWindow).provenanceDiagnostic;
            if (diag) {
              diag.toggle();
            }
          }
