...
export async function diagnoseReplyDisplay(messageId: string, pageId: string, communityId: string): Promise<DiagnosticResults> {
  ...
  try {
    ...
    const query1 = (window.supabase
      .from('messages')
      .select('*')
      .eq('parent_id', messageId) as any)
      .is('deleted_at', null);
    const { data: allReplies, error: allError } = await query1 as { data: unknown[] | null; error: { message: string; code?: string; details?: string; hint?: string } | null };
    ...
    const query2 = (window.supabase
      .from('messages')
      .select('*')
      .eq('parent_id', messageId) as any)
      .eq('page_id', normalizedPageId)
      .is('deleted_at', null);
    const { data: normalizedReplies, error: normalizedError } = await query2 as { data: unknown[] | null; error: { message: string } | null };
    ...
    if (normalizedPageId !== pageId) {
      ...
      const query3 = (window.supabase
        .from('messages')
        .select('*')
        .eq('parent_id', messageId) as any)
        .eq('page_id', pageId)
        .is('deleted_at', null);
      const { data: originalReplies, error: originalError } = await query3 as { data: unknown[] | null; error: { message: string } | null };
    ...
    const query4 = (window.supabase
      .from('messages')
      .select('*')
      .eq('parent_id', messageId) as any)
      .eq('community_id', communityId)
      .is('deleted_at', null);
    const { data: communityReplies, error: communityError } = await query4 as { data: unknown[] | null; error: { message: string } | null };
    ...
    const query5 = (window.supabase
      ...
      .eq('community_id', communityId)
      .is('deleted_at', null);
    const { data: actualQueryReplies, error: actualError } = await query5 as { data: unknown[] | null; error: { message: string } | null };
