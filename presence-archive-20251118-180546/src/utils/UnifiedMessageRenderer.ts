...
declare const window: Window & {
  getMessageActionsMenu?: (message: Message, canEdit: boolean, canDelete: boolean) => string;
...
export class UnifiedMessageRenderer {
   ...
   */
  static async generateMessageHTML(message: Message, options: MessageRenderOptions = {}): Promise<string> {
    const {
      isReply = false,
      isFocusMode = false,
      author = null,
   ...
   */
  static async renderMessage(message: Message, options: MessageRenderOptions = {}): Promise<HTMLElement> {
    const {
      isReply = false,
      isFocusMode = false,
      author = null,
    ...
    if (isReply) {
      ...
      const hasReplies = replyCount > 0 || (message as any).hasReplies;
      if (hasReplies) {
   ...
   * Format message time
   */
  static formatMessageTime(timestamp: string | Date | undefined, isFocusMode: boolean = false): string {
    if (!timestamp) return '';
...
if (typeof window !== 'undefined') {
  (window as typeof window & { UnifiedMessageRenderer: typeof UnifiedMessageRenderer }).UnifiedMessageRenderer = UnifiedMessageRenderer;
