...
declare const window: Window & {
  addMessageToChat?: (message: Message & { _focusModeTarget?: HTMLElement | null }) => Promise<void>;
  addMessageActionListeners?: (element: HTMLElement, message: Message) => void;
  MessageRenderer?: typeof MessageRenderer;
};
...
export class MessageRenderer {
   ...
   */
  static async renderMessage(message: Message, options: MessageRenderOptions = {}): Promise<HTMLElement | null> {
    const {
      isReply = false,
      isFocusMode = false,
      container = null
    ...
    if (window.addMessageToChat && typeof window.addMessageToChat === 'function') {
      ...
      if (isFocusMode && container) {
        (message as any)._focusModeTarget = container;
      }
      
      // Call existing function - it already handles all rendering, styling, and event handlers
      await window.addMessageToChat(message as Message & { _focusModeTarget?: HTMLElement | null });
      ...
      if (renderedElement) {
        ...
        return renderedElement as HTMLElement;
      } else {
...
if (typeof window !== 'undefined') {
  (window as typeof window & { MessageRenderer: typeof MessageRenderer }).MessageRenderer = MessageRenderer;
