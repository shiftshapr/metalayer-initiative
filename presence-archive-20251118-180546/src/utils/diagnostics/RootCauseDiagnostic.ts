...
import { RootCauseDiagnosticResult, RootCauseSummary } from './types.js';

type MaybePromise<T> = T | Promise<T>;

const addFinding = (bucket: RootCauseSummary[], finding?: RootCauseSummary | null): void => {
  if (finding) {
    bucket.push(finding);
  }
};

const resolveState = async <T = unknown>(value: MaybePromise<T> | undefined | null): Promise<T | null> => {
  if (typeof value === 'undefined' || value === null) {
    return null;
  }
  return typeof (value as Promise<T>).then === 'function' ? ((await value) as T) : (value as T);
};

const getGlobal = () =>
  window as typeof window & {
    stateManager?: { getState: (key: string) => MaybePromise<unknown> };
    getState?: (key: string) => unknown;
    activeCommunities?: string[];
    api?: { baseURL?: string };
    loadChatHistory?: () => MaybePromise<unknown>;
    CanopiModule?: unknown;
    rootCauseDiagnosticResults?: RootCauseDiagnosticResult;
  };

const checkModuleAvailability = (): RootCauseSummary | null => {
  const global = getGlobal();
  const scripts = Array.from(document.querySelectorAll<HTMLScriptElement>('script[src*="CanopiModule"]'));
...
const checkActiveCommunities = async (): Promise<RootCauseSummary | null> => {
  const global = getGlobal();
  ...
  const candidate = Object.values(stateChecks).find(value => Array.isArray(value) && value.length > 0) as
    | string[]
    | undefined;
...
const gatherMessages = (): HTMLElement[] =>
  Array.from(document.querySelectorAll<HTMLElement>('.message[data-message-id]'));

const checkMessageOrder = (messages: HTMLElement[]): RootCauseSummary | null => {
...
const checkMessageTimeDisplay = (messages: HTMLElement[]): RootCauseSummary | null => {
...
const checkAvatarRendering = (messages: HTMLElement[]): RootCauseSummary | null => {
...
const checkActionButtons = (messages: HTMLElement[]): RootCauseSummary | null => {
...
const checkTypeScriptBuild = async (): Promise<RootCauseSummary | null> => {
  try {
...
const checkApiEndpoints = (): RootCauseSummary | null => {
...
export const runRootCauseDiagnostic = async (): Promise<RootCauseDiagnosticResult> => {
  ...
  const result: RootCauseDiagnosticResult = {
    timestamp,
    issues: [],
    rootCauses: [],
    recommendations: [],
