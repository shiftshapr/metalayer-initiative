...
type LoadChatHistoryFn = (communityIdOrRawUrl?: string | null, activeCommunities?: string[]) => Promise<void>;
type AddMessageFn = (message: Partial<Message> | Message) => Promise<void> | void;
...
const wrapLoadChatHistory = (domWindow: ChatWindow, doc: Document): void => {
  ...
  domWindow.loadChatHistory = async function patchedLoadChatHistory(this: unknown, ...args: Parameters<LoadChatHistoryFn>) {
...
const wrapAddMessageToChat = (domWindow: ChatWindow, doc: Document): void => {
  ...
  domWindow.addMessageToChat = async function patchedAddMessageToChat(this: unknown, message: Partial<Message> | Message) {
    const result = await originalAddMessageToChat.apply(this, [message] as Parameters<AddMessageFn>);
...
const ensureReplyVisible = (doc: Document, message: Partial<Message> | Message): void => {
  const messageWithMeta = message as Partial<Message> & { isReply?: boolean; messageId?: string };
  if (!messageWithMeta || !messageWithMeta.isReply) return;
  const chatMessages = getChatMessagesContainer(doc);
...
export const applyChatLoadingOverlayPatch = (domWindow: ChatWindow = window as ChatWindow): void => {
  ...
  if ((domWindow as ChatWindow & { __chatLoadingOverlayPatched?: boolean })[PATCH_FLAG]) {
    return;
  }
  (domWindow as ChatWindow & { __chatLoadingOverlayPatched?: boolean })[PATCH_FLAG] = true;
