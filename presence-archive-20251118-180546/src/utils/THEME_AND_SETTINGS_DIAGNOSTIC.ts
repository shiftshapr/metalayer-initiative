...
type WindowWithDiagnostics = Window & {
  ...
  runThemeAndSettingsDiagnostic?: () => Promise<DiagnosticResults>;
}
 ...
 */
function testProfileMenuThemeToggle(): void {
  ...
  const hasClickHandler = (themeToggleBtn as HTMLElement).onclick !== null || 
  ...
  if (typeof win.updateThemeEverywhere === 'function') {
 ...
 */
function testSettingsTabThemeToggle(): void {
  ...
  const win = window as unknown as WindowWithDiagnostics;
  if (win.visibilitySettingsManager && win.visibilitySettingsManager.isInitialized) {
 ...
 */
function testVisibilityToggle(): void {
  ...
  const win = window as unknown as WindowWithDiagnostics;
  if (win.visibilityModalHandler && win.visibilityModalHandler.isInitialized) {
 ...
 */
function testRepliesInFocusMode(): void {
  console.log('🔍 DIAGNOSTIC: Testing Replies in Focus Mode...');
  
  // Check if ReplyLoader is available
  const win = window as unknown as WindowWithDiagnostics;
  if (win.ReplyLoader && typeof win.ReplyLoader.loadAllReplies === 'function') {
  ...
  const hiddenReplies = Array.from(focusReplies).filter(reply => {
    const element = reply as HTMLElement;
    return element.style.display === 'none' || 
           (element.dataset.pendingVisibility === 'true') ||
  ...
  if (hiddenReplies.length > 0) {
    ...
    hiddenReplies.forEach((reply, idx) => {
      const element = reply as HTMLElement;
      const msgId = element.dataset.messageId || 'unknown';
      diagnosticResults.repliesInFocusMode.details.push(`  - Reply ${idx + 1}: ${msgId} (display: ${element.style.display}, width: ${element.offsetWidth}, height: ${element.offsetHeight})`);
    });
  } else {
 ...
 */
async function testHeadlineLoading(): Promise<void> {
  ...
  const win = window as unknown as WindowWithDiagnostics;
  if (win.settingsHeadlineManager) {
    if (win.settingsHeadlineManager.isInitialized) {
    ...
    const currentHeadline = win.settingsHeadlineManager.currentHeadline || '';
  ...
  try {
    ...
    diagnosticResults.headlineLoading.details.push(`❌ Error reading Chrome storage: ${err.message}`);
  }
 ...
 */
async function testDisplayNameLoading(): Promise<void> {
  ...
  const win = window as unknown as WindowWithDiagnostics;
  if (win.displayNameManager) {
    if (win.displayNameManager.isInitialized) {
    ...
    const currentDisplayName = win.displayNameManager.currentDisplayName || '';
  ...
  try {
    ...
    const err = error as Error;
    diagnosticResults.displayNameLoading.details.push(`❌ Error reading Chrome storage: ${err.message}`);
  }
 ...
 * Run all diagnostics
 */
export async function runThemeAndSettingsDiagnostic(): Promise<DiagnosticResults> {
  console.log('═══════════════════════════════════════════════════════════');
  ...
  Object.keys(diagnosticResults).forEach(key => {
    const result = diagnosticResults[key as keyof DiagnosticResults];
...
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      setTimeout(runThemeAndSettingsDiagnostic, 2000);
    });
  } else {
    setTimeout(runThemeAndSettingsDiagnostic, 2000);
  }
