...
declare const window: Window & {
  ...
  emergencyMessageFix?: () => boolean;
};

/**
 * Emergency fix function to force messages to show
 */
export function emergencyMessageFix(): boolean {
  ...
  allOverlays.forEach(overlay => {
    ...
    (overlay as HTMLElement).style.setProperty('visibility', 'hidden', 'important');
    (overlay as HTMLElement).style.setProperty('opacity', '0', 'important');
    overlay.remove();
  });
  ...
  const chatMessagesElement = chatMessages as HTMLElement;
