    ...
    if (win.api && typeof win.api.getChatHistory === 'function') {
      ...
      for (const communityId of activeCommunities) {
        try {
          ...
          const response = await win.api.getChatHistory(communityId, null, urlData.normalizedUrl || '');
    ...
    if (win.supabase) {
      ...
      try {
        ...
        for (const communityId of activeCommunities) {
          ...
          const { data: messages, error } = await win.supabase
            .from('messages')
            .select('*')
            ...
            dbResults.push({
              ...
              messages: messages?.map((m: unknown) => {
                ...
                return {
                  ...
                  parentId: dbMsg.parent_id || dbMsg.parentId || null,
                  createdAt: dbMsg.createdAt || (dbMsg as any).created_at, // Normalize from database (boundary case)
                  pageId: dbMsg.page_id || dbMsg.pageId
                };
              }) || []
            });
