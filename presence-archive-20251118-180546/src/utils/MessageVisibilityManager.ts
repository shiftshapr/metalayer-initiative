...
declare const window: Window & {
  MessageVisibilityManager?: typeof MessageVisibilityManager;
};
...
export class MessageVisibilityManager {
  ...
  static ensureFocusModeClasses(messageElement: HTMLElement | null): boolean {
    ...
    if (display) messageElement.style.setProperty('display', display, 'important');
    if (visibility) messageElement.style.setProperty('visibility', visibility, 'important');
    if (opacity) messageElement.style.setProperty('opacity', opacity, 'important');
    if (width) messageElement.style.setProperty('width', width, 'important');
    if (minWidth) messageElement.style.setProperty('min-width', minWidth, 'important');
    if (maxWidth) messageElement.style.setProperty('max-width', maxWidth, 'important');
    if (boxSizing) messageElement.style.setProperty('box-sizing', boxSizing, 'important');
   ...
   */
  static async fixZeroDimensions(messageElement: HTMLElement | null, messageId: string): Promise<boolean> {
    if (!messageElement) {
      return false;
    }
    ...
    const contentWrapper = messageElement.querySelector('.message-content-wrapper') as HTMLElement;
    if (contentWrapper) {
   ...
   */
  static async verifyDimensions(messageElement: HTMLElement | null, messageId: string): Promise<boolean> {
    if (!messageElement) {
      return false;
    }
   ...
   */
  static async showReply(messageElement: HTMLElement | null, messageId: string = ''): Promise<boolean> {
    if (!messageElement) {
      return false;
    }
   ...
   */
  static async fixAllZeroDimensionReplies(container: HTMLElement | null): Promise<FixStatistics> {
    if (!container) {
    ...
    const results = await Promise.allSettled(
      Array.from(replies).map(async (reply) => {
        const messageId = (reply as HTMLElement).dataset.messageId || 'unknown';
        const width = (reply as HTMLElement).offsetWidth;
        const height = (reply as HTMLElement).offsetHeight;

        if (width === 0 || height === 0) {
          const fixed = await this.fixZeroDimensions(reply as HTMLElement, messageId);
...
if (typeof window !== 'undefined') {
  (window as typeof window & { MessageVisibilityManager: typeof MessageVisibilityManager }).MessageVisibilityManager = MessageVisibilityManager;
