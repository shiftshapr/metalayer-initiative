...
export class UserPreferencesManager {
  ...
  private readonly schema: Record<PreferenceKey, PreferenceConfig>;
  ...
  async loadAllPreferences() {
    ...
    try {
      ...
      for (const [key, config] of Object.entries(this.schema)) {
        ...
        const dbValue = (dbPrefs as Record<string, any>)[key]; // dbPrefs already mapped by key
        
        // Prefer Chrome storage (cache), fallback to database (authoritative), then default
        (mergedPrefs as Record<string, any>)[key] = chromeValue !== undefined && chromeValue !== null 
          ? chromeValue 
  ...
  async loadFromDatabase() {
    ...
    try {
      ...
      const prefs: Record<string, any> = {};
      for (const [key, config] of Object.entries(this.schema)) {
        const dbValue = (response as Record<string, any>)[config.dbColumn];
   ...
   */
  async savePreference(key: PreferenceKey, value: PreferenceValue, options: SaveOptions = {}): Promise<boolean> {
    if (!this.schema[key]) {
   ...
   */
  async savePreferences(preferences: Partial<Preferences>, options: SaveOptions = {}): Promise<Record<string, boolean>> {
    console.log(`📦 USER_PREFERENCES_MANAGER: Batching ${Object.keys(preferences).length} preferences`);
    
    const results: Record<string, boolean> = {};
    const oldValues: Record<string, PreferenceValue> = {};
    ...
    for (const [key, value] of Object.entries(preferences)) {
      ...
      oldValues[key] = this.preferences[prefKey] as PreferenceValue;
    }
    
    // Step 2: Update local state immediately
    const chromeUpdates: Record<string, any> = {};
    for (const [key, value] of Object.entries(preferences)) {
      if (results[key] === false) continue;
      const prefKey = key as PreferenceKey;
      (this.preferences as any)[prefKey] = value;
    ...
    for (const [key, value] of Object.entries(preferences)) {
      if (results[key] === false) continue;
      this.emitPreferenceChanged(key as PreferenceKey, value, oldValues[key], 'user');
    }
   ...
   * Save batch to database
   */
  async saveBatchToDatabase(preferences: Record<string, any>) {
    if (!this.userId || !window.api) {
   ...
   */
  validatePreference(key: PreferenceKey | string, value: PreferenceValue) {
    const config = this.schema[key as PreferenceKey];
    if (!config) {
      return false;
    }
  ...
  async validateIntegrity() {
    ...
    for (const [key, config] of Object.entries(this.schema)) {
      const prefKey = key as PreferenceKey;
      const chromeValue = (chromePrefs as Record<string, any>)[config.chromeKey];
      const dbValue = (dbPrefs as Record<string, any>)[config.dbColumn];
