...
import { messageStore, Message, StoreStatus } from '../services/MessageStore.js';
...
export class MessageLoader {
   ...
   */
  async loadDefault(options: LoadOptions): Promise<Message[]> {
    const {
      pageId,
      limit = 10,
   ...
   */
  async loadFocus(options: LoadOptions & { focusParentId: string }): Promise<{
    parent: Message;
    replies: Message[];
  }> {
    const {
