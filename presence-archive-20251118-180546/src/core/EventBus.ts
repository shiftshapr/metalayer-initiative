...
import { EventBus as IEventBus, EventType, EventCallback, Message } from '../types/index.js';
import { StateValue } from './StateManager.js';

// Event history entry type
...
class EventBus implements IEventBus {
   ...
   */
  on(event: EventType, callback: EventCallback, options: { priority?: number; context?: unknown } = {}): string {
    if (this.isDestroyed) {
   ...
   */
  once(event: EventType, callback: EventCallback, options: { priority?: number; context?: unknown } = {}): string {
    if (this.isDestroyed) {
      console.warn('⚠️ EventBus: Cannot add listener after destruction');
      return '';
    }
    
    const onceCallback: EventCallback = (data?: unknown) => {
      callback(data);
   ...
   * Get event history
   */
  getHistory(event?: EventType): EventHistoryEntry[] {
    if (event) {
