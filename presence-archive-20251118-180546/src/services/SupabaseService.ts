...
// Declare global supabase library and config
...
class SupabaseService {
    ...
    async initialize(): Promise<SupabaseClient> {
        ...
        try {
            ...
            const authSubscription = this.client.auth.onAuthStateChange((event: string, session: any) => {
                ...
                if (session) {
                    ...
                    if (win.robustIntegration && !win.robustIntegration.isInitialized) {
                        ...
                        win.robustIntegration.initialize().then((success: boolean) => {
                            if (success) {
            ...
            // Store subscription for potential cleanup (though service is singleton)
            if (authSubscription && typeof authSubscription.unsubscribe === 'function') {
                // Subscription will remain active for the lifetime of the service
            }
