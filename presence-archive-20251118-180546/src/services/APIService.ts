...
import { User, APIRequestOptions, APIResponse } from '../types/index.js';
import type { ApiResponse } from '../types/api.js';

type LegacyApiClient = {
  ...
  async request<T = unknown>(endpoint: string, options: APIRequestOptions = {}): Promise<APIResponse<T>> {
    ...
    try {
      ...
      if (window.currentUser && window.currentUser.id) {
        ...
        if (user.auraColor && typeof user.auraColor === 'object' && typeof (user.auraColor as any).then === 'function') {
          console.log('🔍 USER_IDENTITY: ⚠️ auraColor is a Promise, resolving...');
          try {
            const resolvedAuraColor = await (user.auraColor as any);
        ...
        if (authUser && authUser.id) {
          user = { id: authUser.id, email: authUser.email } as User;
        }
...
if (typeof window !== 'undefined') {
  const win = window as Window & { api?: LegacyApiClient };
  if (!win.api) {
    const apiInstance = new MetaLayerAPI('http://216.238.91.120:3002');
    win.api = apiInstance as unknown as LegacyApiClient;
  }
}
