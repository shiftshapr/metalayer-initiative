...
class BackendHealthService {
  private status: BackendHealthStatus = 'checking';
  ...
  private readonly HEALTH_CHECK_INTERVAL_MS = 10000; // Check every 10s when offline
   ...
   */
  async waitForHealthy(options: { timeout?: number } = {}): Promise<boolean> {
   ...
   */
  private setDegraded(error: string): void {
    this.status = 'degraded';
    this.lastChecked = Date.now();
    this.lastError = error;
    this.consecutiveFailures++;
    this.retryDelayMs = Math.min(
      this.INITIAL_RETRY_DELAY_MS * Math.pow(2, this.consecutiveFailures),
  ...
  private startPeriodicChecks(): void {
    ...
    this.healthCheckInterval = setInterval(() => {
      if (this.status === 'offline' || this.status === 'degraded') {
        this.checkHealth().catch((error: unknown) => {
          handleError(error, {
            log: true,
            logLevel: 'warn',
            context: {
...
let backendHealthServiceInstance: BackendHealthService | null = null;

/**
 * Get or create BackendHealthService instance
 */
export function getBackendHealthService(baseURL?: string): BackendHealthService {
  if (!backendHealthServiceInstance) {
    ...
    backendHealthServiceInstance.checkHealth().catch((error: unknown) => {
      handleError(error, {
        log: true,
        logLevel: 'warn',
        context: {
