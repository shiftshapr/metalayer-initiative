...
// Type definitions for optional window properties (for graceful degradation)
// Note: Using type instead of interface to avoid Window extension conflicts
...
export async function updateAuraColorEverywhere(color: AuraColor): Promise<boolean> {
  ...
  try {
    ...
    let currentUser = stateManagerInstance.getState('currentUser') as User | null;
    ...
    if (!currentUser) {
      currentUser = stateManagerInstance.getState('currentUser') as User | null;
    }
    if (currentUser) {
    ...
    // ES6 pattern: Get from stateManager instead of window
    const currentVisibilityDataUnfiltered = stateManagerInstance.getState('currentVisibilityDataUnfiltered') as VisibilityData | null;
    if (currentVisibilityDataUnfiltered?.active) {
      const currentUserInVisibility = currentVisibilityDataUnfiltered.active.find(u => 
    ...
    if (currentUser?.id) {
      ...
      const win = window as Window & { handleAuraChange?: (data: { userId?: string; auraColor?: string }) => void };
    ...
    const win2 = window as Window & { 
      refreshAllMessageAvatars?: () => Promise<void> | void;
      refreshVisibilityAvatars?: () => Promise<void> | void;
    };
...
export async function updateAvailabilityEverywhere(availability: AvailabilityStatus): Promise<boolean> {
  ...
  try {
    ...
    // ES6 pattern: Dispatch DOM event instead of calling window function
    (window as Window).dispatchEvent(new CustomEvent('updateUserAvatar', {
      detail: { source: 'UnifiedStorageSync' }
    }));
...
export async function updateThemeEverywhere(theme: Theme): Promise<boolean> {
  ...
  try {
    ...
    if (currentUser) {
      ...
      const userWithTheme = currentUser as User & { theme?: Theme };
...
export async function getCurrentUserTheme(): Promise<Theme> {
  ...
  const userWithTheme = stateManagerInstance.getState('currentUser') as (User & { theme?: Theme }) | null;
  ...
  if (currentUser && currentUser.id && apiServiceInstance) {
    try {
      const response = await apiServiceInstance.request<User & { theme?: Theme }>(`/v1/users/${currentUser.id}`, {
        method: 'GET'
      });
      const userData = response?.data;
...
export async function syncStorageWithDatabase(): Promise<void> {
  ...
  try {
    ...
    const userResponse = await apiServiceInstance.request<User>(`/v1/users/${currentUser.id}`, {
      method: 'GET'
    });
    const userData = userResponse?.data;
    
    const availabilityResponse = await apiServiceInstance.request<{ availability?: AvailabilityStatus }>('/v1/presence/availability', {
      method: 'GET'
    });
    ...
    if (availabilityData && availabilityData.availability) {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        ...
        const visibilityData = stateManagerInstance.getState('currentVisibilityDataUnfiltered') as VisibilityData | null;
        if (visibilityData?.active) {
          const currentUser = stateManagerInstance.getState('currentUser') as User | null;
          if (currentUser?.id) {
            const currentUserInVisibility = visibilityData.active.find(u => 
              String(u.id || u.userId) === String(currentUser.id)
            );
            if (currentUserInVisibility) {
    ...
    // Sync theme
    if (userData) {
      const userDataWithTheme = userData as (User & { theme?: Theme });
