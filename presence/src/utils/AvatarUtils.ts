...
class AvatarUtils {
   ...
   */
  static async getAvatarUrl(user: User | null | undefined, context: string = 'visibility'): Promise<AvatarData> {
  ...
  private static async getAuraIntensity(user: User): Promise<number> {
    ...
    if (intensity === undefined && typeof window !== 'undefined') {
      const win = window as Window & { currentUser?: User & { auraIntensity?: number } };
   ...
   */
  static async createUnifiedAvatar(user: User, context: string = 'visibility', options: AvatarOptions = {}): Promise<string> {
