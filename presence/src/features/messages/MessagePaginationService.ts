...
export interface MessagePaginationServiceOptions {
  pageId: string;
  communityId: string;
}
...
export class MessagePaginationService {
  constructor(private options: MessagePaginationServiceOptions) {}

  /**
   * Load a page of messages
   */
  async loadPage(opts: LoadPageOptions): Promise<Message[]> {
    ...
    const api = typeof window !== 'undefined' ? (window as Window & { api?: { request: <T>(endpoint: string, options?: { method?: string }) => Promise<{ data?: T } | null> } }).api : undefined;
    ...
    try {
      ...
      const response = await api.request<{ items: Array<Record<string, unknown>>; nextCursor?: string; hasMore?: boolean }>(endpoint, { method: 'GET' });
      ...
      if (response?.data) {
        ...
        const dataWithItems = response.data as { items?: Array<Record<string, unknown>> };
   ...
   */
  private transformApiMessages(apiMessages: Array<Record<string, unknown>>): Message[] {
    return apiMessages.map((msg): Message => {
      ...
      const thread = msg.thread as { replyCount?: number; topReply?: unknown } | null | undefined;
      ...
      const attachmentsRaw = msg.attachments as Array<{ type: string; url: string; thumbnailUrl?: string; id?: string; filename?: string; size?: number; mimeType?: string }> | undefined;
      const attachments = attachmentsRaw?.map(att => ({
        id: att.id || `att-${Date.now()}-${Math.random()}`,
        type: att.type as 'image' | 'audio' | 'video',
        url: att.url,
   ...
   */
  private buildEndpoint(opts: LoadPageOptions): string {
