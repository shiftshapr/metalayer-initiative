...
import type { FocusState } from './MessageFeed.js';

export interface MessageRendererOptions {
  container: HTMLElement;
  onAvatarHover?: (user: User, element: HTMLElement) => void;
...
export class MessageRenderer {
  ...
  private renderedMessages = new Map<string, HTMLElement>();
  private expandedMessages = new Set<string>();
  private topReplyThreshold = 3; // Configurable threshold for top reply algorithm

  constructor(private options: MessageRendererOptions) {
   ...
   */
  private async renderDefaultMode(messages: Message[]): Promise<void> {
   ...
   */
  private async renderFocusMode(messages: Message[], focusState: FocusState): Promise<void> {
  ...
  private async renderMessage(
    ...
    if (options.isReply) element.classList.add('message-reply');
    if (options.isParent) element.classList.add('message-parent');
    if (options.isTopReply) element.classList.add('message-top-reply');
    if (options.isHighlighted) element.classList.add('message-highlighted');
    if (options.isFocusMode) element.classList.add('focus-mode');
   ...
   */
  private async applyMessageFeatures(
    element: HTMLElement,
    message: Message,
    options: {
      isTopReply?: boolean;
   ...
   */
  private createQuoteElement(quoteText: string): HTMLElement {
   ...
   */
  private createMediaElement(attachment: { type: string; url: string; thumbnailUrl?: string }): HTMLElement {
   ...
   */
  private applyShowMore(contentElement: HTMLElement, messageId: string, fullText: string): void {
  ...
  expandMessage(messageId: string): void {
    ...
    const preview = element.querySelector('.message-content-preview') as HTMLElement | null;
    const full = element.querySelector('.message-content-full') as HTMLElement | null;
   ...
   */
  private applyTopReplyVisual(element: HTMLElement, parentMessageId: string): void {
   ...
   */
  private createReplyInput(parentMessageId: string): HTMLElement {
   ...
   */
  private attachEventListeners(element: HTMLElement, message: Message): void {
    ...
    if (avatar && message.author && this.options.onAvatarHover) {
      avatar.addEventListener('mouseenter', (e) => {
        e.stopPropagation();
        this.options.onAvatarHover!(message.author!, avatar as HTMLElement);
      });
    }
    ...
    if (this.options.onMessageClick) {
      element.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).closest('.message-actions-menu, .reaction-btn, .reply-btn')) {
   ...
   */
  async addMessage(message: Message, options: AddMessageOptions): Promise<void> {
    const element = await this.renderMessage(message, {
   ...
   * Update reply count
   */
  async updateReplyCount(parentMessageId: string): Promise<void> {
   ...
   */
  async updateReactionCount(messageId: string, count: number): Promise<void> {
    const element = this.renderedMessages.get(messageId);
    if (!element) return;

    const reactionBtn = element.querySelector('.reaction-btn');
    if (reactionBtn) {
      const countEl = reactionBtn.querySelector('.icon-count') as HTMLElement | null;
      if (countEl) {
   ...
   */
  private isTopReply(_reply: Message, allReplies: Message[]): boolean {
   ...
   */
  private getReplyCount(messageId: string, allMessages: Message[]): number {
