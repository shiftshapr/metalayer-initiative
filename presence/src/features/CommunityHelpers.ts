 ...
 */
function readActiveCommunitiesFromState(): string[] {
  try {
...
export async function updateCommunityDropdown(communities: Community[]): Promise<void> {
  ...
  try {
    ...
    const primaryCommunityId =
      (typeof primaryCommunityIdRaw === 'string' ? primaryCommunityIdRaw : null) ||
      communities[0]?.id ||
      null;
    ...
    const primaryCommunity =
      sortedCommunities.find((c) => c.id === primaryCommunityId) || sortedCommunities[0];
    const currentCommunityNameEl = document.getElementById('current-community-name');
    const primaryCommunityLogoEl = document.getElementById(
      'primary-community-logo'
    ) as HTMLImageElement;
    ...
    Logger.debug(
      `🔍 COMMUNITIES: Primary community: ${primaryCommunity?.name || 'NONE'} | logoUrl: "${primaryLogoUrlStr}"`,
      {
    ...
    if (currentCommunityNameEl) {
      if (primaryCommunity) {
        ...
        Logger.debug(
          `✅ COMMUNITIES: Set primary community name to: ${primaryCommunity.name}`,
          null,
          'community'
        );
      } else {
      ...
      if (primaryCommunity && primaryCommunity.logoUrl) {
        ...
        if (
          ...
          if (cleanedLogoUrl.endsWith('$0')) {
            ...
            Logger.warn(
              `⚠️ COMMUNITIES: Removed $0 suffix from primary community logoUrl`,
              {
                original: logoUrl,
          ...
          Logger.debug(
            `✅ COMMUNITIES: Set primary community logo to: ${cleanedLogoUrl}`,
            null,
            'community'
          );
        }
    ...
    Logger.info(
      `✅ COMMUNITIES: Updated dropdown with ${communities.length} communities (primary: ${primaryCommunity?.name || 'NONE'})`,
      null,
      'community'
    );
 ...
 */
async function handleCommunitySelection(communityId: string): Promise<void> {
  try {
 ...
 */
async function loadCommunitiesFromAPI(): Promise<Community[]> {
  try {
 ...
 */
export function initializeCommunityDropdown(): void {
 ...
 */
export function getPrimaryCommunityName(communities: Community[]): string {
