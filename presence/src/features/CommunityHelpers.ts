...
export async function updateCommunityDropdown(communities: Community[]): Promise<void> {
  ...
  const sortedCommunities = [...communities].sort((a, b) => {
    ...
    if (a.id === primaryCommunityId) return -1;
    if (b.id === primaryCommunityId) return 1;
  ...
  const primaryCommunityLogoEl = document.getElementById('primary-community-logo') as HTMLImageElement;
  ...
  if (primaryCommunityLogoEl) {
    if (primaryCommunity && primaryCommunity.logoUrl) {
      ...
      if (!logoUrl.startsWith('http://') && !logoUrl.startsWith('https://') && !logoUrl.startsWith('/')) {
        ...
        primaryCommunityLogoEl.style.display = 'none';
        primaryCommunityLogoEl.style.visibility = 'hidden';
        primaryCommunityLogoEl.style.opacity = '0';
    ...
    if (checkbox) {
      checkbox.addEventListener('change', async (e: Event) => {
        try {
          ...
          try {
            const currentUrlData = getState('currentUrlData') as { pageId?: string; rawUrl?: string } | null;
    ...
    if (!isPrimary) {
      ...
      if (menuBtn && menuDropdown) {
        ...
        menuBtn.addEventListener('click', (e: Event) => {
          ...
          menuDropdown.style.visibility = isVisible ? 'hidden' : 'visible';
        ...
        const leaveBtn = menuDropdown.querySelector('.leave-community-btn') as HTMLButtonElement;
        if (leaveBtn) {
          leaveBtn.addEventListener('click', (e: Event) => {
            e.stopPropagation();
            const target = e.target as HTMLButtonElement;
            const communityId = target.dataset.communityId;
 ...
 */
function showLeaveCommunityModal(communityId: string, communityName: string, allCommunities: Community[], currentPrimaryId: string | null): void {
  const modal = document.getElementById('leave-community-modal') as HTMLElement;
  const messageEl = document.getElementById('leave-community-message') as HTMLElement;
  ...
  confirmBtn.addEventListener('click', async () => {
    try {
      ...
      // Reload community dropdown to reflect changes
      updateCommunityDropdown(remainingCommunities).catch((error: unknown) => {
        handleError(error, {
          log: true,
          logLevel: 'error',
          context: {
 ...
 */
export function initializeCommunityDropdownHandler(): void {
