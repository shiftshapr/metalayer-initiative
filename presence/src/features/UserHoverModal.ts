...
class UserHoverModal {
  ...
  private resolveViewerUserId(forceRefresh = false): string | null {
  ...
  private createErrorContext(operation: string, extra: Record<string, unknown> = {}): ErrorContext {
  ...
  public getViewerUserId(): string | null {
   ...
   */
  public show(user: User, targetElement: HTMLElement): void {
   ...
   */
  handleHover(_event: Event, userId: string, targetElement: HTMLElement | null) {
  ...
  async getUserData(userId: string): Promise<{
    ...
    const communities = userData.communities && Array.isArray(userData.communities)
      ? userData.communities.filter((c): c is Community =>
    ...
    return {
      ...
      auraColor: typeof userData.auraColor === 'string' ? userData.auraColor : undefined,
      headline: typeof userData.headline === 'string' ? userData.headline : null,
      communities
    };
  }
      ...
      if (window.currentVisibilityDataUnfiltered && window.currentVisibilityDataUnfiltered.active) {
        ...
        if (userInVisibility && Array.isArray(userInVisibility.communities)) {
          ...
          for (const c of userInVisibility.communities) {
            if (typeof c === 'object' && c !== null && 'id' in c && 'name' in c && typeof (c as { id: unknown }).id === 'string' && typeof (c as { name: unknown }).name === 'string') {
              filtered.push({ id: (c as { id: string }).id, name: (c as { name: string }).name } as Community);
            }
          }
          return filtered;
        }
...
if (typeof window !== 'undefined') {
  ...
  const win = window as Window & { userHoverModal?: UserHoverModal };
