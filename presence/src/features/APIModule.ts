...
let getCurrentUserEmail: (() => Promise<string | null>) | undefined;
...
function isSupabaseClientLike(value: unknown): value is SupabaseClient {
...
class MetaLayerAPI {
  ...
  async request<T = unknown>(endpoint: string, options: ExtendedAPIRequestOptions = {}): Promise<APIResponseOrNull<T>> {
    ...
    if (endpoint.startsWith('http')) {
      ...
      try {
        ...
        if (currentUser && currentUser.id) {
          ...
          if (user.auraColor && typeof user.auraColor === 'object' && 'then' in user.auraColor && typeof (user.auraColor as Promise<string>).then === 'function') {
          ...
          try {
              const resolvedAuraColor = await (user.auraColor as Promise<string>);
          ...
          try {
            ...
            if (userResponse && userResponse.data) {
              ...
              const avatarUrl = (userResponse.data as { avatarUrl?: string }).avatarUrl;
    ...
    try {
      ...
      if (response.status === 404 && options.allow404) {
        ...
        return null as APIResponseOrNull<T>;
      }
      ...
      if (!response.ok) {
        ...
        const error: ErrorWithStatus = new Error(errorDetails);
      ...
      const isConnectionError = errorObj.message && (
      ...
      if (isConnectionError) {
        ...
        let apiConnectionErrors = stateManagerInstance.getState('_apiConnectionErrors') as number | undefined || 0;
  ...
  async getAvatars(communityId: string): Promise<APIResponseOrNull> {
    return this.request(`/avatars/active?communityId=${communityId}`);
  }

  async getPresenceByUrl(url: string, communityIds: string[] | null = null): Promise<APIResponseOrNull> {
  ...
  async getPresenceByCommunities(communityIds: string[]): Promise<APIResponseOrNull> {
  ...
  async login(): Promise<APIResponseOrNull> {
    return this.request('/auth/login', { method: 'POST' });
  }

  async getMe(): Promise<APIResponseOrNull> {
    return this.request('/auth/me');
  }

  async sendMessage(userId: string, communityId: string, content: string, uri: string | null = null, parentId: string | null = null, threadId: string | null = null, optionalContent: Record<string, unknown> | null = null): Promise<APIResponseOrNull<Message>> {
  ...
  async getChatHistory(communityId: string, threadId: string | null = null, uri: string | null = null): Promise<{ conversations: Array<{ id: string; [key: string]: unknown }>; messages: Message[] }> {
    ...
    try {
      ...
      if (uri) {
        ...
        const normalizedUrl: string | { pageId: string } | null = normalizeUrl ? normalizeUrl(uri) : uri;
        if (typeof normalizedUrl === 'string') {
          ...
          pageId = (normalizedUrl as { pageId: string }).pageId;
      ...
      let query = supabase.from<SupabaseMessageRow>('messages').select('*, AppUser:user_id(*)');
      ...
      if (messages && messages.length > 0) {
        ...
        Logger.debug(`🔍 CHAT_API: Sample message pageIds:`, messages.slice(0, 3).map((m: { page_id?: string }) => m.page_id), 'api');
      } else {
        ...
        const { data: allMessages } = await supabase.from<SupabaseMessageRow>('messages').select('page_id, community_id').limit(10);
        Logger.debug(`🔍 CHAT_API: Sample messages in DB (first 10):`, allMessages?.map((m: { page_id?: string; community_id?: string }) => ({ page_id: m.page_id, community_id: m.community_id })), 'api');
      }
      ...
      const msgs = messages?.map((msg: { id: string; content: string; user_id?: string; created_at?: string; updated_at?: string; parent_id?: string | null; page_id?: string; AppUser?: { id?: string; name?: string; handle?: string; avatarUrl?: string; auraColor?: string } }) => {
  ...
  async deleteMessage(messageId: string): Promise<APIResponseOrNull> {
    // COMP METHOD: Use exact same endpoint as COMP
    return this.request(`/v1/posts/${messageId}`, { method: 'DELETE' });
  }

  async editMessage(messageId: string, newContent: string): Promise<APIResponseOrNull> {
  ...
  async getReactions(messageId: string): Promise<APIResponseOrNull> {
    return this.request(`/v1/reactions?messageId=${messageId}`);
  }

  async addReaction(messageId: string, reactionType: string): Promise<APIResponseOrNull> {
  ...
  async removeReaction(messageId: string, reactionType: string): Promise<APIResponseOrNull> {
    return this.request('/v1/reactions', {
      method: 'DELETE',
      body: JSON.stringify({ messageId, reactionType })
    });
  ...
  async getReactionsByMessage(messageId: string): Promise<APIResponseOrNull> {
