...
export interface LoadingGifOptions {
  container?: string;
  showDelay?: number;
  hideDelay?: number;
  text?: string;
...
export class LoadingGifManager {
  ...
  private options: Required<LoadingGifOptions>;
  private loadingGifElement: HTMLElement | null = null;
  private showTimeout: ReturnType<typeof setTimeout> | null = null;
  private hideTimeout: ReturnType<typeof setTimeout> | null = null;

  constructor(logger: typeof Logger = Logger, options: LoadingGifOptions = {}) {
    this.logger = logger;
    this.options = {
   ...
   */
  show(options?: { container?: string; text?: string }): void {
   ...
   */
  private _showLoadingGif(containerSelector?: string, loadingText?: string): void {
    try {
      ...
      const container = document.querySelector(effectiveContainer) as HTMLElement;
      if (!container) {
      ...
      let loadingGif = container.querySelector('.loading-container') as HTMLElement;
      if (loadingGif) {
        // Update existing gif text if different
        const textElement = loadingGif.querySelector('div:last-child') as HTMLElement;
...
let loadingGifManagerInstance: LoadingGifManager | null = null;

export function getLoadingGifManager(logger?: typeof Logger, options?: LoadingGifOptions): LoadingGifManager {
...
if (typeof window !== 'undefined') {
  (window as any).loadingGifManager = getLoadingGifManager();
}
