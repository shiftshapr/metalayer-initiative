...
export class TabDisplay {
  ...
  private createManageButton(): void {
    ...
    let existingManageButton = this.container.querySelector('[data-tab="manage-tab"]') as HTMLElement | null;
    if (!existingManageButton) {
      existingManageButton = this.container.querySelector('#tab-manager-button') as HTMLElement | null;
    }
    if (existingManageButton) {
