...
// Global chrome API declaration for extension environment
...
export class TabStateManager {
  ...
  private themeChangeInProgress: boolean = false;
  private logger: typeof Logger;

  // Security: Memory limits to prevent DoS attacks
  private readonly MAX_LOADING_STATES = 50;
  private readonly MAX_LOADED_TABS = 20;
   ...
   */
  shouldLoadTab(tabId: string, operation: TabOperation): boolean {
    switch (operation) {
   ...
   */
  getOperationDescription(operation: TabOperation): string {
    switch (operation) {
      case TabOperation.LOAD:
   ...
   */
  setThemeChanging(changing: boolean): void {
    // ROOT CAUSE FIX: Prevent theme changes during tab operations
    const win = typeof window !== 'undefined' ? window : null;
    const tabManager = (win as any)?.tabContextManager;
   ...
   */
  validateOperation(tabId: string, operation: TabOperation): { valid: boolean; reason?: string } {
...
let tabStateManagerInstance: TabStateManager | null = null;

export function getTabStateManager(logger?: typeof Logger): TabStateManager {
...
if (typeof window !== 'undefined') {
  ...
  (window as any).tabStateManager = instance;

  // Also expose constructor for legacy compatibility
  (window as any).TabStateManager = TabStateManager;
}
