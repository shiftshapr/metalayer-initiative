...
export class BootController {
  ...
  async initializeAuthFlow(): Promise<void> {
    ...
    const win = typeof window !== 'undefined' ? (window as Window & { initializeRealGoogleAuth?: () => void }) : null;
          ...
          from(table: string): {
            select(columns: string): {
              eq(column: string, value: unknown): {
                single(): Promise<{ data: { id: string } | null; error: { message: string; code?: string; details?: string } | null }>;
              };
            };
          };
        }
