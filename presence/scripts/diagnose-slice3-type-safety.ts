...
// Standard browser APIs that are part of TypeScript's DOM types
const standardBrowserAPIs = new Set([
  'getComputedStyle', 'location', 'innerHeight', 'innerWidth', 'performance',
  'setTimeout', 'clearTimeout', 'setInterval', 'clearInterval',
  'open', 'close', 'addEventListener', 'removeEventListener', 'dispatchEvent',
  'getSelection', 'requestAnimationFrame', 'cancelAnimationFrame',
  'localStorage', 'sessionStorage', 'navigator', 'document', 'history',
  'screen', 'devicePixelRatio', 'matchMedia', 'fetch', 'XMLHttpRequest',
  'console', 'alert', 'confirm', 'prompt', 'atob', 'btoa'
]);

let windowProperties: Set<string> = new Set(standardBrowserAPIs);
if (fs.existsSync(globalTypesFile)) {
  ...
  if (windowInterfaceMatch) {
    const interfaceContent = windowInterfaceMatch[1];
    
    // Extract all property names using multiple patterns
    // Pattern 1: propertyName?: type; or propertyName: type;
    const propPattern1 = /(?:^|\n)\s*(\w+)\??\s*[:?]/gm;
    let match;
    while ((match = propPattern1.exec(interfaceContent)) !== null) {
      const name = match[1].trim();
      // Skip TypeScript keywords and common type names
      if (name && !['key', 'string', 'number', 'boolean', 'unknown', 'any', 'void', 'null', 'undefined'].includes(name)) {
        windowProperties.add(name);
      }
    }
    
    // Pattern 2: Extract from nested objects (e.g., diagnosticFramework?: { diagnoseAll?: ... })
    const nestedObjectPattern = /(\w+)\??\s*:\s*\{([^}]*)\}/g;
    while ((match = nestedObjectPattern.exec(interfaceContent)) !== null) {
      const objName = match[1];
      const nestedContent = match[2];
      
      // Extract nested properties
      const nestedPropPattern = /(\w+)\??\s*[:?]/g;
      let nestedMatch;
      while ((nestedMatch = nestedPropPattern.exec(nestedContent)) !== null) {
        const nestedName = nestedMatch[1];
        if (nestedName && !['key', 'string', 'number', 'boolean', 'unknown', 'any', 'void'].includes(nestedName)) {
          // Store as parent.child for nested access detection
          windowProperties.add(`${objName}.${nestedName}`);
        }
      }
    }
    
    // Pattern 3: Extract from type references (e.g., InstanceType<ProfileManagerConstructor>)
    // These are already handled by Pattern 1, but we can add the constructor names
    const constructorPattern = /(\w+)\??\s*:\s*(?:InstanceType<|typeof\s+)?(\w+)/g;
    while ((match = constructorPattern.exec(interfaceContent)) !== null) {
      const propName = match[1];
      if (propName) {
        windowProperties.add(propName);
      }
    }
  }
}
...
function analyzeFile(filePath: string, relativePath: string): void {
  ...
  // Match window.propertyName patterns
  const windowPropRegex = /window\.(\w+)(?:\.|\[|;|\)|,|\s|$)/g;
  while ((match = windowPropRegex.exec(content)) !== null) {
    const propName = match[1];
    
    // Skip if it's a standard browser API or internal property
    if (propName.startsWith('__') || standardBrowserAPIs.has(propName)) {
      continue;
    }
    
    // Check if property exists directly
    const existsDirectly = windowProperties.has(propName);
    
    // Check if property exists as nested (e.g., diagnosticFramework.diagnoseAll)
    const existsNested = Array.from(windowProperties).some(prop => {
      if (prop.includes('.')) {
        const parts = prop.split('.');
        return parts.length === 2 && parts[1] === propName;
      }
      return false;
    });
    
    // Also check if it's part of a grouped object (e.g., diagnosticFramework contains diagnoseAll)
    const existsInGroup = Array.from(windowProperties).some(prop => {
      // Check if propName is a nested property of any grouped object
      return prop.includes('.') && prop.endsWith(`.${propName}`);
    });
    
    if (!existsDirectly && !existsNested && !existsInGroup) {
...
if (allIssues.length > 0) {
  ...
  // Output detailed issue locations for quick wins
  const anyIssues = allIssues.filter(i => i.type === 'any');
  const assertionIssues = allIssues.filter(i => i.type === 'unsafe-assertion');
  
  if (anyIssues.length > 0) {
    console.log('\n🔴 `any` TYPES:');
    anyIssues.forEach(issue => {
      console.log(`  ${issue.file}:${issue.line}:${issue.column} - ${issue.context.substring(0, 80)}`);
    });
  }
  
  if (assertionIssues.length > 0) {
    console.log('\n🔴 UNSAFE ASSERTIONS:');
    assertionIssues.forEach(issue => {
      console.log(`  ${issue.file}:${issue.line}:${issue.column} - ${issue.context.substring(0, 80)}`);
    });
  }
  
  const missingTypeIssues = allIssues.filter(i => i.type === 'missing-window-type');
  if (missingTypeIssues.length > 0) {
    console.log('\n🔴 MISSING WINDOW TYPES:');
    // Group by property name
    const propsByFile = new Map<string, Set<string>>();
    missingTypeIssues.forEach(issue => {
      const match = issue.context.match(/window\.(\w+)/);
      if (match) {
        const prop = match[1];
        if (!propsByFile.has(issue.file)) {
          propsByFile.set(issue.file, new Set());
        }
        propsByFile.get(issue.file)!.add(prop);
      }
    });
    
    // Show unique properties per file
    propsByFile.forEach((props, file) => {
      console.log(`  ${file}:`);
      Array.from(props).sort().forEach(prop => {
        console.log(`    - ${prop}`);
      });
    });
    
    // Show all unique properties across all files
    ...
    console.log(`\n  📋 All unique missing properties (${allProps.size}):`);
