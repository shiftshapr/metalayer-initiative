...
const errorPatterns: Record<string, { category: ErrorAnalysis['category'], severity: ErrorAnalysis['severity'], fixPattern: string }> = {
...
export function analyzeMessagesModuleErrors(errorOutput: string): ErrorAnalysis[] {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      const pattern = errorPatterns[code] || { category: 'other' as const, severity: 'warning' as const, fixPattern: 'Review and fix' };
      errors.push({
...
export function generateFixReport(errors: ErrorAnalysis[]): string {
...
// For direct execution (ES module)
...
if (isMainModule || process.argv[1]?.includes('diagnose-messages-module-types')) {
