...
function addResult(category: string, severity: DiagnosticResult['severity'], message: string, file?: string, line?: number, code?: string): void {
...
function checkCallbackSignatures(): void {
...
function checkWindowAugmentations(): void {
...
function checkOptionalRendererReferences(): void {
  const modalFile = join(projectRoot, 'components', 'UnifiedMessageModal.ts');
  const hoverFile = join(projectRoot, 'features', 'UserHoverModal.ts');
  const rendererFile = join(projectRoot, 'utils', 'UnifiedMessageRenderer.ts');
...
function checkHoverModalRegistration(): void {
  const hoverFile = join(projectRoot, 'features', 'UserHoverModal.ts');
  const hoverContent = readFileSafe(hoverFile);
