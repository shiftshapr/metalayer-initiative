...
function checkTypeScriptConfigs(): void {
  ...
  let presenceConfigPath: string | null = null;
  ...
  const rootConfig = readJsonFile(rootConfigPath) as { compilerOptions?: Record<string, unknown> } | null;
  const presenceConfig = readJsonFile(presenceConfigPath) as { compilerOptions?: Record<string, unknown> } | null;
...
function checkTypeExports(): void {
