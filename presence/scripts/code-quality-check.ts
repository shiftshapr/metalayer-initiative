...
function runTypeScriptCheck(): { errors: number; output: string[] } {
  try {
    const presencePath = join(process.cwd(), 'presence');
    if (!presencePath) {
      console.error('❌ Error: Could not determine presence directory path');
      return { errors: 1, output: ['Path resolution failed'] };
    }
    ...
    if (!error || typeof error !== 'object') {
      console.error('❌ Error: Unknown error type in TypeScript check');
      return { errors: 1, output: ['Unknown error occurred'] };
    }
    ...
    if (!errorOutput) {
      return { errors: 1, output: ['No error output available'] };
    }
...
try {
  ...
  if (!report) {
    console.error('❌ Error: Failed to generate report');
    process.exit(1);
  }
  printReport(report);
} catch (error) {
