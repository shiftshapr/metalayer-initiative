...
interface CleanupCandidate {
  path: string;
  reason: string;
  category: 'markdown' | 'test' | 'diagnostic' | 'duplicate' | 'stale';
  lastModified: Date;
  size: number;
}
...
function findFiles(dir: string, pattern: RegExp, category: CleanupCandidate['category'], reason: string): void {
...
if (!fs.existsSync(presenceRoot)) {
  ...
  let presenceRootFiles: fs.Dirent[];
  try {
...
const byCategory = candidates.reduce((acc, candidate) => {
  if (!acc[candidate.category]) acc[candidate.category] = [];
  acc[candidate.category].push(candidate);
  return acc;
}, {} as Record<string, CleanupCandidate[]>);
