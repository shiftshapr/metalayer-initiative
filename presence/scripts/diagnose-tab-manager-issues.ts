...
class TabManagerDiagnostic {
  ...
  private checkTabContentSwitching(): void {
    ...
    tabButtons.forEach(btn => {
      const listeners = (btn as any).__eventListeners || [];
      if (listeners.length > 0) hasClickHandlers = true;
    });
  ...
  private checkCurrentPreviousTabTracking(): void {
  ...
  private checkVisibleTabCount(): void {
  ...
  private checkTabDisplayRendering(): void {
  ...
  private checkStoragePersistence(): void {
...
if (typeof window !== 'undefined') {
  (window as any).TabManagerDiagnostic = TabManagerDiagnostic;
