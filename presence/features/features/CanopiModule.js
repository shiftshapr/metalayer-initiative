...
function updateMessageInChat(updatedMessage) {
    ...
    if (!messageElement) {
        ...
        // Try to add as new message if available
        if (typeof window !== 'undefined' && window.addMessageToChat) {
            window.addMessageToChat(updatedMessage);
        }
        return;
    }
    ...
    if (typeof window !== 'undefined' && window.currentChatData) {
...
function getSenderName(userId) {
    if (!userId)
        return 'Unknown User';
    // Try to get from currentChatData (messages already loaded)
    if (typeof window !== 'undefined' && window.currentChatData) {
        const message = window.currentChatData.find((m) => m.authorId === userId || m.author?.id === userId || m.user_id === userId);
        if (message && message.author) {
            return message.author.name || message.author.handle || 'Unknown User';
        }
    }
    // Try to get from window.currentUser
    if (typeof window !== 'undefined' && window.currentUser) {
        if (window.currentUser.id === userId || window.currentUser.user_id === userId) {
            return window.currentUser.name || window.currentUser.email?.split('@')[0] || 'Unknown User';
        }
    }
    // Try to get from visibility data
    if (typeof window !== 'undefined' && window.currentVisibilityData) {
        ...
        const user = activeUsers.find((u) => u.id === userId || u.user_id === userId || u.userId === userId);
        if (user) {
...
function canUserEditMessage(message) {
    ...
    const currentUserEmail = typeof window !== 'undefined' && window.currentUser ? window.currentUser.email : null;
...
async function createUnifiedMessageElement(message) {
    ...
    if (typeof window.UnifiedMessageRenderer !== 'undefined' &&
        typeof window.UnifiedMessageRenderer.renderMessage === 'function') {
    ...
    <div class="message-content-wrapper">
      <div class="message-header-new">
        <span class="message-sender-name">${getSenderName(message.authorId || message.author?.id || message.user_id || '')}</span>
      </div>
...
function addMessageActionListeners(messageDiv, message) {
    ...
    if (replyButton) {
        replyButton.addEventListener('click', (e) => {
            e.stopPropagation();
            if (typeof window.handleReplyClick === 'function') {
                window.handleReplyClick(messageId, message);
            }
            else {
                ...
                if (chatTextarea) {
                    ...
                    if (typeof window.setReplyContext === 'function') {
                        window.setReplyContext(messageId, message);
                    }
                }
            }
        });
    ...
    if (reactionButton) {
        reactionButton.addEventListener('click', (e) => {
            ...
            if (typeof window.handleReactionClick === 'function') {
                window.handleReactionClick(messageId, message);
            }
            else {
    ...
    if (bookmarkButton) {
        bookmarkButton.addEventListener('click', (e) => {
            e.stopPropagation();
            if (typeof window.handleBookmarkClick === 'function') {
                window.handleBookmarkClick(messageId, message);
            }
            else {
    ...
    if (shareButton) {
        shareButton.addEventListener('click', (e) => {
            e.stopPropagation();
            if (typeof window.handleShareClick === 'function') {
                window.handleShareClick(messageId, message);
            }
            else {
    ...
    if (editButton) {
        editButton.addEventListener('click', (e) => {
            ...
            if (typeof window.handleEditClick === 'function') {
                window.handleEditClick(messageId, message);
            }
            else {
    ...
    if (deleteButton) {
        deleteButton.addEventListener('click', (e) => {
            ...
            if (typeof window.handleDeleteClick === 'function') {
                window.handleDeleteClick(messageId, message);
            }
            else {
...
async function handleMessageFocus(messageOrId) {
    try {
        ...
        if (typeof messageOrId === 'string') {
            ...
            // Try to find message in currentChatData
            if (typeof window !== 'undefined' && window.currentChatData) {
                message = window.currentChatData.find(m => m.id === messageId) || null;
            }
        }
        else {
            message = messageOrId;
...
async function loadChatHistory(communityIdOrRawUrl, activeCommunitiesOrUndefined) {
    ...
    try {
        ...
        if (!rawUrl && typeof window !== 'undefined' && window.currentUrlData) {
            rawUrl = window.currentUrlData.rawUrl || window.location.href;
        }
        if (!rawUrl) {
        ...
        if (!activeCommunities && typeof window !== 'undefined') {
            try {
                // Method 1: Try window.getState() (PRIMARY - defined in sidepanel.js line 3549)
                ...
                // Method 2: Try stateManager instance (direct access)
                if ((!activeCommunities || activeCommunities.length === 0) &&
                    window.stateManager && typeof window.stateManager.getState === 'function') {
                ...
                // Method 3: Fallback to window.activeCommunities (direct property - legacy support)
                ...
                // If still no communities, wait a bit and retry (communities may be loading)
                if ((!activeCommunities || activeCommunities.length === 0)) {
                    console.log('⏳ loadChatHistory: Active communities not available yet, waiting...');
                    // Wait up to 5 seconds for communities to load (increased from 2 seconds)
                    for (let i = 0; i < 25; i++) {
                        await new Promise(resolve => setTimeout(resolve, 200));
                        // Try window.getState() first (PRIMARY)
                        if (typeof window.getState === 'function') {
                            activeCommunities = window.getState('ui.activeCommunities') || [];
                        }
                        // Try stateManager instance
                        if ((!activeCommunities || activeCommunities.length === 0) &&
                            window.stateManager && typeof window.stateManager.getState === 'function') {
                            activeCommunities = window.stateManager.getState('ui.activeCommunities') || [];
                        }
                        ...
                        if (activeCommunities && activeCommunities.length > 0) {
                            console.log('✅ loadChatHistory: Active communities loaded after retry:', activeCommunities);
                            break;
                        }
                    }
                }
                ...
                console.error('❌ loadChatHistory: Error getting active communities:', error);
                // Fallback to direct property if StateManager access fails
                if (window.activeCommunities) {
                    activeCommunities = window.activeCommunities;
                    console.log('🔍 loadChatHistory: Using fallback window.activeCommunities:', activeCommunities);
                }
            }
        }
        if (!activeCommunities || activeCommunities.length === 0) {
        ...
        if (typeof window !== 'undefined' && window.currentUrlData) {
            pageId = window.currentUrlData.pageId;
            normalizedUrlData = window.currentUrlData;
        }
        else {
            // Fallback: use normalizeUrl if available
            if (typeof window.normalizeUrl === 'function') {
                normalizedUrlData = await window.normalizeUrl(rawUrl);
        ...
        const existingMessageIds = new Set(Array.from(existingMessages)
            .map(msg => msg.dataset.messageId)
            .filter((id) => !!id));
        // Fetch messages for each active community using the working API method
        const allMessages = [];
        if (typeof window !== 'undefined' && window.api && typeof window.api.getChatHistory === 'function') {
            const api = window.api;
        ...
        if (allMessages.length > 0 && existingMessages.length === 0) {
        ...
        else if (allMessages.length > 0 && existingMessages.length > 0) {
            ...
            console.log('🔍 loadChatHistory: New message count:', allMessages.length);
        ...
        if (allMessages.length > 0) {
            console.log(`📜 loadChatHistory: Rendering ${allMessages.length} messages`);
            // Use UnifiedMessageRenderer (the working implementation)
            if (typeof window.UnifiedMessageRenderer !== 'undefined' &&
                typeof window.UnifiedMessageRenderer.generateMessageHTML === 'function') {
                const UnifiedMessageRenderer = window.UnifiedMessageRenderer;
                for (const message of allMessages) {
                    // COMP METHOD: Skip if message already exists in DOM (avoid duplicates)
                    ...
                    try {
                        ...
                        const html = await UnifiedMessageRenderer.generateMessageHTML(message, {
                            ...
                            isFocusMode: false,
                            author: message.author,
                            communityName: '', // Will be populated if needed
                            formattedTime: (message.createdAt || message.created_at) ? UnifiedMessageRenderer.formatMessageTime(message.createdAt || message.created_at || '') : '',
                            ...
                            hasUserReposted: false,
                            hasUserShared: false,
                            canEdit: false, // Will be determined by permissions
                            canDelete: false // Will be determined by permissions
                        });
                        const messageDiv = document.createElement('div');
                        ...
                        messageDiv.dataset.authorId = message.authorId || message.author?.id || message.user_id || '';
                        if (message.parentId) {
                            messageDiv.dataset.parentId = message.parentId;
                        }
                        // COMP METHOD: Check if reply should be visible (thread expanded or focus mode)
                        // This matches the original working JavaScript implementation
                        if (isReply) {
                            ...
                            if (threadToggle && threadToggle.dataset.expanded === 'true') {
                                ...
                                console.log('📜 loadChatHistory: Reply added, keeping collapsed by default');
                            }
                        }
                ...
                if (window.addMessageActionListeners) {
                    ...
                    messageElements.forEach((msgEl) => {
                        ...
                        if (message) {
                            window.addMessageActionListeners(msgEl, message);
                        }
                    });
                }
                    ...
                    try {
                        ...
                        if (hasRepliesFlag && !message.parentId) {
                            ...
                            if (threadToggle) {
                                ...
                                toggleCountEl.style.display = 'inline-block';
                            }
                        }
                        // COMP METHOD: Check permissions (user can edit/delete their own messages)
                        const currentUserId = (typeof window !== 'undefined' && window.currentUser)
                            ? (window.currentUser.id || window.currentUser.user_id)
                            : null;
                        const messageAuthorId = message.authorId || message.author?.id || message.user_id;
                        ...
                        if (window.updateReactionDisplay) {
                            ...
                            if (window.reactionsIntegration && window.reactionsIntegration.reactionsManager) {
                                try {
                                    reactions = await window.reactionsIntegration.reactionsManager.getReactions(message.id) || [];
                                }
                                catch (error) {
                                    console.warn(`⚠️ loadChatHistory: Could not load reactions for message ${message.id}:`, error);
                                }
                            }
                            window.updateReactionDisplay(message.id, reactions);
                        }
                    }
                    catch (error) {
                ...
                console.log(`✅ loadChatHistory: Successfully rendered ${allMessages.length} messages with initial data loaded (COMP METHOD)`);
            }
            else if (window.createUnifiedMessageElement) {
                // Fallback to createUnifiedMessageElement
                for (const message of allMessages) {
                    try {
                        const messageDivResult = window.createUnifiedMessageElement(message);
                        const messageDiv = messageDivResult instanceof Promise ? await messageDivResult : messageDivResult;
                        chatMessages.appendChild(messageDiv);
                    }
                    catch (error) {
                        console.error('❌ loadChatHistory: Error rendering message:', error);
                ...
                console.log(`✅ loadChatHistory: Successfully rendered ${allMessages.length} messages (fallback method)`);
            }
            else {
                console.warn('⚠️ loadChatHistory: No message renderer available');
            }
        }
        else {
            console.log('ℹ️ loadChatHistory: No messages to display');
        }
        // Store messages in window.currentChatData for avatar updates
        if (typeof window !== 'undefined') {
            window.currentChatData = allMessages;
        }
            ...
            if (footer) {
                ...
                if (toggleBtn) {
                    toggleBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        if (typeof window !== 'undefined' && window.toggleThreadReplies) {
                            window.toggleThreadReplies(conversationId, messageElement);
                        }
                    });
                }
            }
...
async function handleDeleteMessage(message) {
    ...
    try {
        ...
        if (isUUID) {
            ...
            if (typeof window !== 'undefined' && window.robustIntegration && window.robustIntegration.isInitialized) {
                await window.robustIntegration.deleteMessage(message.id);
            }
            else {
                // Fallback to legacy system
                const client = typeof window !== 'undefined' ? (window.supabaseRealtimeClient || window.supabaseRealtimeClient) : null;
            ...
            if (typeof window !== 'undefined' && window.api && typeof window.api.deleteMessage === 'function') {
        ...
        // Remove from global chat data
        if (typeof window !== 'undefined' && window.currentChatData) {
            const index = window.currentChatData.findIndex(m => m.id === message.id);
            if (index !== -1) {
                window.currentChatData.splice(index, 1);
            }
        }
    }
    catch (error) {
        ...
        if (newContent && newContent !== (message.body || message.content)) {
            try {
                ...
                if (typeof window !== 'undefined' && window.currentChatData) {
                    const index = window.currentChatData.findIndex(m => m.id === message.id);
                    if (index !== -1) {
                        window.currentChatData[index] = { ...window.currentChatData[index], body: newContent, content: newContent };
                    }
                }
            }
            catch (error) {
...
async function sendMessageViaSupabase(content, parentId = null, conversationId = null) {
    ...
    if (typeof window !== 'undefined' && window.robustIntegration && window.robustIntegration.isInitialized) {
        ...
        try {
            const messageData = await window.robustIntegration.sendMessage(content, parentId, conversationId);
            if (messageData) {
...
async function getSenderAvatar(author) {
    if (!author)
        return getSenderInitial('Unknown');
    // Use unified avatar system for consistency
    if (typeof window !== 'undefined' && window.AvatarUtils && typeof window.AvatarUtils.createUnifiedAvatar === 'function') {
        const avatarHTML = await window.AvatarUtils.createUnifiedAvatar(author, 'message', {
            size: 32,
            showStatus: true,
            showAura: true,
            context: 'message',
    ...
    return getSenderInitial(author.name || author.email || 'Unknown');
}
/**
    ...
    try {
        if (shareType === 'link') {
            ...
            if (typeof window !== 'undefined' && window.showNotification) {
                window.showNotification('Message link copied to clipboard!');
            }
            console.log('✅ SHARE: Message link copied:', messageUrl);
        }
        else if (shareType === 'twitter') {
            const tweetText = `Check out this message from @${message.author?.handle || message.author?.name || 'Metalayer'}: "${message.body || message.content}" ${messageUrl}`;
        ...
        if (typeof window !== 'undefined' && window.showNotification) {
            window.showNotification('Failed to share message');
        }
    }
}
/**
...
function handleStartThread(message) {
    ...
    if (typeof window !== 'undefined' && window.uiManager && typeof window.uiManager.switchTab === 'function') {
        window.uiManager.switchTab('discuss-tab');
    }
    const chatInput = document.getElementById('chat-textarea');
    if (chatInput) {
        chatInput.focus();
        chatInput.placeholder = `Replying to thread by ${message.author?.name || 'Unknown'}...`;
        chatInput.dataset.replyTo = message.id;
        chatInput.dataset.replyToConversation = message.conversationId || '';
        chatInput.dataset.contextMode = 'reply';
    }
    if (typeof window !== 'undefined' && window.setReplyContext) {
        window.setReplyContext(message.id, message);
    }
}
/**
 * Handle copy link
...
async function focusOnMessage(message) {
    ...
    if (typeof window !== 'undefined' && window.handleMessageFocus) {
        await window.handleMessageFocus(message);
    }
    else {
...
async function handleBackNavigation() {
    ...
    if (typeof window !== 'undefined' && window.focusedMessage && window.previousView) {
...
async function handleReaction(message) {
    ...
    if (typeof window !== 'undefined' && window.reactionsIntegration && window.reactionsIntegration.reactionsManager) {
        await window.reactionsIntegration.reactionsManager.addReaction(message.id, '👍');
    }
    else {
...
function setupMessageInputEventListeners() {
    ...
    if (cancelContextButton) {
        cancelContextButton.addEventListener('click', () => {
            if (typeof window !== 'undefined' && window.clearContext) {
                window.clearContext();
            }
        });
    }
...
async function sendChatMessage() {
    ...
    try {
        if (editingMessageId) {
            // Handle edit
            if (typeof window !== 'undefined' && window.handleEditMessage) {
                const messageToEdit = (window.currentChatData || []).find(m => m.id === editingMessageId);
                if (messageToEdit) {
                    await window.handleEditMessage({ ...messageToEdit, body: content, content: content });
                }
                else {
            ...
            if (typeof window !== 'undefined' && window.sendMessageViaSupabase) {
                const messageData = await window.sendMessageViaSupabase(content, parentId, conversationId);
                if (messageData) {
                    console.log('✅ SEND_CHAT_MESSAGE: Message sent:', messageData);
                    // Add message to UI immediately for responsiveness
                    if (typeof window !== 'undefined' && window.addMessageToChat) {
                        await window.addMessageToChat(messageData);
                    }
                }
            }
            else {
        ...
        if (typeof window !== 'undefined' && window.showNotification) {
            window.showNotification('Failed to send message');
        }
    }
    finally {
        chatTextarea.value = '';
        if (typeof window !== 'undefined' && window.autoResize) {
            window.autoResize(chatTextarea);
        }
        if (typeof window !== 'undefined' && window.clearContext) {
            window.clearContext();
        }
...
// ES6 module exports
export { CanopiModule, createUnifiedMessageElement, updateReactionDisplay, addMessageActionListeners, loadMessageReactions, handleMessageFocus, loadChatHistory, // CRITICAL: Export for module imports
