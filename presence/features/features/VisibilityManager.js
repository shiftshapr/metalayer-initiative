            ...
            const usersWithAvatars = await Promise.all(users.map(async (user) => {
                try {
                    ...
                    return {
                        ...
                        auraColor: user.aura_color || AVATAR_FALLBACK_COLOR,
                        aura_color: user.aura_color || AVATAR_FALLBACK_COLOR,
...
async function updateVisibleTab(avatars) {
    ...
    const usersWithAvatars = avatars.filter(avatar => {
        ...
        const isCurrentUser = (currentUserId && avatarId && String(avatarId) === String(currentUserId)) ||
            ...
            (currentUser !== null && currentUser !== undefined &&
                (avatar.id === currentUser.id || avatar.userId === currentUser.user_id));
        if (isCurrentUser) {
    ...
    if (usersWithAvatars.length > 0) {
        for (const user of usersWithAvatars) {
            try {
                const isActive = user.is_active === true || user.isActive === true;
                const hasLeft = user.status === 'offline' || !isActive;
                const userName = user.name || user.email?.split('@')[0] || 'Unknown';
                const avatarUrl = user.avatarUrl || user.avatar_url || user.picture || '/icons/default-user.svg';
                const auraColor = user.auraColor || user.aura_color || '#aaaaaa';
                // Create list item
