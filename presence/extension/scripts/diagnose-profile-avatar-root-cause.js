...
async function diagnoseProfileAvatarRootCause() {
  ...
  if (results.checks.dom.userAvatarContainer && !results.checks.dom.userAvatar) {
    results.rootCauses.push('user-avatar-container exists but .user-avatar element missing - avatar creation may have failed');
    results.recommendations.push('Check ProfileManager.createUserAvatar() - ensure it creates .user-avatar element inside container');
  }
