...
const getChatMessagesContainer = (doc) => doc.querySelector('.chat-messages');
const ensureOverlayContainer = (doc, chatMessages) => {
    if (!chatMessages)
        return null;
...
const showLoading = (doc, chatMessages) => {
    if (!chatMessages)
        return null;
...
const makeElementsVisible = (elements) => {
...
export const forceHideAllOverlays = (domWindow, doc) => {
    ...
    const overlays = chatMessages.querySelectorAll('.chat-loading-overlay, .chat-loading-indicator, .loading-spinner, [class*="loading"]');
    ...
    makeElementsVisible(chatMessages.querySelectorAll('.message, [data-message-id]'));
    console.log('🔧 CHAT_PATCH: Force-hid all overlays and made messages visible');
};
const hideLoading = async (domWindow, doc, chatMessages, state) => {
    ...
    chatMessages.style.setProperty('display', 'block', 'important');
    chatMessages
        .querySelectorAll('[style*="visibility: hidden"], [style*="opacity: 0"]')
        .forEach(el => {
    ...
    setTimeout(() => {
        ...
        if (allMessagesAfterDelay.length === 0) {
            ...
            chatMessages.querySelectorAll('.empty-state-message').forEach(message => message.remove());
        }
    }, 100);
    console.log('✅ CHAT_PATCH: Loading overlay hidden after', (performance.now() - state.startTime).toFixed(0), 'ms');
};
const wrapLoadChatHistory = (domWindow, doc) => {
    if (typeof domWindow.loadChatHistory !== 'function')
        return;
    const originalLoadChatHistory = domWindow.loadChatHistory;
    let activeLoadState = null;
    domWindow.loadChatHistory = async function patchedLoadChatHistory(...args) {
...
const wrapAddMessageToChat = (domWindow, doc) => {
    if (typeof domWindow.addMessageToChat !== 'function')
        return;
...
const startStuckOverlayWatcher = (domWindow, doc) => {
...
export const applyChatLoadingOverlayPatch = (domWindow = window) => {
