...
(function() {
  ...
  async function addAuraControls() {
    ...
    // ROOT CAUSE FIX: Remove duplicate aura/status sections - they already exist in HTML
    // These sections are already defined in sidepanel.html Visibility Settings section,
    // so we don't need to create them here. Remove any duplicates that may have been created.
    
    const existingAuraSection = settingsTab.querySelector('.aura-settings-section');
    if (existingAuraSection) {
      existingAuraSection.remove();
      console.log('✅ UI AURAS: Removed duplicate aura-settings-section');
    }
    
    const existingStatusSection = settingsTab.querySelector('.status-settings-section');
    if (existingStatusSection) {
      existingStatusSection.remove();
      console.log('✅ UI AURAS: Removed duplicate status-settings-section');
    }
    
    console.log('✅ UI AURAS: Duplicate sections removed (aura/status already exist in HTML)');
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      await addAuraAndStatusControls(); // ROOT CAUSE FIX: Call function that removes duplicates
