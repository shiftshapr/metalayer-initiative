 ...
 */
const getVisibilityRefresherFactory = () => {
    return (graph) => {
        return async (pageId) => {
            try {
                ...
                if (!targetPageId) {
                    const currentUrlData = graph.stateManager.getState('currentUrlData');
                ...
                if (graph.visibilityManager &&
                    ...
                    graph.logger?.debug?.('refreshVisibility: Calling VisibilityManager.refreshVisibilityAvatars', { pageId: targetPageId }, 'visibility');
                    await graph.visibilityManager.refreshVisibilityAvatars(targetPageId);
                }
                else {
                    graph.logger?.warn?.('refreshVisibility: VisibilityManager not available', null, 'visibility');
                }
            }
            catch (error) {
                graph.logger?.error?.('refreshVisibility: Error refreshing visibility', error, 'visibility');
            }
        };
    };
};
        ...
        try {
            ...
            const loadChatHistoryWrapper = async (pageIdOrRawUrl, activeCommunities) => {
                try {
                    ...
                    if (pageId) {
                        ...
                        graph.logger?.warn?.('loadChatHistoryWrapper: No valid URL available', null, 'sidepanel');
                    }
                }
                catch (error) {
            ...
            if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
                chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
                    if (request.type === 'HIGHLIGHT_SHARED_MESSAGE' && request.messageId) {
                        (async () => {
                            try {
                                graph.logger?.debug?.('Sidepanel: Received HIGHLIGHT_SHARED_MESSAGE', { messageId: request.messageId }, 'sidepanel');
                                // Get active MessageFeed
                                const { getActiveMessageFeed } = await import('../features/messages/MessageFeedIntegration.js');
                                ...
                                try {
                                    ...
                                    if (activeFeed && request.messageId) {
                                        ...
                                        graph.logger?.debug?.('Sidepanel: Entered focus mode for shared message', { messageId: request.messageId }, 'sidepanel');
                                        sendResponse({ success: true });
                                    }
                                    else {
                                        graph.logger?.warn?.('Sidepanel: No active MessageFeed for shared message after waiting', { messageId: request.messageId }, 'sidepanel');
                                        sendResponse({ success: false, error: 'No active feed' });
                                    }
                                }
                                catch (error) {
                                    graph.logger?.warn?.('Sidepanel: Timeout waiting for MessageFeed', { messageId: request.messageId, error }, 'sidepanel');
                                    sendResponse({ success: false, error: 'Timeout waiting for feed' });
                                }
                            }
                            catch (error) {
                                graph.logger?.error?.('Sidepanel: Error handling HIGHLIGHT_SHARED_MESSAGE', error, 'sidepanel');
                                sendResponse({
                                    success: false,
            ...
            if (typeof window !== 'undefined') {
                ...
                if (graph.visibilityManager) {
                    ...
                    win.VisibilityManager = graph.visibilityManager.constructor;
                    win.visibilityManager = graph.visibilityManager;
                }
                // Expose updateVisibleTab
                try {
                    const visibilityTabRenderer = await import('../features/visibility/ui/VisibilityTabRenderer.js');
                    if (visibilityTabRenderer.updateVisibleTab) {
                        const winWithVisibility = win;
                ...
                try {
                    ...
                    Logger.warn(`❌ SIDEPANEL: ${errorMsg}`, error instanceof Error ? error : null, 'sidepanel');
                }
                ...
                try {
                    ...
                    if (AvatarUtilsClass) {
                        const winWithAvatar = win;
                        winWithAvatar.AvatarUtils = AvatarUtilsClass;
                        Logger.debug('✅ SIDEPANEL: AvatarUtils exposed to window', {
                ...
                const syncCurrentUrlData = () => {
                    try {
                        const urlData = graph.stateManager.getState('currentUrlData');
                        if (urlData) {
                ...
                graph.logger?.debug?.('Window APIs exposed for diagnostics', {
