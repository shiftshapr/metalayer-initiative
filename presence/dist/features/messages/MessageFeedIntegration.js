...
let activeMessageFeed = null;
/**
 * Initialize MessageFeed for a container
 */
export async function initializeMessageFeed(options) {
 ...
 */
export function getActiveMessageFeed() {
    return activeMessageFeed;
}
/**
 * Destroy active MessageFeed
 */
export function destroyMessageFeed() {
    if (activeMessageFeed) {
        activeMessageFeed.destroy();
        activeMessageFeed = null;
 ...
 */
export async function loadMessagesViaFeed(pageId, communityId, container, supabaseClient) {
    ...
    try {
        ...
        const needsNewFeed = !feed || feed.options.pageId !== pageId;
