...
const getMessages = () => Array.from(requireDocument().querySelectorAll('.message[data-message-id]'));
const checkAvatars = (messages) => {
    const result = {
    ...
    messages.forEach((msg, index) => {
        const avatar = msg.querySelector('.avatar-container, .avatar, .message-avatar');
...
const checkInfo = (messages) => {
    ...
    messages.forEach((msg, index) => {
        ...
        if (hasTime)
            result.messagesWithTime++;
        if (hasSender)
            result.messagesWithSender++;
        if (hasContent)
            result.messagesWithContent++;
...
export const runMessageDisplayDiagnostic = async () => {
    ...
    logSection('🔧 Rendering Functions', rendering);
    const result = {
        timestamp,
        avatars,
        icons,
    ...
    window.messageDisplayDiagnosticResults =
        result;
    const issues = [];
    if (avatars.messagesWithoutAvatars > 0)
        issues.push(`${avatars.messagesWithoutAvatars} messages missing avatars`);
    if (icons.messagesWithoutIcons > 0)
        issues.push(`${icons.messagesWithoutIcons} messages missing icons`);
    if (actions.messagesWithoutIcons > 0)
        issues.push(`${actions.messagesWithoutIcons} messages missing actions`);
    if (!order.isDescending)
        issues.push('Messages not sorted descending');
    if (replies.visibleReplies > 0)
        issues.push(`${replies.visibleReplies} replies visible in default view`);
    if (info.missingInfo.length > 0)
        issues.push(`${info.missingInfo.length} messages missing info`);
...
//# sourceMappingURL=MessageDisplayDiagnostic.js.map