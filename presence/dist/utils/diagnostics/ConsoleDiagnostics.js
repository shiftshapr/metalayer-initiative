 ...
 */
const registerConsoleDiagnostics = () => {
    ...
    window.testMessagePropagation = async function () {
        ...
        try {
            ...
            const testSupabaseMessage = {
                ...
                createdAt: new Date().toISOString(), // Use camelCase for test data
                page_id: 'test-page'
            };
    ...
    window.runFullTest = async function () {
        ...
        if (window.quickStatus)
            window.quickStatus();
        if (window.testDatabase)
            await window.testDatabase();
        if (window.checkSubscriptions)
            window.checkSubscriptions();
        if (window.testEventHandlers)
            window.testEventHandlers();
        if (window.testMessage)
            await window.testMessage();
        if (window.testAura)
            window.testAura('#00ff00');
        if (window.testVisibility)
            await window.testVisibility();
        if (window.testMessagePropagation)
            await window.testMessagePropagation();
        if (window.testVisibilityUI)
            await window.testVisibilityUI();
...
//# sourceMappingURL=ConsoleDiagnostics.js.map