...
async function initializeCursorVisibilityUI() {
    ...
    let cursorModule = getGlobalWindow().cursorVisibilityModule;
    if (!cursorModule) {
        // Try importing it
        try {
            const module = await import('../../features/CursorVisibilityModule.js');
            cursorModule = module.cursorVisibilityModule || getGlobalWindow().cursorVisibilityModule;
        }
        catch (error) {
    ...
    liveButton.addEventListener('click', async () => {
        ...
        if (isLive) {
            ...
            if (parkControls)
                parkControls.style.display = 'none';
        }
        else {
            ...
            if (parkControls)
                parkControls.style.display = 'block';
        }
    });
    // Park Cursor button
    if (parkButton) {
...
//# sourceMappingURL=CursorVisibilityUI.js.map