...
export class TimelineRealtime {
    ...
    async subscribeToUser(userId) {
        ...
        try {
            ...
            if (bookmarksSub)
                subscriptions.push(bookmarksSub);
     ...
     * Subscribe to reactions
     */
    async subscribeToReactions(userId) {
        try {
            if (!this.client || !this.client.channel) {
                console.warn('Supabase client not available for realtime subscriptions');
                return null;
     ...
     * Subscribe to bookmarks
     */
    async subscribeToBookmarks(userId) {
        try {
            if (!this.client || !this.client.channel) {
                console.warn('Supabase client not available for realtime subscriptions');
                return null;
     ...
     */
    async subscribeToProfileUpdates(userId) {
        try {
            if (!this.client || !this.client.channel) {
                console.warn('Supabase client not available for realtime subscriptions');
                return null;
     ...
     */
    async subscribeToCommunityJoins(userId) {
        try {
            if (!this.client || !this.client.channel) {
                console.warn('Supabase client not available for realtime subscriptions');
                return null;
     ...
     */
    handleReactionEvent(userId, payload) {
     ...
     */
    handleBookmarkEvent(userId, payload) {
     ...
     */
    handleProfileUpdateEvent(userId, payload) {
     ...
     */
    handleCommunityJoinEvent(userId, payload) {
    ...
    normalizeMessageEvent(payload) {
        ...
        return {
            ...
            visibility: this.determineMessageVisibility(payload.new.community_id),
            data: {
     ...
     */
    normalizeReactionEvent(payload) {
        if (!payload.new)
            return null;
        return {
            id: payload.new.id,
     ...
     */
    normalizeBookmarkEvent(payload) {
        if (!payload.new)
            return null;
        return {
            id: payload.new.id,
     ...
     */
    normalizeProfileUpdateEvent(payload) {
        if (!payload.new)
            return null;
        return {
            id: payload.new.id,
            ...
            userId: payload.new.user_id,
            timestamp: payload.new.created_at,
            visibility: this.determineProfileUpdateVisibility(payload.new.field_name),
            data: {
     ...
     */
    normalizeCommunityJoinEvent(payload) {
        if (!payload.new)
            return null;
        return {
            id: payload.new.id,
            ...
            timestamp: payload.new.joinedAt,
...
//# sourceMappingURL=TimelineRealtime.js.map