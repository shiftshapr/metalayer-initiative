...
import { FeedFilters } from './components/FeedFilters.js';
...
class TimelineApp {
    constructor() {
        ...
        this.persistenceMultiSelect = null;
        ...
        if (feedFilterButtons) {
            this.feedFilters = new FeedFilters(feedFilterButtons, () => this.query.getAuthHeaders(), (filters) => this.handleFeedFilterChange(filters));
        }
        // Initialize right sidebar (Column C)
        this.sidebarRight = new SidebarRight('global-search', 'rewards', 'top-canopies', 'top-timeline', 'top-posts', 'top-bridgers', () => this.query.getAuthHeaders(), (query) => this.handleSearch(query));
        // Current user info
    ...
    async init() {
        try {
            ...
            document.addEventListener('timeline:filters:changed', async (e) => {
                const customEvent = e;
    ...
    setupEventListeners() {
        ...
        if (persistenceSelectContainer) {
            ...
            persistenceSelectContainer.addEventListener('multiselect:change', (e) => {
                const customEvent = e;
                ...
                const persistence = (selectedValues[0] || 'all');
                if (this.currentProfileId) {
     ...
     */
    async handleFeedFilterChange(filters) {
     ...
     * Load "Following" feed
     */
    async loadFollowingFeed(_filters) {
        if (!this.currentUserId) {
     ...
     */
    async loadCommunitiesFeed(filters) {
