...
declare const AvatarUtils: {
  getAvatarUrl(user: User, context: string): Promise<{ avatarUrl: string; userName: string; userHandle: string; source: string }>;
...
export class TimelineView {
  ...
  async render(timeline: TimelineData, options: TimelineViewOptions = {}): Promise<void> {
    ...
    items.forEach(itemHtml => {
      if (!itemHtml) return; // Skip null/undefined items
  ...
  async renderActivity(activity: Activity): Promise<string | null> {
    // For messages, use the unified message element from CanopiModule (RED-LINE: imported, not window)
    const createUnifiedMessageElement = (window as any).createUnifiedMessageElement;
    if (activity.type === 'message' && typeof createUnifiedMessageElement === 'function') {
      try {
        ...
        const addMessageActionListenersFn = (window as any).addMessageActionListeners;
        ...
        if (message.reactions && Array.isArray(message.reactions) && message.reactions.length > 0) {
          setTimeout(async () => {
            ...
            const updateReactionDisplayFn = (window as any).updateReactionDisplay;
        ...
        messageElement.addEventListener('click', (e: Event) => {
          // Don't navigate if clicking on action buttons
          if ((e.target as HTMLElement).closest('.message-actions-new, .message-footer-actions, button, .action-item, .action-dropdown')) {
            return;
          }
  ...
  private getStatusIcon(availability: string | null | undefined): string {
  ...
  private getVisibilityBadge(visibility: string): string {
  ...
  private setupEventListeners(): void {
    ...
    this.container.querySelectorAll('.timeline-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        const activityType = (item as HTMLElement).dataset.activityType;
        this.handleActivityClick(activityId || '', activityType || '', item as HTMLElement);
      });
  ...
  private handleActivityClick(activityId: string, activityType: string, element: HTMLElement): void {
