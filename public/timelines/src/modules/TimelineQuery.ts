...
// RED-LINE: Import AuthManager directly, NOT from window
// @ts-ignore - Runtime path, TypeScript can't resolve
import { AuthManager } from '/presence/features/AuthManager.js';
import type { 
  TimelineQueryOptions, 
  TimelineApiResponse, 
  MultipleTimelinesApiResponse,
  User 
} from '../types';

// Type for AuthManager (since it's not typed)
...
export class TimelineQuery {
  ...
  constructor(baseUrl: string = '/api/timelines') {
  ...
  getAuthHeaders(): Record<string, string> {
    ...
    if (avatar) headers['X-User-Avatar'] = avatar;
  ...
  getCurrentUserName(): string | null {
    ...
    if (typeof AuthManager !== 'undefined') {
      try {
        ...
        const user = authManager.getCurrentUser();
        if (user) {
          return user.name || (user as any).displayName || (user as any).user_metadata?.full_name || null;
        }
      } catch (error) {
        // Ignore
      }
    ...
    if (window.currentUser) {
      return window.currentUser.name || (window.currentUser as any).displayName || null;
    }
  ...
  getCurrentUserAvatar(): string | null {
    ...
    if (typeof AuthManager !== 'undefined') {
      try {
        ...
        const user = authManager.getCurrentUser();
        if (user) {
          return user.avatarUrl || user.avatar_url || (user as any).user_metadata?.avatar_url || (user as any).picture || null;
        }
      } catch (error) {
        // Ignore
      }
   ...
   */
  async getTimeline(identifier: string, options: TimelineQueryOptions = {}): Promise<TimelineApiResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error((error as any).message || `HTTP ${response.status}: ${response.statusText}`);
      }
      ...
      if (!data.success) {
        throw new Error((data as any).message || 'Failed to fetch timeline');
      }
   ...
   */
  async getMultipleTimelines(userIds: string[], options: TimelineQueryOptions = {}): Promise<MultipleTimelinesApiResponse> {
    ...
    try {
      ...
      const data = await response.json() as MultipleTimelinesApiResponse;
      
      if (!data.success) {
        throw new Error((data as any).message || 'Failed to fetch timelines');
      }
