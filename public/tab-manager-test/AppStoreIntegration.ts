...
export class AppStoreIntegration {
  ...
  private apps: SDKApp[] = [];
  private filters: AppStoreFilters = {};
   ...
   */
  async getAppReviews(appId: string, limit?: number, offset?: number): Promise<any[]> {
   ...
   */
  async verifyProvenance(appId: string): Promise<DigitalProvenance | null> {
    const app = this.getAppById(appId);
